...
struct  wifi_user_conf {
	...
	unsigned char rtw_edcca_th_l2h_ini;
...
//#endif /* CONFIG_WMMPS_STA */
	...
	*/
	...
	unsigned char
	softap_addr_offset_idx;	 ///< 1~5: Softap's mac address will equal chip's specified mac address offset + 1. The lowest bit of mac[0] is 1, which represents the multicast address, so skip mac[0]
	unsigned char rx_ampdu_num;
	unsigned char cfg80211;

	/* WPS */
	unsigned char wps_retry_count;
	unsigned int  wps_retry_interval; // in ms
};
