...
void inic_ipc_wifi_on(void)
{
	u32 param_buf[1];
	param_buf[0] = 0;
	inic_ipc_host_api_send_msg(IPC_API_WIFI_ON, param_buf, 1);
}

int inic_ipc_wifi_scan(rtw_scan_param_t *scan_param, u32 ssid_length, u32 block)
{
	int ret = 0;
	u32 param_buf[3];
	dma_addr_t dma_addr_scan_param = 0, dma_addr_ssid = 0, dma_addr_ch_list = 0;
	struct device *pdev = piihp_priv->papi_ipc_ch->pdev;
	printk("ss=%x,%x,%x\n", scan_param, scan_param->ssid, scan_param->channel_list);

	if (scan_param->ssid && (ssid_length > 0)) {
		dma_addr_ssid = dma_map_single(pdev, scan_param->ssid, ssid_length, DMA_TO_DEVICE);
		if (dma_mapping_error(pdev, dma_addr_ssid)) {
			printk(KERN_ERR "%s: mapping dma error!\n", __func__);
			return -1;
		}
