...
u16 rtw_get_efuse_mask_arraylen(PADAPTER pAdapter)
{
	...
	if (IS_HARDWARE_TYPE_8730A(pAdapter)) {
		return GET_MASK_ARRAY_LEN(8730A, _MAXI);
	}
#endif/*CONFIG_AXI_HCI*/
	return 0;
}
...
void rtw_efuse_analyze(PADAPTER	padapter, u8 Type, u8 Fake)
{
	...
	for (bank = startBank; bank <= endBank; bank++) {
		...
		while ((efuseHeader != 0xFF) && ((efuseType == EFUSE_WIFI && (eFuse_Addr < maprawlen)) || (efuseType == EFUSE_BT &&
...
)
{
	...
	total = BANK_NUM;
	for (bank = 1; bank <= total; bank++) { /* 8730a Max bake 0~2 */
		eFuse_Addr = 0;
