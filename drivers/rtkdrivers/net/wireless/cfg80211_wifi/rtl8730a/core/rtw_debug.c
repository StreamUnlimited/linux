...
void dump_drv_cfg(void *sel)
{
	...
	RTW_PRINT_SEL(sel, "CONFIG_AXI_TX_POLLING\n");
#endif
...
#if defined(CONFIG_RTL8822C) || defined(CONFIG_RTL8814B) || defined(CONFIG_RTL8730A)
...
#endif /* CONFIG_RTL8822C || CONFIG_RTL8814B || CONFIG_8730A */
	...
	if (check_fwstate(pmlmepriv, WIFI_STATION_STATE)) {
		...
		if (psta) {
			...
			if (pmlmeinfo->ht_vht_received & BIT(1)) {
				...
				RTW_PRINT_SEL(m, "vht_ldpc_cap=0x%x, vht_stbc_cap=0x%x, vht_beamform_cap=0x%x\n", pvhtpriv->ldpc_cap_infra_ap, pvhtpriv->stbc_cap_infra_ap,
					      pvhtpriv->beamform_cap_infra_ap);
...
int proc_get_ap_info(struct seq_file *m, void *v)
{
	...
	if (psta) {
		...
		RTW_PRINT_SEL(m, "sta's macaddr:" MAC_FMT "\n", MAC_ARG(psta->cmn.mac_addr));
		RTW_PRINT_SEL(m, "cur_channel=%d, cur_bwmode=%d(%s), cur_ch_offset=%d\n", pmlmeext->cur_channel, pmlmeext->cur_bwmode, ch_width_str(pmlmeext->cur_bwmode),
	...
	for (i = 0; i < NUM_STA; i++) {
		...
		while ((rtw_end_of_queue_search(phead, plist)) == _FALSE) {
			...
			/* if(extra_arg == psta->cmn.aid) */
			{
				...
				RTW_PRINT_SEL(m, "tx_bitrate_100kbps=%u\n", rtw_desc_rate_to_bitrate(psta->cmn.bw_mode, rtw_get_current_tx_rate(padapter, psta),
...
ssize_t proc_set_pci_bridge_conf_space(struct file *file, const char __user *buffer, size_t count, loff_t *pos, void *data)
{
	...
	struct pci_dev  *pdev = pdvobjpriv->pdev;
...
int proc_get_rx_ring(struct seq_file *m, void *v)
{
	...
	for (i = 0; i < precvpriv->rxringcount; i++) {
		...
		RTW_PRINT_SEL(m, "  desc[%03d]: %08x, rx_buf[%03d]: 0x%08x\n",
