...
u8 _rtw_sys_read8(_adapter *adapter, u32 base, u32 addr)
{
	u8 r_val;
	/* struct	io_queue  	*pio_queue = (struct io_queue *)adapter->pio_queue; */
	struct io_priv *pio_priv = &adapter->iopriv;
	struct	intf_hdl		*pintfhdl = &(pio_priv->intf);
	u8(*_read8)(struct intf_hdl * pintfhdl, u32 base, u32 addr);
	_read8 = pintfhdl->io_ops._sys_read8;

	r_val = _read8(pintfhdl, base, addr);
	return r_val;
}

u16 _rtw_sys_read16(_adapter *adapter, u32 base, u32 addr)
{
	u16 r_val;
	/* struct	io_queue  	*pio_queue = (struct io_queue *)adapter->pio_queue; */
	struct io_priv *pio_priv = &adapter->iopriv;
	struct	intf_hdl		*pintfhdl = &(pio_priv->intf);
	u16(*_read16)(struct intf_hdl * pintfhdl, u32 base, u32 addr);
	_read16 = pintfhdl->io_ops._sys_read16;

	r_val = _read16(pintfhdl, base, addr);
	return rtw_le16_to_cpu(r_val);
}

u32 _rtw_sys_read32(_adapter *adapter, u32 base, u32 addr)
{
	u32 r_val;
	/* struct	io_queue  	*pio_queue = (struct io_queue *)adapter->pio_queue; */
	struct io_priv *pio_priv = &adapter->iopriv;
	struct	intf_hdl		*pintfhdl = &(pio_priv->intf);
	u32(*_read32)(struct intf_hdl * pintfhdl, u32 base, u32 addr);
	_read32 = pintfhdl->io_ops._sys_read32;

	r_val = _read32(pintfhdl, base, addr);
	return rtw_le32_to_cpu(r_val);

}

int _rtw_sys_write8(_adapter *adapter, u32 base, u32 addr, u8 val)
{
	/* struct	io_queue  	*pio_queue = (struct io_queue *)adapter->pio_queue; */
	struct io_priv *pio_priv = &adapter->iopriv;
	struct	intf_hdl		*pintfhdl = &(pio_priv->intf);
	int (*_write8)(struct intf_hdl * pintfhdl, u32 base, u32 addr, u8 val);
	int ret;
	_write8 = pintfhdl->io_ops._sys_write8;

	ret = _write8(pintfhdl, base, addr, val);

	return RTW_STATUS_CODE(ret);
}
int _rtw_sys_write16(_adapter *adapter, u32 base, u32 addr, u16 val)
{
	/* struct	io_queue  	*pio_queue = (struct io_queue *)adapter->pio_queue; */
	struct io_priv *pio_priv = &adapter->iopriv;
	struct	intf_hdl		*pintfhdl = &(pio_priv->intf);
	int (*_write16)(struct intf_hdl * pintfhdl, u32 base, u32 addr, u16 val);
	int ret;
	_write16 = pintfhdl->io_ops._sys_write16;

	val = rtw_cpu_to_le16(val);
	ret = _write16(pintfhdl, base, addr, val);

	return RTW_STATUS_CODE(ret);
}
int _rtw_sys_write32(_adapter *adapter, u32 base, u32 addr, u32 val)
{
	/* struct	io_queue  	*pio_queue = (struct io_queue *)adapter->pio_queue; */
	struct io_priv *pio_priv = &adapter->iopriv;
	struct	intf_hdl		*pintfhdl = &(pio_priv->intf);
	int (*_write32)(struct intf_hdl * pintfhdl, u32 base, u32 addr, u32 val);
	int ret;
	_write32 = pintfhdl->io_ops._sys_write32;

	val = rtw_cpu_to_le32(val);
	ret = _write32(pintfhdl, base, addr, val);
