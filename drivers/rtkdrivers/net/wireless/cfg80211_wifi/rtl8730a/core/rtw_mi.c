...
u8 rtw_mi_check_status(_adapter *adapter, u8 type)
{
	...
	switch (type) {
	case MI_LINKED:
		if (MSTATE_STA_LD_NUM(iface_state) || MSTATE_AP_NUM(iface_state) || MSTATE_ADHOC_NUM(iface_state) ||
		    MSTATE_MESH_NUM(iface_state)) { /*check_fwstate(&iface->mlmepriv, WIFI_ASOC_STATE)*/
			ret = _TRUE;
		}
		break;
	case MI_ASSOC:
