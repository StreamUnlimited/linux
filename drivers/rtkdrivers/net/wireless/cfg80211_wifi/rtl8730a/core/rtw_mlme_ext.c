...
{
#if (defined(CONFIG_PCI_HCI) || defined(CONFIG_AXI_HCI)) && !defined(CONFIG_AXI_BCN_POLLING)
#ifdef CONFIG_FW_HANDLE_TXBCN
	...
	/* CONFIG_AXI_BCN_POLLING is for pci interface beacon polling mode */
#if defined(CONFIG_USB_HCI) || defined(CONFIG_SDIO_HCI) || defined(CONFIG_GSPI_HCI)|| defined(CONFIG_AXI_BCN_POLLING)
	u8 bxmitok = _FALSE;
	int issue = 0;
	int poll = 0;
	systime start = rtw_get_current_time();
...
#endif
		do {
#if defined(CONFIG_AXI_BCN_POLLING)
			issue_beacon(padapter, 0);
#else
