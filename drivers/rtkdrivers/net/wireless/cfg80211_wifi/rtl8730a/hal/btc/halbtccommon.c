		   ...
		   boolean tx_limit_en,  boolean ampdu_limit_en)
{
	...
	if (!coex_sta->wl_tx_limit_en) {
		coex_sta->wl_arfb1 = btc->btc_read_4byte(btc, REG_DARFRC_L);
		coex_sta->wl_arfb2 = btc->btc_read_4byte(btc, REG_DARFRC_H);

		coex_sta->wl_txlimit = btc->btc_read_2byte(btc,
				       REG_RETRY_LIMIT_SIFS + 2);
	}
	...
	if (tx_limit_en) {
		...
		if (link_info_ext->num_of_active_port <= 1)
			btc->btc_write_1byte_bitmask(btc, REG_BNDY_LIFETIME_TAIL + 2, 0xf,
						     0xf);
		else
			btc->btc_write_1byte_bitmask(btc, REG_BNDY_LIFETIME_TAIL + 2, 0xf,
						     0x0);
		...
		btc->btc_write_2byte(btc, REG_RETRY_LIMIT_SIFS + 2, 0x0808);

		btc->btc_get(btc, BTC_GET_BL_WIFI_UNDER_B_MODE, &wl_b_mode);

		/* Auto rate fallback step within 8 retry*/
		if (wl_b_mode) {
			btc->btc_write_4byte(btc, REG_DARFRC_L, 0x1000000);
			btc->btc_write_4byte(btc, REG_DARFRC_H, 0x1010101);
		} else {
			btc->btc_write_4byte(btc, REG_DARFRC_L, 0x1000000);
			btc->btc_write_4byte(btc, REG_DARFRC_H, 0x4030201);
		}
	} else {
		 ...
		 * if tx is always break by GNT_BT.
		 */
		btc->btc_write_1byte_bitmask(btc, REG_BNDY_LIFETIME_TAIL + 2, 0xf, 0x0);

		/* Recovery Max Tx retry limit*/
		btc->btc_write_2byte(btc, REG_RETRY_LIMIT_SIFS + 2,
				     coex_sta->wl_txlimit);
		btc->btc_write_4byte(btc, REG_DARFRC_L, coex_sta->wl_arfb1);
		btc->btc_write_4byte(btc, REG_DARFRC_H, coex_sta->wl_arfb2);
	}
...
rtw_btc_update_wl_link_info(struct btc_coexist *btc, u8 reason)
{
	...
	if (btc->chip_interface != BTC_INTF_SDIO)
		btc->btc_write_1byte_bitmask(btc, REG_BCN_AMPDU_CTCL,
...
rtw_btc_init_hw_config(struct btc_coexist *btc, boolean wifi_only)
{
	...
	/* TBTT enable */
	btc->btc_write_1byte_bitmask(btc, REG_BCN_CTRL_PORT0, BIT_P0_EN_BCN_FUNCTION,
				     0x1);
...
void rtw_btc_ex_power_on_setting(struct btc_coexist *btc)
{
	...
	/* enable BB, REG_WL_FUNC_EN to write reg correctly. */
	btc->btc_write_1byte_bitmask(btc, REG_WL_FUNC_EN,
				     BIT_FEN_BBRSTB_V2 | BIT_FEN_BB_GLB_RSTN_V2, 0x3);
