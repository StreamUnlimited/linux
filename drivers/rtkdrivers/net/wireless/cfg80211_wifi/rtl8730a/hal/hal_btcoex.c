...
void halbtcoutsrc_DisplayWifiStatus(PBTC_COEXIST pBtCoexist)
{
	...
	if (wifiLinkStatus & WIFI_STA_CONNECTED) {
		CL_SPRINTF(cliBuf, BT_TMP_BUF_SIZE, "\r\n %-35s = %s/ "MAC_FMT"", "IOT Peer/BSSID", GLBtcIotPeerString[padapter->mlmeextpriv.mlmext_info.assoc_AP_vendor],
...
void halbtcoutsrc_reduce_wl_tx_power(void *pBtcContext, s8 tx_power)
{
	...
	if (IS_HARDWARE_TYPE_8822C(pBtCoexist->Adapter) || IS_HARDWARE_TYPE_8730A(pBtCoexist->Adapter)) {
...
u8 EXhalbtcoutsrc_BindBtCoexWithAdapter(void *padapter)
{
	...
	if (IS_HARDWARE_TYPE_8822C(padapter)) {
		...
		pBtCoexist->chip_type = BTC_CHIP_RTL8730A;
		pBtCoexist->chip_para = &btc_chip_para_8730a;
	}
#endif
#endif
	else {
...
u8 EXhalbtcoutsrc_InitlizeVariables(void *padapter)
{
	...
	pwifionlycfg->chip_interface = BTC_INTF_AXI;
