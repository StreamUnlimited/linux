...
void dump_chip_info(HAL_VERSION	ChipVersion)
{
	...
	if (IS_8730A_SERIES(ChipVersion)) {
		cnt += sprintf((buf + cnt), "Chip Version Info: CHIP_8730A_");
	} else {
...
s32 c2h_evt_read_88xx(_adapter *adapter, u8 *buf)
{
	...
	/* Read ID, LEN, SEQ */
	SET_C2H_ID_88XX(buf, rtw_read8(adapter, REG_C2HEVT));
	...
	for (i = 0; i < C2H_PLEN_88XX(buf); i++) {
		*(C2H_PAYLOAD_88XX(buf) + i) = rtw_read8(adapter, REG_C2HEVT + 2 + i);
	}
...
#if defined(CONFIG_8730A)
	if (IS_8730A_SERIES(hal_data->version_id)) {
...
int hal_read_mac_hidden_rpt(_adapter *adapter)
{
	...
	if ((hci_type == RTW_USB || hci_type == RTW_AXI)
	...
	rtw_write8(adapter, REG_C2HEVT, C2H_DEFEATURE_RSVD);

	/* download FW */
	pHalData->not_xmitframe_fw_dl = 1;
	ret_fwdl = rtw_hal_fw_en(adapter, _TRUE);
	...
	do {
		cnt++;
		id = rtw_read8(adapter, REG_C2HEVT);
	...
	if (id == C2H_MAC_HIDDEN_RPT) {
		...
		for (i = 0; i < MAC_HIDDEN_RPT_LEN + MAC_HIDDEN_RPT_2_LEN; i++) {
			mac_hidden_rpt[i] = rtw_read8(adapter, REG_C2HEVT + 2 + i);
		}
	}

	/* inform FW mac hidden rpt has read */
	rtw_write8(adapter, REG_C2HEVT, C2H_DBG);
...
static u32 hw_bcn_ctrl_addr(_adapter *adapter, u8 hw_port)
{
	...
	switch (hw_port) {
	case HW_PORT0:
		return REG_BCN_CTRL_PORT0;
	case HW_PORT1:
		return REG_BCN_CTRL_PORT1;
	}
...
u32 rtw_sec_read_cam(_adapter *adapter, u8 addr)
{
	...
	rtw_write32(adapter, REG_KEYCAMCMD, CAM_POLLINIG | addr);
	...
	while (1) {
		...
		cnt++;
		if (0 == (rtw_read32(adapter, REG_KEYCAMCMD) & CAM_POLLINIG)) {
			break;
		}
...
void rtw_sec_write_cam(_adapter *adapter, u8 addr, u32 wdata)
{
	...
	rtw_write32(adapter, REG_KEYCAM_WD, wdata);
	rtw_write32(adapter, REG_KEYCAMCMD, CAM_POLLINIG | CAM_WRITE | addr);
...
void rtw_ap_multi_bcn_cfg(_adapter *adapter)
{
	...
	rtw_write16(adapter, REG_RXFLTMAP, 0xFFFF);

	/*Disable Port0's beacon function*/
	rtw_write8(adapter, REG_BCN_CTRL_PORT0, rtw_read8(adapter, REG_BCN_CTRL_PORT0) & ~BIT_P0_EN_BCN_FUNCTION);
	/*Reset Port0's TSF*/
	...
	/*rtw_write8(adapter, REG_BCN_AMPDU_CTCL, rtw_read8(adapter, REG_BCN_AMPDU_CTCL) | BIT_EN_BCN_PKT_REL);*/

	/*Enable Port0's beacon function*/
	rtw_write8(adapter, REG_BCN_CTRL_PORT0,
		   rtw_read8(adapter, REG_BCN_CTRL_PORT0) | BIT_P0_DIS_RX_BSSID_FIT | BIT_P0_EN_BCN_RPT | BIT_P0_DIS_TSF_UDT  | BIT_P0_EN_BCN_FUNCTION);
...
void rtw_ap_multi_bcn_cfg(_adapter *adapter)
{
	...
	rtw_write8(adapter, REG_BCN_CTRL_PORT0, DIS_TSF_UDT);
#else
	rtw_write8(adapter, REG_BCN_CTRL_PORT0, DIS_TSF_UDT | DIS_BCNQ_SUB);
#endif
	/*enable to rx data frame*/
	rtw_write16(adapter, REG_RXFLTMAP, 0xFFFF);
...
#if defined(CONFIG_RTL8822B) || defined(CONFIG_RTL8821C) || defined(CONFIG_RTL8822C)
	rtw_write8(adapter, REG_BCN_CTRL_PORT0, BIT_P0_DIS_RX_BSSID_FIT | BIT_P0_EN_BCN_RPT | BIT_P0_DIS_TSF_UDT | BIT_P0_EN_BCN_FUNCTION);
#else
	rtw_write8(adapter, REG_BCN_CTRL_PORT0, (DIS_TSF_UDT | EN_BCN_FUNCTION | EN_TXBCN_RPT | DIS_BCNQ_SUB));
#endif
	...
	if (IS_HARDWARE_TYPE_8821(adapter) || IS_HARDWARE_TYPE_8192E(adapter)) { /* select BCN on port 0 for DualBeacon*/
		rtw_write8(adapter, REG_BCN_AMPDU_CTCL, rtw_read8(adapter, REG_BCN_AMPDU_CTCL) & (~BIT_BCN_PORT_SEL));
	}
...
void rtw_hal_update_tx_aclt(_adapter *adapter)
{
	...
	lt_en_ori = rtw_read8(adapter, REG_BNDY_LIFETIME_TAIL + 2);
	...
	if (lt_en != lt_en_ori) {
		rtw_write8(adapter, REG_BNDY_LIFETIME_TAIL + 2, lt_en);
	}
	if (lt != lt_ori) {
...
void hw_var_port_switch(_adapter *adapter)
{
	...
	*/
	...
	bcn_ctrl = rtw_read8(adapter, REG_BCN_CTRL_PORT0);
	...
	rtw_write8(adapter, REG_BCN_CTRL_PORT0, (bcn_ctrl & (~EN_BCN_FUNCTION)) | DIS_TSF_UDT);
	...
	/* write port0 */
	rtw_write8(adapter, REG_BCN_CTRL_PORT0, bcn_ctrl_1 & ~EN_BCN_FUNCTION);
	for (i = 0; i < 2; i++) {
	...
	if (IS_HARDWARE_TYPE_8723B(adapter) || IS_HARDWARE_TYPE_8703B(adapter)
	    || IS_HARDWARE_TYPE_8730A(adapter)) {
...
#endif
	rtw_write8(adapter, REG_BCN_CTRL_PORT0, bcn_ctrl_1);
...
void rtw_hal_switch_gpio_wl_ctrl(_adapter *padapter, u8 index, u8 enable)
{
	...
	if (IS_8730A_SERIES(pHalData->version_id) || IS_8192F_SERIES(pHalData->version_id)
...
bool rtw_read_from_frame_mask(_adapter *adapter, u8 idx)
{
	...
	if (_rtw_wow_chk_cap(adapter, WOW_CAP_HALMAC_ACCESS_PATTERN_IN_TXFIFO)) {
		/* 8730A cannot indirect access tx fifo
...
static void hw_var_set_bcn_func(_adapter *adapter, u8 enable)
{
	...
	if (adapter->hw_port == HW_PORT1) {
		bcn_ctrl_reg = REG_BCN_CTRL_PORT1;
	} else
#endif
		bcn_ctrl_reg = REG_BCN_CTRL_PORT0;

	if (enable) {
		rtw_write8(adapter, bcn_ctrl_reg, (BIT_P0_EN_BCN_FUNCTION | BIT_P0_TXBCN_RPT_ENABLE));
	} else {
		u8 val8;

		val8 = rtw_read8(adapter, bcn_ctrl_reg);
		val8 &= ~(BIT_P0_EN_BCN_FUNCTION | BIT_P0_TXBCN_RPT_ENABLE);

#ifdef CONFIG_BT_COEXIST
		if (GET_HAL_DATA(adapter)->EEPROMBluetoothCoexist == 1) {
			/* Always enable port0 beacon function for PSTDMA */
			if (REG_BCN_CTRL_PORT0 == bcn_ctrl_reg) {
...
static void hw_var_set_mlme_disconnect(_adapter *adapter)
{
...
#endif
		rtw_write16(adapter, REG_RXFLTMAP, 0x0000);
	...
	if (adapter->hw_port == HW_PORT1) {
		...
		if (!IS_HARDWARE_TYPE_8730A(adapter)
		    && !IS_HARDWARE_TYPE_8192F(adapter)
		    && !IS_HARDWARE_TYPE_8710B(adapter)
		   ) {
			/* disable Port1's beacon function */
			val8 = rtw_read8(adapter, REG_BCN_CTRL_PORT1);
			val8 &= ~EN_BCN_FUNCTION;
			rtw_write8(adapter, REG_BCN_CTRL_PORT1, val8);
		}
	} else
#endif
	{
...
static void hw_var_set_mlme_sitesurvey(_adapter *adapter, u8 enable)
{
	...
	if (enable) { /* under sitesurvey */
		...
		*/
		rtw_write16(adapter, REG_RXFLTMAP, value_rxfltmap2);
...
static void hw_var_set_mlme_join(_adapter *adapter, u8 type)
{
	...
	if (type == 0) {
		...
		if (rtw_mi_check_status(adapter, MI_LINKED) == _FALSE) {
			rtw_write16(adapter, REG_RXFLTMAP, 0x00);
		}
	...
	rtw_write16(adapter, REG_RETRY_LIMIT_SIFS + 2, val16);
...
static void rtw_hal_correct_tsf(_adapter *padapter, u8 hw_port, u64 tsf)
{
	if (hw_port == HW_PORT0) {
		...
		rtw_write8(padapter, REG_BCN_CTRL_PORT0, rtw_read8(padapter, REG_BCN_CTRL_PORT0) & (~BIT_P0_EN_BCN_FUNCTION));

		rtw_write32(padapter, REG_P0_TSFTR_L, tsf);
		rtw_write32(padapter, REG_P0_TSFTR_H, tsf >> 32);
		...
		/*disable related TSF function*/
		rtw_write8(padapter, REG_BCN_CTRL_PORT1, rtw_read8(padapter, REG_BCN_CTRL_PORT1) & (~BIT_P1_EN_BCN_FUNCTION));

		rtw_write32(padapter, REG_P1_TSFTR_L, tsf);
		rtw_write32(padapter, REG_P1_TSFTR_H, tsf >> 32);
...
u64 rtw_hal_get_tsftr_by_port(_adapter *adapter, u8 port)
{
	...
	case RTL8822C: {
		...
		tsftr = rtw_read32(adapter, REG_P0_TSFTR_H);
		tsftr = tsftr << 32;
	...
	|| defined(CONFIG_RTL8723B) || defined(CONFIG_RTL8703B) || defined(CONFIG_RTL8730A) \
...
u8 SetHwReg(_adapter *adapter, u8 variable, u8 *val)
{
	...
	case HW_VAR_SET_RTS_BW: {
		...
		u8 temp;
		if (*val) {
			temp = ((rtw_read8(adapter, REG_CFEND_RATE_SC_CTRL)) | BIT5);
		} else {
			temp = ((rtw_read8(adapter, REG_CFEND_RATE_SC_CTRL)) & (~BIT5));
		}
		rtw_write8(adapter, REG_CFEND_RATE_SC_CTRL, temp);
		/*RTW_INFO("HW_VAR_SET_RTS_BW	val=%u REG480=0x%x\n", *val, rtw_read8(adapter, REG_CFEND_RATE_SC_CTRL));*/
#endif
	}
	break;
...
void GetHwReg(_adapter *adapter, u8 variable, u8 *val)
{
	...
	case HW_VAR_CAM_EMPTY_ENTRY: {
		...
		for (i = 0; i < CAM_CONTENT_COUNT; i++) {
			...
			rtw_write32(adapter, REG_KEYCAM_WD, ulContent);  /* delay_ms(40); */
			rtw_write32(adapter, REG_KEYCAMCMD, ulCommand);  /* delay_ms(40); */
		}
	}
#endif
...
void rtw_hal_check_rxfifo_full(_adapter *adapter)
{
	...
	if (regsty->check_hw_status == 1) {
		...
		if (IS_8188E(pHalData->version_id) ||
		    ...
		    IS_8730A_SERIES(pHalData->version_id)) {
...
int hal_efuse_macaddr_offset(_adapter *adapter)
{
	...
	case RTL8730A:
		if (interface_type == RTW_AXI) {
			addr_offset = EEPROM_MAC_ADDR_8730A;
		}
		break;
#endif /* CONFIG_RTL8730A */
	}
...
void rtw_bb_rf_gain_offset(_adapter *padapter)
{
	...
	if (value == _TRUE) {
		RTW_INFO("8730A Offset RF Gain.\n");
		RTW_INFO("8730A Offset RF Gain. EEPROMRFGainVal=0x%x\n", \
			 pHalData->EEPROMRFGainVal);
		if (pHalData->EEPROMRFGainVal != 0xff) {
			res = rtw_hal_read_rfreg(padapter, RF_PATH_A,
						 REG_RF_BB_GAIN_OFFSET, 0xffffffff);
			RTW_INFO("Offset RF Gain. reg 0x55=0x%x\n", res);
			res &= 0xfff07fff; // RF 0x55 [19:15]

			res |= (pHalData->EEPROMRFGainVal & BIT(0)) << 19; //powerbit EEPROMRFGainVal[0]==>RF 0x55[19]
			res |= (pHalData->EEPROMRFGainVal & (BIT(1) | BIT(2) | BIT(3))) << 15; // EEPROMRFGainVal[3:1] ==>RF 0x55[18:15]
			RTW_INFO("Offset RF Gain. res=0x%x\n", res);
...
void dm_DynamicUsbTxAgg(_adapter *padapter, u8 from_timer)
{
	...
	if (IS_HARDWARE_TYPE_8822BU(padapter) || IS_HARDWARE_TYPE_8821CU(padapter)
	    ...
	    || IS_HARDWARE_TYPE_8730AU(padapter)) {
...
void rtw_reset_mac_rx_counters(_adapter *padapter)
{
	...
	if (IS_HARDWARE_TYPE_8703B(padapter) ||
	    IS_HARDWARE_TYPE_8730A(padapter) ||
...
#ifdef CONFIG_RTL8730A
	case RTL8730A:
		rtl8730a_init_hal_spec(adapter);
		break;
#endif
	default:
...
#else
	rtw_write32(padapter, REG_FWHW_TXQ_HWSSN_CTRL,
		    rtw_read32(padapter, REG_FWHW_TXQ_HWSSN_CTRL) | BIT_EN_BCNQ_DL);
#endif
...
#endif
		rtw_write32(padapter, REG_TBTT_PROHIBIT, \
			    BIT_SET_TBTT_HOLD_TIME(rtw_read32(padapter, REG_TBTT_PROHIBIT), \
						   TBTT_PROHIBIT_HOLD_TIME));
}
