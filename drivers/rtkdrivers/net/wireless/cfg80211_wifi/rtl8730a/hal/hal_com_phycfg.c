...
static const struct map_t rtl8730a_pg_txpwr_def_info =
	MAP_ENT(0xB8, 1, 0xFF
		, MAPSEG_ARRAY_ENT(0x10, 82,
				   0x33, 0x33, 0x33, 0x33, 0x33, 0x33, 0x33, 0x33, 0x33, 0x33, 0x33, 0x02, 0x00, 0x00, 0xFF, 0xFF,
				   0xFF, 0xFF, 0x33, 0x33, 0x33, 0x33, 0x33, 0x33, 0x33, 0x33, 0x33, 0x33, 0x33, 0x33, 0x33, 0x33,
...
#ifdef CONFIG_RTL8723D
#define EEPROM_TX_PWR_CALIBRATE_RATE EEPROM_TX_PWR_CALIBRATE_RATE_8723D
#endif
#ifdef CONFIG_RTL8814B
...
#endif
#ifdef CONFIG_RTL8730A
#define EEPROM_TX_PWR_CALIBRATE_RATE EEPROM_TX_PWR_CAL_RATE_8730A
#endif
...
#ifdef CONFIG_RTL8730A
	case RTL8730A:
		map = &rtl8730a_pg_txpwr_def_info;
		break;
#endif
	}
					...
					for (n = 0; n < ch_num; n++) {
						...
						do {
							for (bw_pos = bw; bw_pos >= CHANNEL_WIDTH_40; bw_pos--) {
								cch_by_bw[bw_pos - 1] = rtw_get_scch_by_cch_offset(cch_by_bw[bw_pos], bw_pos,
											offset_by_bw & BIT(bw_pos) ? HAL_PRIME_CHNL_OFFSET_UPPER : HAL_PRIME_CHNL_OFFSET_LOWER);
							}
...
void dump_tx_power_index_inline(void *sel, _adapter *adapter, u8 rfpath, enum channel_width bw, u8 cch, enum MGN_RATE rate, u8 pwr_idx,
				struct txpwr_idx_comp *tic)
{
	struct hal_spec_t *hal_spec = GET_HAL_SPEC(adapter);
	...
	if (tic->utarget == hal_spec->txgi_max) {
		RTW_PRINT_SEL(sel,
			      "TXPWR: [%c][%s]cch:%u, %s %uT, idx:%u(0x%02x) = base(%d) + min((byr(%d) + btc(%d) + extra(%d)), rlmt(%d), lmt(%d), ulmt(%d)) + tpc(%d) + tpt(%d) + dpd(%d)\n"
...
bool phy_is_tx_power_limit_needed(_adapter *adapter)
{
	...
	printk("%s, RegEnableTxPowerLimit %d, EEPROMRegulatory %d.\n", __func__, regsty->RegEnableTxPowerLimit, hal_data->EEPROMRegulatory);
			...
			, enum channel_width bw, BAND_TYPE band, u8 cch, struct txpwr_idx_comp *tic)
{
	...
	if (IS_HARDWARE_TYPE_8188E(adapter) || IS_HARDWARE_TYPE_8188F(adapter) || IS_HARDWARE_TYPE_8188GTV(adapter)
	    ...
	    || IS_HARDWARE_TYPE_8723B(adapter) || IS_HARDWARE_TYPE_8703B(adapter) || IS_HARDWARE_TYPE_8730A(adapter)
			 ...
			 , struct txpwr_idx_comp *tic)
{
	...
	case TXPWR_PG_WITH_PWR_IDX: {
		...
		*/
		...
		power_idx = base + (rate_target - rs_target) * 2 + (rate_amends);
