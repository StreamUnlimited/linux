...
#include "rtl8730a/hal8730a_fw.h"
...
static u8 _halmac_sys_reg_read_8(void *p, u32 base, u32 offset)
{
	struct dvobj_priv *d;
	PADAPTER adapter;
	...
	return _rtw_sys_read8(adapter, base, offset);
}

static u16 _halmac_sys_reg_read_16(void *p, u32 base, u32 offset)
{
	struct dvobj_priv *d;
	PADAPTER adapter;
	...
	return _rtw_sys_read16(adapter, base, offset);
}

static u32 _halmac_sys_reg_read_32(void *p, u32 base, u32 offset)
{
	struct dvobj_priv *d;
	PADAPTER adapter;
	...
	return _rtw_sys_read32(adapter, base, offset);
}

static void _halmac_sys_reg_write_8(void *p, u32 base, u32 offset, u8 val)
{
	struct dvobj_priv *d;
	PADAPTER adapter;
	int err;
	...
	err = _rtw_sys_write8(adapter, base, offset, val);
	if (err == _FAIL) {
		RTW_ERR("%s: I/O FAIL!\n", __FUNCTION__);
	}
}

static void _halmac_sys_reg_write_16(void *p, u32 base, u32 offset, u16 val)
{
	struct dvobj_priv *d;
	PADAPTER adapter;
	int err;
	...
	err = _rtw_sys_write16(adapter, base, offset, val);
	if (err == _FAIL) {
		RTW_ERR("%s: I/O FAIL!\n", __FUNCTION__);
	}
}

static void _halmac_sys_reg_write_32(void *p, u32 base, u32 offset, u32 val)
{
	struct dvobj_priv *d;
	PADAPTER adapter;
	int err;
	...
	err = _rtw_sys_write32(adapter, base, offset, val);
	if (err == _FAIL) {
		RTW_ERR("%s: I/O FAIL!\n", __FUNCTION__);
	}
}
...
struct halmac_platform_api rtw_halmac_platform_api = {
	...
	.SYS_REG_READ_8 = _halmac_sys_reg_read_8,
	.SYS_REG_READ_16 = _halmac_sys_reg_read_16,
	.SYS_REG_READ_32 = _halmac_sys_reg_read_32,
	.SYS_REG_WRITE_8 = _halmac_sys_reg_write_8,
	.SYS_REG_WRITE_16 = _halmac_sys_reg_write_16,
	.SYS_REG_WRITE_32 = _halmac_sys_reg_write_32,
...
int rtw_halmac_get_tx_dma_ch_map(struct dvobj_priv *d, u8 *dma_ch_map, u8 map_size)
{
	...
	status = api->halmac_get_hw_value(halmac, HALMAC_HW_RQPN_MAPPING, &map);
	if (status != HALMAC_RET_SUCCESS) {
		return -1;
	}
...
#ifdef CONFIG_AXI_HCI
static void _dump_axi_cfg_space(struct dvobj_priv *d)
{
 ...
 */
static int _halmac_init_hal(struct dvobj_priv *d)
{
	PADAPTER adapter;
	struct halmac_adapter *halmac;
	struct halmac_api *api;
	...
	/* Enable FW */
	err = rtl8730a_FirmwareEnable(adapter, 1);
	if (err) {
		goto out;
	}
	fw_ok = _TRUE;
...
int rtw_halmac_deinit_hal(struct dvobj_priv *d)
{
	...
	/* Disable FW */
	err = rtl8730a_FirmwareEnable(adapter, 0);
	if (err) {
		goto out;
	}
