...
void rtw_hal_dm_init(_adapter *padapter)
{
	if (is_primary_adapter(padapter)) {
		...
		if ((hci_type == RTW_AXI) && !rtw_is_hw_init_completed(padapter)) {
			rtw_hal_power_on(padapter);
		}
...
{
#if defined(CONFIG_PCI_HCI) || defined(CONFIG_SDIO_HCI) || defined (CONFIG_GSPI_HCI) || defined(CONFIG_AXI_HCI)
...
s32 rtw_hal_fw_en(_adapter *padapter, u8 enable)
{
	...
	ret = padapter->hal_func.fw_en(padapter, enable);
...
#if (defined(CONFIG_PCI_HCI)) || (defined(CONFIG_USB_HCI) && defined(CONFIG_SUPPORT_USB_INT)) || defined(CONFIG_AXI_HCI)
