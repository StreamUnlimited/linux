...
#define RX_DMA_SIZE_8730A 0x4000
#define RX_DMA_RESERVED_SIZE_8730A 0x80
#define RX_DMA_BOUNDARY_8730A (RX_DMA_SIZE_8730A - RX_DMA_RESERVED_SIZE_8730A - 1)

#define SYS_FUNC_EN (BIT_FEN_MAC_PMC | BIT_FEN_MACREG | BIT_FEN_WLMCU | BIT_FEN_WLON | BIT_FEN_MLPLT | BIT_DAI_REG00_WREN | BIT_DAI_RW_EN)
...
#define RSVD_PG_DLLB_NUM		(TX_FIFO_SIZE_8730A / 3 >> \
...
#define WLAN_RCR_CFG		0x741840ce
...
#define WLAN_PRE_TXCNT_TIME_TH		0x3A
...
#define WLAN_DATA_RATE_FB_CNT_1_4	0x02000000
...
static struct halmac_rqpn HALMAC_RQPN_SDIO_8730A[] = {
...
/*PCIE RQPN Mapping*/
static struct halmac_rqpn HALMAC_RQPN_PCIE_8730A[] = {
	/* { mode, vo_map, vi_map, be_map, bk_map, mg_map, hi_map } */
	{
		HALMAC_TRX_MODE_NORMAL,
		HALMAC_MAP2_NQ, HALMAC_MAP2_NQ, HALMAC_MAP2_LQ, HALMAC_MAP2_LQ,
...
static struct halmac_rqpn HALMAC_RQPN_2BULKOUT_8730A[] = {
	/* { mode, vo_map, vi_map, be_map, bk_map, mg_map, hi_map } */
	{
		HALMAC_TRX_MODE_NORMAL,
...
static struct halmac_rqpn HALMAC_RQPN_3BULKOUT_8730A[] = {
	/* { mode, vo_map, vi_map, be_map, bk_map, mg_map, hi_map } */
	{
		HALMAC_TRX_MODE_NORMAL,
		HALMAC_MAP2_NQ, HALMAC_MAP2_NQ, HALMAC_MAP2_LQ, HALMAC_MAP2_LQ,
...
static struct halmac_rqpn HALMAC_RQPN_4BULKOUT_8730A[] = {
	/* { mode, vo_map, vi_map, be_map, bk_map, mg_map, hi_map } */
	{
		HALMAC_TRX_MODE_NORMAL,
		HALMAC_MAP2_NQ, HALMAC_MAP2_NQ, HALMAC_MAP2_LQ, HALMAC_MAP2_LQ,
...
#if HALMAC_PLATFORM_WINDOWS
/*SDIO Page Number*/
static struct halmac_pg_num HALMAC_PG_NUM_SDIO_8730A[] = {
...
/*AXI Page Number*/
static struct halmac_pg_num HALMAC_PG_NUM_AXI_8730A[] = {
	/* { mode, hq_num, nq_num, lq_num, exq_num, gap_num} */
...
/*PCIE Page Number*/
static struct halmac_pg_num HALMAC_PG_NUM_PCIE_8730A[] = {
	/* { mode, hq_num, nq_num, lq_num, exq_num, gap_num} */
	{HALMAC_TRX_MODE_NORMAL, 8, 8, 8, 8, 1},
	{HALMAC_TRX_MODE_TRXSHARE, 8, 8, 8, 8, 1},
	{HALMAC_TRX_MODE_WMM, 8, 8, 8, 8, 1},
...
static struct halmac_pg_num HALMAC_PG_NUM_2BULKOUT_8730A[] = {
	/* { mode, hq_num, nq_num, lq_num, exq_num, gap_num} */
...
static struct halmac_pg_num HALMAC_PG_NUM_3BULKOUT_8730A[] = {
	/* { mode, hq_num, nq_num, lq_num, exq_num, gap_num} */
	{HALMAC_TRX_MODE_NORMAL, 8, 8, 8, 0, 1},
...
static struct halmac_pg_num HALMAC_PG_NUM_4BULKOUT_8730A[] = {
	/* { mode, hq_num, nq_num, lq_num, exq_num, gap_num} */
	{HALMAC_TRX_MODE_NORMAL, 8, 8, 8, 8, 1},
	{HALMAC_TRX_MODE_TRXSHARE, 8, 8, 8, 8, 1},
	{HALMAC_TRX_MODE_WMM, 8, 8, 8, 8, 1},
...
txdma_queue_mapping_8730a(struct halmac_adapter *adapter,
			  enum halmac_trx_mode mode);

static enum halmac_ret_status
priority_queue_cfg_8730a(struct halmac_adapter *adapter,
			 enum halmac_trx_mode mode);

static enum halmac_ret_status
set_trx_fifo_info_8730a(struct halmac_adapter *adapter,
			enum halmac_trx_mode mode);

static void
init_txq_ctrl_8730a(struct halmac_adapter *adapter);

static void
init_sifs_ctrl_8730a(struct halmac_adapter *adapter);

static void
init_rate_fallback_ctrl_8730a(struct halmac_adapter *adapter);

u8 CRC5_USB(unsigned char *in, unsigned long byte_num);

static void wifi_init_mac_ckl(struct halmac_adapter *adapter, u8 clk)
{
	struct halmac_api *api = (struct halmac_api *)adapter->halmac_api;
	u32 value32;

	/*mac clock select*/
	value32 = HALMAC_REG_R32(REG_CLKDIV_CKSEL);
	if (clk == 20) {
		value32 = (value32 & ~BIT_MASK_MAC_CLK_SEL) | BIT_MAC_CLK_SEL(2);
	} else if (clk == 40) {
		value32 = (value32 & ~BIT_MASK_MAC_CLK_SEL) | BIT_MAC_CLK_SEL(1);
	}
	HALMAC_REG_W32(REG_CLKDIV_CKSEL, value32);

	/*clock count for 1us, 20 for 20MHZ, 40 for 40MHZ*/
	HALMAC_REG_W8(REG_USTIME_TSF, clk);
	HALMAC_REG_W8(REG_USTIME_EDCA, clk);
	HALMAC_REG_W8(REG_WSEC_OPTION, clk);

	/*clock count for 3us, 60 for 20MHZ, 120 for 40MHZ*/
	value32 = HALMAC_REG_R32(REG_PRETX_DROP_CTRL);
	value32 &= (~BIT_MASK_PRETX_AGGR_TIME_MAX);
	value32 |= BIT_PRETX_AGGR_TIME_MAX(clk * 3);
	HALMAC_REG_W32(REG_PRETX_DROP_CTRL, value32);
}

enum halmac_ret_status
mount_api_8730a(struct halmac_adapter *adapter) {
	struct halmac_api *api = (struct halmac_api *)adapter->halmac_api;

	adapter->chip_id = HALMAC_CHIP_ID_8730A;
	adapter->hw_cfg_info.efuse_size = EFUSE_SIZE_8730A;
	adapter->hw_cfg_info.eeprom_size = EEPROM_SIZE_8730A;
	adapter->hw_cfg_info.bt_efuse_size = BT_EFUSE_SIZE_8730A;
	adapter->hw_cfg_info.prtct_efuse_size = PRTCT_EFUSE_SIZE_8730A;
	adapter->hw_cfg_info.cam_entry_num = SEC_CAM_NUM_8730A;
	adapter->hw_cfg_info.tx_fifo_size = TX_FIFO_SIZE_8730A;
	adapter->hw_cfg_info.rx_fifo_size = RX_FIFO_SIZE_8730A;
	adapter->hw_cfg_info.ac_oqt_size = OQT_ENTRY_AC_8730A;
	adapter->hw_cfg_info.non_ac_oqt_size = OQT_ENTRY_NOAC_8730A;
	adapter->hw_cfg_info.usb_txagg_num = BLK_DESC_NUM;
	adapter->txff_alloc.rsvd_drv_pg_num = RSVD_PG_DRV_NUM;

	api->halmac_init_trx_cfg = init_trx_cfg_8730a;
	api->halmac_init_system_cfg = init_system_cfg_8730a;
	api->halmac_init_protocol_cfg = init_protocol_cfg_8730a;
	api->halmac_pinmux_get_func = pinmux_get_func_8730a;
	api->halmac_pinmux_set_func = pinmux_set_func_8730a;
	api->halmac_pinmux_free_func = pinmux_free_func_8730a;
	api->halmac_get_hw_value = get_hw_value_8730a;
	api->halmac_set_hw_value = set_hw_value_8730a;
	api->halmac_cfg_drv_info = cfg_drv_info_8730a;
	api->halmac_fill_txdesc_checksum = fill_txdesc_check_sum_8730a;
	api->halmac_init_low_pwr = init_low_pwr_8730a;
	api->halmac_pre_init_system_cfg = pre_init_system_cfg_8730a;

	api->halmac_init_wmac_cfg = init_wmac_cfg_8730a;
	api->halmac_init_edca_cfg = init_edca_cfg_8730a;

	if (adapter->intf == HALMAC_INTERFACE_AXI)
	{
#if HALMAC_AXI_SUPPORT
		api->halmac_mac_power_switch = mac_pwr_switch_axi_8730a;
		api->halmac_phy_cfg = phy_cfg_axi_8730a;
		api->halmac_interface_integration_tuning = intf_tun_axi_8730a;
		api->halmac_cfgspc_set_pcie = cfgspc_set_axi_8730a;
#endif
	} else
	{
...
void wifi_init_driver_info_size(struct halmac_adapter *adapter, u8 size)
{
	u8 value8 = 0;
	struct halmac_api *api = (struct halmac_api *)adapter->halmac_api;

	value8 = HALMAC_REG_R8(REG_RXPKT_LIMIT + 3);
	value8 = ((value8 & ~0xF) | (size & 0xF));
	HALMAC_REG_W8(REG_RXPKT_LIMIT + 3, value8);
}

/**
 * init_trx_cfg_8730a() - config trx dma register
 ...
 */
enum halmac_ret_status
init_trx_cfg_8730a(struct halmac_adapter *adapter, enum halmac_trx_mode mode) {
	u8 value8;
	struct halmac_api *api = (struct halmac_api *)adapter->halmac_api;
	...
	status = txdma_queue_mapping_8730a(adapter, mode);
	...
	*/
	...
	status = priority_queue_cfg_8730a(adapter, mode);
	if (status != HALMAC_RET_SUCCESS)
	{
	...
	wifi_init_driver_info_size(adapter, DRVINFO_SZ_87XX);
...
txdma_queue_mapping_8730a(struct halmac_adapter *adapter,
	...
	if (adapter->intf == HALMAC_INTERFACE_AXI)
	{
		cur_rqpn_sel = HALMAC_RQPN_PCIE_8730A;
	} else
	{
...
priority_queue_cfg_8730a(struct halmac_adapter *adapter,
	...
	status = set_trx_fifo_info_8730a(adapter, mode);
	if (status != HALMAC_RET_SUCCESS)
	{
	...
	if (adapter->intf == HALMAC_INTERFACE_AXI)
	{
		cur_pg_num = HALMAC_PG_NUM_AXI_8730A;
	} else
	{
		return HALMAC_RET_NOT_SUPPORT;
	}
	...
	HALMAC_REG_W32(REG_RQPN_CTRL_HLPQ, ((txff_info->pub_queue_pg_num) << 16) + \
	...
	HALMAC_REG_W16((REG_TRXFF_BNDY + 2), RX_DMA_BOUNDARY_8730A);
...
set_trx_fifo_info_8730a(struct halmac_adapter *adapter,
			enum halmac_trx_mode mode) {
	u16 cur_pg_addr;
	u32 txff_size = TX_FIFO_SIZE_8730A;
	u32 rxff_size = RX_FIFO_SIZE_8730A;
	struct halmac_txff_allocation *info = &adapter->txff_alloc;

	if (info->rx_fifo_exp_mode == HALMAC_RX_FIFO_EXPANDING_MODE_1_BLOCK)
	{
		txff_size = TX_FIFO_SIZE_RX_EXPAND_1BLK_8730A;
		rxff_size = RX_FIFO_SIZE_RX_EXPAND_1BLK_8730A;
	} else if (info->rx_fifo_exp_mode ==
		   HALMAC_RX_FIFO_EXPANDING_MODE_2_BLOCK)
	{
		txff_size = TX_FIFO_SIZE_RX_EXPAND_2BLK_8730A;
		rxff_size = RX_FIFO_SIZE_RX_EXPAND_2BLK_8730A;
	} else if (info->rx_fifo_exp_mode ==
		   HALMAC_RX_FIFO_EXPANDING_MODE_3_BLOCK)
	{
		txff_size = TX_FIFO_SIZE_RX_EXPAND_3BLK_8730A;
		rxff_size = RX_FIFO_SIZE_RX_EXPAND_3BLK_8730A;
	} else if (info->rx_fifo_exp_mode ==
		   HALMAC_RX_FIFO_EXPANDING_MODE_4_BLOCK)
	{
		txff_size = TX_FIFO_SIZE_RX_EXPAND_4BLK_8730A;
		rxff_size = RX_FIFO_SIZE_RX_EXPAND_4BLK_8730A;
	}

	if (info->la_mode != HALMAC_LA_MODE_DISABLE)
	{
		txff_size = TX_FIFO_SIZE_LA_8730A;
		rxff_size = RX_FIFO_SIZE_8730A;
	}
...
/**
 * init_system_cfg_8730a() -  init system config
 * @adapter : the adapter of halmac
 * Author : KaiYuan Chang/Ivan Lin
 * Return : enum halmac_ret_status
 * More details of status code can be found in prototype document
...
enum halmac_ret_status
init_system_cfg_8730a(struct halmac_adapter *adapter) {
	struct halmac_api *api = (struct halmac_api *)adapter->halmac_api;
	u16 value16;
	u32 tmp = 0;
	u32 value32;
	...
	value32 = HALMAC_REG_R32(REG_CR);
	value32 |= (BIT_HCI_TXDMA_EN | BIT_HCI_RXDMA_EN | BIT_TXDMA_EN \
		    | BIT_RXDMA_EN | BIT_PROTOCOL_EN | BIT_SCHEDULE_EN \
		    | BIT_MAC_SEC_EN);
	HALMAC_REG_W32(REG_CR, value32);

	value32 = HALMAC_REG_R32(0x1f08);
	value32 &= (~0x1FFFF);
	value32 |=  0x14443;
	HALMAC_REG_W32(0x1f08, value32);

	value32 = HALMAC_REG_R32(0x1f54);
	value32 &= (~0x1FFFF);
	value32 |=  0x14443;
	HALMAC_REG_W32(0x1f54, value32);

	value32 = HALMAC_REG_R32(0x1f04);
	value32 &= (~0x7FF000);
	value32 |= (0x14443 << 12);
	HALMAC_REG_W32(0x1f04, value32);

	value32 = HALMAC_REG_R32(0x1f58);
	value32 &= (~0x1FFFF);
	value32 |=  0x14443;
	HALMAC_REG_W32(0x1f58, value32);
...
/**
 * init_protocol_cfg_8730a() - config protocol register
 * @adapter : the adapter of halmac
 * Author : KaiYuan Chang/Ivan Lin
 * Return : enum halmac_ret_status
 * More details of status code can be found in prototype document
 */
enum halmac_ret_status
init_protocol_cfg_8730a(struct halmac_adapter *adapter) {
	u32 max_agg_num;
	u32 max_rts_agg_num;
	u32 value32;
	u32 pre_txcnt;
	...
	init_txq_ctrl_8730a(adapter);
	init_sifs_ctrl_8730a(adapter);
	init_rate_fallback_ctrl_8730a(adapter);

	HALMAC_REG_W8(REG_BCN_AMPDU_CTCL + 2, WLAN_AMPDU_MAX_TIME);
	HALMAC_REG_W8_SET(REG_QUEUE_CTRL, BIT_R_EOF_EN);

	pre_txcnt = WLAN_PRE_TXCNT_TIME_TH | BIT_PRETX_AGGR_EN;
	HALMAC_REG_W8(REG_PRETX_DROP_CTRL + 1, (u16)(pre_txcnt & 0xFF));
	HALMAC_REG_W8(REG_PRETX_DROP_CTRL + 2, (u16)(pre_txcnt >> 16));
	...
	HALMAC_REG_W8_CLR(REG_BNDY_LIFETIME_TAIL + 2, BIT(5));
	...
	value32 |= 0xFFFF1; /* Only use CCK 1M rate for ACK */
	HALMAC_REG_W32(REG_RRSR, value32);

	value8 = HALMAC_REG_R8(REG_CFEND_RATE_SC_CTRL);
	HALMAC_REG_W8(REG_CFEND_RATE_SC_CTRL, value8 | BIT(5));

	//jerry_zhou
	value8 = HALMAC_REG_R8(REG_BCN_AMPDU_CTCL);
	value8 = value8 & (~(BIT(3))) & (~(BIT(6)));
	HALMAC_REG_W8(REG_BCN_AMPDU_CTCL, value8); //for driver not update beacon every TBTT

	value8 = HALMAC_REG_R8(REG_FWHW_TXQ_HWSSN_CTRL + 2);
	value8 = value8 | BIT(2);
	HALMAC_REG_W8(REG_FWHW_TXQ_HWSSN_CTRL + 2, value8);
	...
	HALMAC_REG_W8(REG_FWHW_TXQ_HWSSN_CTRL + 3, 0xFF);
...
/**
 * init_edca_cfg_8730a() - init EDCA config
 * @adapter : the adapter of halmac
 * Author : KaiYuan Chang/Ivan Lin
 * Return : enum halmac_ret_status
 * More details of status code can be found in prototype document
 */
enum halmac_ret_status
init_edca_cfg_8730a(struct halmac_adapter *adapter) {
	u8 value8;
	u8 value32;
	struct halmac_api *api = (struct halmac_api *)adapter->halmac_api;
	...
	value32 = HALMAC_REG_R32(REG_BCN_CFG_PIFS);
	HALMAC_REG_W32(REG_BCN_CFG_PIFS, BIT_SET_PIFS(value32, WLAN_PIFS_TIME));
	...
	value32 = HALMAC_REG_R32(REG_TSFT_SYN_OFFSET_SLOT);
	HALMAC_REG_W32(REG_TSFT_SYN_OFFSET_SLOT, BIT_SET_SLOTTIME(value32, WLAN_SLOT_TIME));

	HALMAC_REG_W32(REG_NAV_CTRL, WLAN_NAV_CFG);
	HALMAC_REG_W16(REG_RXTSF_OFFSET_CCK, WLAN_RX_TSF_CFG);

	/* Set beacon cotnrol - enable TSF and other related functions */
	HALMAC_REG_W8(REG_BCN_CTRL_PORT0, (u8)(HALMAC_REG_R8(REG_BCN_CTRL_PORT0) |
					       BIT_P0_EN_BCN_FUNCTION));

	/* Set send beacon related registers */
	HALMAC_REG_W32(REG_TBTT_PROHIBIT, WLAN_TBTT_TIME);
	HALMAC_REG_W8(REG_PORT0_BCN_ERLY_DMA_CTRL, WLAN_DRV_EARLY_INT);
	HALMAC_REG_W8(REG_BCN_CTRL_PORT1, WLAN_BCN_CTRL_CLT0);
	HALMAC_REG_W8(REG_PORT0_BCN_ERLY_DMA_CTRL + 1, WLAN_BCN_DMA_TIME);
	...
	/*testchip mac is 20MHz clock*/
	wifi_init_mac_ckl(adapter, 20);
...
init_txq_ctrl_8730a(struct halmac_adapter *adapter)
{
	...
	value8 = HALMAC_REG_R8(REG_FWHW_TXQ_HWSSN_CTRL);
	value8 |= (BIT(7) & ~BIT(1) & ~BIT(2));
	HALMAC_REG_W8(REG_FWHW_TXQ_HWSSN_CTRL, value8);

	HALMAC_REG_W8(REG_FWHW_TXQ_HWSSN_CTRL + 1, WLAN_TXQ_RPT_EN);
}
...
init_sifs_ctrl_8730a(struct halmac_adapter *adapter)
{
	...
	HALMAC_REG_W16(REG_RETRY_LIMIT_SIFS, WLAN_SIFS_DUR_TUNE);
...
init_rate_fallback_ctrl_8730a(struct halmac_adapter *adapter)
{
	...
	HALMAC_REG_W32(REG_DARFRC_L, WLAN_DATA_RATE_FB_CNT_1_4);
	HALMAC_REG_W32(REG_DARFRC_H, WLAN_DATA_RATE_FB_CNT_5_8);
	HALMAC_REG_W32(REG_RARFRC_H, WLAN_RTS_RATE_FB_CNT_5_8);

	HALMAC_REG_W32(REG_ARFR_L0, WLAN_DATA_RATE_FB_RATE0);
	HALMAC_REG_W32(REG_ARFR_H0, WLAN_DATA_RATE_FB_RATE0_H);
	HALMAC_REG_W32(REG_ARFR_L1, WLAN_RTS_RATE_FB_RATE1);
	HALMAC_REG_W32(REG_ARFR_H1, WLAN_RTS_RATE_FB_RATE1_H);
	HALMAC_REG_W32(REG_ARFR_L4, WLAN_RTS_RATE_FB_RATE4);
	HALMAC_REG_W32(REG_ARFR_H4, WLAN_RTS_RATE_FB_RATE4_H);
	HALMAC_REG_W32(REG_ARFR_L5, WLAN_RTS_RATE_FB_RATE5);
	HALMAC_REG_W32(REG_ARFR_H5, WLAN_RTS_RATE_FB_RATE5_H);
}


void wifi_init_mac_clk(struct halmac_adapter *adapter, u8 clk)
{
	u32 value32 = 0;
	struct halmac_api *api = (struct halmac_api *)adapter->halmac_api;
...
/**
 * init_wmac_cfg_8730a() - init wmac config
 * @adapter : the adapter of halmac
 * Author : KaiYuan Chang/Ivan Lin
 * Return : enum halmac_ret_status
 * More details of status code can be found in prototype document
 */
enum halmac_ret_status
init_wmac_cfg_8730a(struct halmac_adapter *adapter) {
	u8 value8;
	u16 value16;
	struct halmac_api *api = (struct halmac_api *)adapter->halmac_api;
	enum halmac_ret_status status = HALMAC_RET_SUCCESS;
	...
	HALMAC_REG_W8(REG_NAV_THRSHOLD + 2, WLAN_NAV_MAX);
	...
	HALMAC_REG_W16(REG_RXFLTMAP, WLAN_RX_FILTER2);
	...
	HALMAC_REG_W8(REG_RXPKT_LIMIT, WLAN_RXPKT_MAX_SZ_512);
	...
	//indirect access,8730A not supported
	...
	//crc5=CRC5_USB(DA,6);
	//HALMAC_REG_W32(REG_SEARCH_MACID_8730A);
...
/**
 * pre_init_system_cfg_8730a() - pre-init system config
 * @adapter : the adapter of halmac
 * Author : KaiYuan Chang/Ivan Lin
 * Return : enum halmac_ret_status
 * More details of status code can be found in prototype document
 */
enum halmac_ret_status
pre_init_system_cfg_8730a(struct halmac_adapter *adapter) {
	struct halmac_api *api = (struct halmac_api *)adapter->halmac_api;
	u8 enable_bb;
