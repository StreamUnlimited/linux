...
cfg_net_type_87xx(struct halmac_adapter *adapter, u8 port,
	...
	case HALMAC_PORTID0:
		value32 = net_type;
		value32 = BIT_SET_NETYPE0(HALMAC_REG_R32(REG_CR), value32);
		HALMAC_REG_W32(REG_CR, value32);
		break;
	case HALMAC_PORTID1:
		value32 = net_type;
		value32 = BIT_SET_NETYPE1(HALMAC_REG_R32(REG_CR), value32);
		HALMAC_REG_W32(REG_CR, value32);
		break;
	default:
		break;
...
cfg_tsf_rst_87xx(struct halmac_adapter *adapter, u8 port) {
	...
	case HALMAC_PORTID0:
		...
		break;
	case HALMAC_PORTID1:
		tsf_rst = BIT_TSFTR1_RST;
		break;
	case HALMAC_PORTID2:
		tsf_rst = BIT_TSFTR2_RST;
		break;
	default:
		break;
	}
...
cfg_bcn_space_87xx(struct halmac_adapter *adapter, u8 port, u32 bcn_space) {
	...
	{
	case HALMAC_PORTID0:
		HALMAC_REG_W16(REG_PORT0_BCN_SPACE, bcn_space_real);
		break;
	case HALMAC_PORTID1:
		value16 = HALMAC_REG_R16(REG_BCN_SPACE_PORT1) & 0xF000;
		value16 |= bcn_space_real;
		HALMAC_REG_W16(REG_BCN_SPACE_PORT1, value16);
		break;
	case HALMAC_PORTID2:
		value16 = HALMAC_REG_R16(REG_BCN_SPACE_PORT2) & 0xF000;
		value16 |= bcn_space_real;
		HALMAC_REG_W16(REG_BCN_SPACE_PORT2, value16);
		break;
	default:
		break;
	}
...
rw_bcn_ctrl_87xx(struct halmac_adapter *adapter, u8 port, u8 write_en,
	...
	if (write_en)
	{
		if (ctrl->dis_rx_bssid_fit == 1) {
			ctrl_value |= BIT_P0_DIS_RX_BSSID_FIT;
		}

		if (ctrl->en_txbcn_rpt == 1) {
			ctrl_value |= BIT_P0_EN_BCN_RPT;
		}

		if (ctrl->dis_tsf_udt == 1) {
			ctrl_value |= BIT_P0_DIS_TSF_UDT;
		}

		if (ctrl->en_bcn == 1) {
			ctrl_value |= BIT_P0_EN_BCN_FUNCTION;
		}

		if (ctrl->en_rxbcn_rpt == 1) {
			ctrl_value |= BIT_P0_RXBCN_RPT_ENABLE;
		}

		if (ctrl->en_p2p_ctwin == 1) {
			ctrl_value |= BIT_P0_EN_P2P_CTWINDOW;
		}

		if (ctrl->en_p2p_bcn_area == 1) {
			ctrl_value |= BIT_P0_EN_P2P_BCNQ_AREA;
		}

		switch (port) {
		case HALMAC_PORTID0:
			HALMAC_REG_W8(REG_BCN_CTRL_PORT0, ctrl_value);
			break;
		case HALMAC_PORTID1:
			HALMAC_REG_W8(REG_BCN_CTRL_PORT1, ctrl_value);
			break;
		case HALMAC_PORTID2:
			HALMAC_REG_W8(REG_BCN_CTRL_PORT1, ctrl_value);
			break;
	...
	} else
	{
		switch (port) {
		case HALMAC_PORTID0:
			ctrl_value = HALMAC_REG_R8(REG_BCN_CTRL_PORT0);
			break;
		case HALMAC_PORTID1:
			ctrl_value = HALMAC_REG_R8(REG_BCN_CTRL_PORT1);
			break;
		case HALMAC_PORTID2:
			ctrl_value = HALMAC_REG_R8(REG_BCN_CTRL_PORT1);
			break;
		...
		if (ctrl_value & BIT_P0_EN_P2P_BCNQ_AREA) {
			ctrl->en_p2p_bcn_area = 1;
		} else {
			ctrl->en_p2p_bcn_area = 0;
		}

		if (ctrl_value & BIT_P0_EN_P2P_CTWINDOW) {
			ctrl->en_p2p_ctwin = 1;
		} else {
			ctrl->en_p2p_ctwin = 0;
		}

		if (ctrl_value & BIT_P0_RXBCN_RPT_ENABLE) {
			ctrl->en_rxbcn_rpt = 1;
		} else {
			ctrl->en_rxbcn_rpt = 0;
		}

		if (ctrl_value & BIT_P0_EN_BCN_FUNCTION) {
			ctrl->en_bcn = 1;
		} else {
			ctrl->en_bcn = 0;
		}

		if (ctrl_value & BIT_P0_DIS_TSF_UDT) {
			ctrl->dis_tsf_udt = 1;
		} else {
			ctrl->dis_tsf_udt = 0;
		}

		if (ctrl_value & BIT_P0_EN_BCN_RPT) {
			ctrl->en_txbcn_rpt = 1;
		} else {
			ctrl->en_txbcn_rpt = 0;
		}

		if (ctrl_value & BIT_P0_DIS_RX_BSSID_FIT) {
...
cfg_ch_87xx(struct halmac_adapter *adapter, u8 ch) {
	...
	HALMAC_REG_W8(REG_BCN_AMPDU_CTCL, value8);
...
cfg_pri_ch_idx_87xx(struct halmac_adapter *adapter, enum halmac_pri_ch_idx idx) {
	...
	if (txsc20 == HALMAC_CH_IDX_1 || txsc20 == HALMAC_CH_IDX_3)
		...
		txsc40 = 10;
	*/
	//8730A only support bw 40M
	txsc40 = 0;
	HALMAC_REG_W32(REG_CFEND_RATE_SC_CTRL, HALMAC_REG_R32(REG_CFEND_RATE_SC_CTRL) | BIT_TXSC_20M(txsc20) | BIT_TXSC_40M(txsc40));
		   ...
		   struct halmac_txfifo_lifetime_cfg *cfg)
{
	...
	if (cfg->enable == 1) {
		value8 = HALMAC_REG_R8(REG_BNDY_LIFETIME_TAIL + 2);
		value8 = value8 | BIT(0) | BIT(1) | BIT(2) | BIT(3);
		HALMAC_REG_W8(REG_BNDY_LIFETIME_TAIL + 2, value8);
...
cfg_mac_clk_87xx(struct halmac_adapter *adapter)
{
	...
	*/
	...
	value32 = BIT_SET_MAC_CLK_SEL(value32, 0x02);
	HALMAC_REG_W32(REG_CLKDIV_CKSEL, value32);

	//sec clk 160M
	value32 = HALMAC_REG_R32(REG_CLKDIV_CKSEL);
	value32 = BIT_SET_SEC_CLK_SEL(value32, 0x00);
	HALMAC_REG_W32(REG_CLKDIV_CKSEL, value32);
