...
dl_rsvd_page_87xx(struct halmac_adapter *adapter, u16 pg_addr, u8 *buf,
	...
	*state = HALMAC_RSVD_PG_STATE_BUSY;
	...
	pg_addr &= 0xff;  //8730A bcn head mask(0xff), TXBUF has only 8bit page offset
	HALMAC_REG_W8(REG_DWBCN0_CTRL + 1, (u8)(pg_addr));
	value8 = HALMAC_REG_R8(REG_DWBCN0_CTRL + 2);
	value8 |= BIT(0);
	HALMAC_REG_W8(REG_DWBCN0_CTRL + 2, value8);

	/*enable sw bcn mode, but only pcie has this mode, so this setting is useless on 8730A*/
...
DL_RSVD_PG_END:
	...
	value8 |= BIT(0);
	HALMAC_REG_W8(REG_DWBCN0_CTRL + 2, value8);
	HALMAC_REG_W8(REG_FWHW_TXQ_HWSSN_CTRL + 2, restore[1]);
...
chk_txdesc_87xx(struct halmac_adapter *adapter, u8 *buf, u32 size) {
	...
	if (GET_TX_DESC_USE_MAX_TIME_EN(buf) == 1)
	{
		...
		if (value8 > (u8)BIT_GET_AMPDU_MAX_TIME(HALMAC_REG_R32(REG_BCN_AMPDU_CTCL))) {
			PLTFM_MSG_ERR("[ERR]txdesc - ampdu_max_time\n");
			status = HALMAC_RET_TXDESC_SET_FAIL;
		}
	}

	switch (BIT_GET_MAC_CLK_SEL(HALMAC_REG_R32(REG_CLKDIV_CKSEL)))
	{
	case 0x0:
		mac_clk = 80;
		break;
...
pwr_cmd_polling_87xx(struct halmac_adapter *adapter,
	...
	do
	{
		...
		if (value == (cmd->value & cmd->msk)) {
			...
			if (cnt == 0) {
				if (intf == HALMAC_INTERFACE_AXI && flg == 0) {
					/* PCIE + USB package */
					/* power bit polling timeout issue */
					stats++;
					PLTFM_MSG_WARN("[WARN]PCIE stats:%d\n",
						       stats);
					value = HALMAC_REG_R8(REG_WL_FUNC_EN);
					value |= REG_WL_FUNC_EN;
					HALMAC_REG_W8(REG_WL_FUNC_EN, value);
					value &= ~REG_WL_FUNC_EN;
					HALMAC_REG_W8(REG_WL_FUNC_EN, value);
					poll_bit = 0;
					cnt = HALMAC_PWR_POLLING_CNT;
