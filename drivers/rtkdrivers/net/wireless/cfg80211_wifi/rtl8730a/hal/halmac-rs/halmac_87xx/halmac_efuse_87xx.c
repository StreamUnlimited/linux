...
//jerry_zhou
static enum halmac_ret_status
proc_write_log_efuse_8730A(struct halmac_adapter *adapter, u32 offset, u16 value);
...
static enum halmac_ret_status
otp_read_efuse_logic_map(struct halmac_adapter *adapter, u32 size,
			 u8 *map)
{
	u8 *tmp_map = NULL;
	...
	if (rtk_otp_process(&otp_req, tmp_map) != 1) {
		PLTFM_FREE(tmp_map, 1024);
		return HALMAC_RET_FAIL;
	}

	PLTFM_MEMCPY(map, tmp_map, size);
	PLTFM_FREE(tmp_map, 1024);
...
static enum halmac_ret_status
otp_write_efuse_logic_map(struct halmac_adapter *adapter, u32 addr, u32 cnts,
			 u8 *map)
{
	u8 *tmp_map = NULL;
	otp_ipc_host_req_t otp_req = {0};
	...
	if (rtk_otp_process(&otp_req, map) != 1) {
		return HALMAC_RET_FAIL;
	}
...
static enum halmac_ret_status
otp_read_phy_efuse(struct halmac_adapter *adapter, u32 offset,
			 u8 *value)
{
	otp_ipc_host_req_t otp_req = {0};
	...
	otp_req.addr = offset;
	otp_req.len = 1;
	if (rtk_otp_process(&otp_req, value) != 1) {
		return HALMAC_RET_FAIL;
	}
...
static enum halmac_ret_status
otp_write_phy_efuse(struct halmac_adapter *adapter, u32 offset,
			 u8 value)
{
	otp_ipc_host_req_t otp_req = {0};
...
read_efuse_bt_87xx(struct halmac_adapter *adapter, u32 offset, u8 *value,
	...
	status = read_efuse_87xx(adapter, offset, value);
	if (status != HALMAC_RET_SUCCESS)
	{
...
dump_log_efuse_map_87xx(struct halmac_adapter *adapter,
	...
	*proc_status = HALMAC_CMD_PROCESS_DONE;
	...
	status = otp_read_efuse_logic_map(adapter, size, map);
	if (status != HALMAC_RET_SUCCESS) {
		PLTFM_MSG_ERR("[ERR]read efuse\n");
		PLTFM_FREE(map, size);
		return status;
	...
	if (cnv_efuse_state_87xx(adapter, HALMAC_CMD_CNSTR_IDLE) !=
	    HALMAC_RET_SUCCESS)
	{
		PLTFM_MSG_ERR("[ERR] HALMAC_RET_ERROR_STATE\n");
		return HALMAC_RET_ERROR_STATE;
	}
...
//jerry_zhou
enum halmac_ret_status
write_log_efuse_8730A(struct halmac_adapter *adapter, u32 offset, u16 value) {
	enum halmac_ret_status status = HALMAC_RET_SUCCESS;
	enum halmac_cmd_process_status *proc_status;
	...
	status = proc_write_log_efuse_8730A(adapter, offset, value);
	if (status != HALMAC_RET_SUCCESS)
	{
		PLTFM_MSG_ERR("[ERR]write logical efuse\n");
		return status;
		   ...
		   u8 *map)
{
	...
	for (cnt = 0; cnt < size; cnt++) {
		if (otp_read_phy_efuse(adapter, offset + cnt, &map[cnt])
		    != HALMAC_RET_SUCCESS) {
			return HALMAC_RET_FAIL;
		}
	}
...
write_hw_efuse_87xx(struct halmac_adapter *adapter, u32 offset, u8 value) 
{
	...
	return otp_write_phy_efuse(adapter, offset, value);
}
...
read_log_efuse_map_87xx(struct halmac_adapter *adapter, u8 *map) {
	...
	if (adapter->efuse_map_valid == 0)
	{
		...
		status = otp_read_efuse_logic_map(adapter, efuse_size, local_map);
		if (status != HALMAC_RET_SUCCESS) {
			PLTFM_MSG_ERR("[ERR]read efuse\n");
...
dump_efuse_drv_87xx(struct halmac_adapter *adapter) {
	...
	if (adapter->efuse_map_valid == 0)
	{
		...
		adapter->efuse_map_valid = 1;
		PLTFM_MUTEX_UNLOCK(&adapter->efuse_mutex);
		PLTFM_MSG_ALWAYS("[INFOP] Get efuse OK\n");
...
enum halmac_ret_status
read_efuse_87xx(struct halmac_adapter *adapter, u32 offset, u8 *value)
{
	return otp_read_phy_efuse(adapter, offset, value);
}

enum halmac_ret_status
write_efuse_87xx(struct halmac_adapter *adapter, u32 offset, u8 value)
{
	return otp_write_phy_efuse(adapter, offset, value);
}
...
get_efuse_data_87xx(struct halmac_adapter *adapter, u8 *buf, u32 size) {
	...
	if ((enum halmac_h2c_return_code)fw_rc == HALMAC_H2C_RETURN_SUCCESS)
	{
		...
		if (adapter->evnt.log_efuse_map == 1) {
			if (otp_read_efuse_logic_map(adapter, eeprom_size,
