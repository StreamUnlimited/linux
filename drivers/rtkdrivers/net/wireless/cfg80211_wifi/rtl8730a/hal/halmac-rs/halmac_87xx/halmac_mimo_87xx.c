...
cfg_sounding_87xx(struct halmac_adapter *adapter, enum halmac_snd_role role,
	...
	tmp6dc = HALMAC_REG_R32(REG_RESP_TXINFO_RATE) | BIT_USE_NDPARATE
	| (csi_rsc << 13);
	...
	*bmp_ofdm54 = 0xFF;
...
#else
	csi_cfg = HALMAC_REG_R32(REG_RESP_TXINFO_RATE) & ~BITS_CSI_RATE;
#endif

#if ( HALMAC_8730A_SUPPORT)
	if (adapter->chip_id == HALMAC_CHIP_ID_8730A)
	{
		HALMAC_REG_W32_SET(REG_RESP_TXINFO_RATE, BIT(15));
	}
#endif
	...
	if (rssi >= 40)
	{
		if (cur_rate != HALMAC_OFDM54) {
			csi_cfg |= BIT_CSI_RATE(HALMAC_OFDM54);
			HALMAC_REG_W32(REG_RESP_TXINFO_RATE, csi_cfg);
			*bmp_ofdm54 = 1;
		}
	...
	} else
	{
		if (cur_rate != HALMAC_OFDM24) {
			csi_cfg |= BIT_CSI_RATE(HALMAC_OFDM24);
			HALMAC_REG_W32(REG_RESP_TXINFO_RATE, csi_cfg);
			*bmp_ofdm54 = 0;
		}
