...
#include "halmac_87xx/halmac_init_win8730a.h"
#endif
...
halmac_init_adapter(void *drv_adapter, struct halmac_platform_api *pltfm_api,
	...
	*halmac_adapter = adapter;
	...
	intf = (intf == HALMAC_INTERFACE_AXI) ? HALMAC_INTERFACE_AXI : intf;
	adapter->intf = intf;
...
get_chip_info(void *drv_adapter, struct halmac_platform_api *pltfm_api,
	...
	chip_id = CHIP_ID_HW_DEF_8730A;//pltfm_api->REG_READ_8(drv_adapter, REG_SYS_CFG2);
	chip_ver = HALMAC_CHIP_VER_TEST;//pltfm_api->REG_READ_8(drv_adapter, REG_SYS_CFG1 + 1) >> 4;
	...
	} else if (chip_id == CHIP_ID_HW_DEF_8730A)
	{
