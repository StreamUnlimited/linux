		...
		if (priv->pshare->rf_ft_var.pwr_track_file) {
			...
			if (is_decrease) {
				for (i = 0; i < rf; i++) {
					...
					RF_DBG(dm, DBG_RF_TX_PWR_TRACK, ">>> Decrese power ---> new CCK_INDEX:%d (%d + %d)\n",  CCK_index, priv->pshare->CCK_index0, get_tx_tracking_index(priv,
							channel, i, delta, is_decrease, 1));
				}
			} else {
				for (i = 0; i < rf; i++) {
					OFDM_index[i] = priv->pshare->OFDM_index0[i] - get_tx_tracking_index(priv, channel, i, delta, is_decrease, 0);
...
odm_txpowertracking_callback_thermal_meter_jaguar_series4(void *dm_void)
{
	...
	if (dm->support_ic_type & (ODM_RTL8822C | ODM_RTL8812F)) {
		...
		for (i = 0; i < c.rf_path_count; i++) {
			...
			if (dm->support_ic_type & (ODM_RTL8814B | ODM_RTL8814C)) {
				...
				RF_DBG(dm, DBG_RF_TX_PWR_TRACK,
				       "thermal_value_temp[%d](%d) = thermal_value[%d](%d) + multi_thermal_trim(%d)\n", i, thermal_value_temp[i], i, thermal_value[i],
				       phydm_get_multi_thermal_offset(dm, i));
			} else {
	...
	for (i = 0; i < c.rf_path_count; i++) {
		...
		delta[i] = thermal_value[i] > priv->pmib->dot11RFEntry.thermal[i] ? (thermal_value[i] - priv->pmib->dot11RFEntry.thermal[i]) :
			   (priv->pmib->dot11RFEntry.thermal[i] - thermal_value[i]);
	...
	for (p = RF_PATH_A; p < c.rf_path_count; p++) {
		if (cali_info->delta_power_index_path[p] == cali_info->delta_power_index_last_path[p]) {     /*If Thermal value changes but lookup table value still the same*/
			...
			cali_info->power_index_offset_path[p] = cali_info->delta_power_index_path[p] -
								cali_info->delta_power_index_last_path[p];        /*Power index diff between 2 times Power Tracking*/
		}
	}
...
)
{
	...
	if (thermal_value != priv->pshare->thermal_value) {
		...
		/* if (*dm->band_type == ODM_BAND_5G) */
		{
			...
			if (priv->pshare->rf_ft_var.pwr_track_file) {
				for (rf_path = 0; rf_path < max_rf_path; rf_path++) {
					RF_DBG(dm, DBG_RF_TX_PWR_TRACK, "diff: (%s)%d ==> get index from table : %d)\n", (is_decrease ? "-" : "+"), delta, get_tx_tracking_index(priv, channel, rf_path,
							delta, is_decrease, 0));
					if (is_decrease) {
...
)
{
	...
	*/
	...
	delta	  = (thermal_value > dm->rf_calibrate_info.thermal_value) ? (thermal_value - dm->rf_calibrate_info.thermal_value) :
		    (dm->rf_calibrate_info.thermal_value - thermal_value);
	delta_LCK = (thermal_value > dm->rf_calibrate_info.thermal_value_lck) ? (thermal_value - dm->rf_calibrate_info.thermal_value_lck) :
		    (dm->rf_calibrate_info.thermal_value_lck - thermal_value);
	delta_IQK = (thermal_value > dm->rf_calibrate_info.thermal_value_iqk) ? (thermal_value - dm->rf_calibrate_info.thermal_value_iqk) :
		    (dm->rf_calibrate_info.thermal_value_iqk - thermal_value);
	...
	if (delta > 0 && dm->rf_calibrate_info.txpowertrack_control) {
		...
		delta = (thermal_value > dm->priv->pmib->dot11RFEntry.ther) ? (thermal_value - dm->priv->pmib->dot11RFEntry.ther) :
		...
		RF_DBG(dm, DBG_RF_TX_PWR_TRACK, "The 'CCK' final index(%d) = BaseIndex(%d) + power_index_offset(%d)\n", cali_info->bb_swing_idx_cck,
		       cali_info->bb_swing_idx_cck_base, dm->rf_calibrate_info.power_index_offset);
		RF_DBG(dm, DBG_RF_TX_PWR_TRACK, "The 'OFDM' final index(%d) = BaseIndex(%d) + power_index_offset(%d)\n", cali_info->bb_swing_idx_ofdm[RF_PATH_A],
		       cali_info->bb_swing_idx_ofdm_base, dm->rf_calibrate_info.power_index_offset);
