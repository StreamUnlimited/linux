...
void phydm_get_set_pa_bias_offset_8735b(void *dm_void)
{
	...
	if (pg_pa_bias != 0xff) {
		...
		if ((pg_pa_bias & BIT(0)) == 0) {
			pg_pa_bias_2g = (-1 * (pg_pa_bias >> 1));
		} else {
			pg_pa_bias_2g = (pg_pa_bias >> 1);
		}
		...
		if ((pg_pa_bias & BIT(0)) == 0) {
			pg_pa_bias_5g = (-1 * (pg_pa_bias >> 1));
		} else {
			pg_pa_bias_5g = (pg_pa_bias >> 1);
		}
...
void phydm_get_thermal_trim_offset(void *dm_void)
{
	...
	if (HAL_MAC_Dump_EFUSE(&GET_HAL_MAC_INFO((PADAPTER)adapter), EFUSE_WIFI, eFuseContent, pEfuseHal->PhysicalLen_WiFi, HAL_MAC_EFUSE_PHYSICAL,
...
void phydm_get_power_trim_offset(void *dm_void)
{
	...
	if (HAL_MAC_Dump_EFUSE(&GET_HAL_MAC_INFO(adapter), EFUSE_WIFI, eFuseContent, pEfuseHal->PhysicalLen_WiFi, HAL_MAC_EFUSE_PHYSICAL,
			       HAL_MAC_EFUSE_PARSE_DRV) != RT_STATUS_SUCCESS) {
		RF_DBG(dm, DBG_RF_MP, "[kfree] dump efuse fail !!!\n");
	}
#endif
