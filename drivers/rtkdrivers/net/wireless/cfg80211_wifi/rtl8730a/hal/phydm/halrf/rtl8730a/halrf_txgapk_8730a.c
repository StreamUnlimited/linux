...
/*8730A GAPK ver:0x00 202104*/

void _txgapk_backup_rf_registers_8730a(struct dm_struct *dm,
				       u32 *rf_reg,
				       u32 rf_reg_backup[][GAPK_RF_PATH_NUM_8730A])
{
	u32 i;

	for (i = 0; i < GAPK_RF_REG_NUM_8730A; i++) {
...
void _txgapk_reload_rf_registers_8730a(struct dm_struct *dm,
				       u32 *rf_reg,
				       u32 rf_reg_backup[][GAPK_RF_PATH_NUM_8730A])
{
	u32 i;
...
void _txgapk_backup_bb_registers_8730a(struct dm_struct *dm,
				       u32 *reg,
				       u32 *reg_backup)
{
	u32 i;

	for (i = 0; i < GAPK_BB_REG_NUM_8730A; i++) {
...
void _txgapk_reload_bb_registers_8730a(struct dm_struct *dm,
				       u32 *reg,
				       u32 *reg_backup)
...
void _txgapk_dbg_diff_calculate_8730a(struct dm_struct *dm)
{
	u8 i, diff[10];
...
boolean _txgapk_dbg_psd_pwr_8730a(struct dm_struct *dm)
{
	u32 psd_pwr[2];
	u8 psd_pwr_h8[2];
...
void _txgapk_psd_one_shot_8730a(struct dm_struct *dm, u8 point)
{
...
void _txgapk_enablek_one_shot_8730a(struct dm_struct *dm, u8 sel)
{
...
void _txgapk_afe_setting_8730a(struct dm_struct *dm, boolean do_txgapk)
{
...
void _txgapk_rf_gain_setting_8730a(struct dm_struct *dm, u8 band, u8 path)
{
	u8 i;
...
void _txgapk_enable_gapk_8730a(struct dm_struct *dm, u8 path)
{
	RF_DBG(dm, DBG_RF_TXGAPK, "[TXGAPK] ======>%s\n", __func__);
	...
	/*D_clr*/
	_txgapk_enablek_one_shot_8730a(dm, D_CLR_8730A);
}

void _txgapk_clear_gain_table_8730a(struct dm_struct *dm, u8 path)
{
	u8 i, idx;
...
void _txgapk_write_gain_table_8730a(struct dm_struct *dm,
				    u8 path,
				    u8 table_sel)
{
	u8 idx, i, ta[10] = {0};
	...
	/*cal_Ta_en*/
	_txgapk_enablek_one_shot_8730a(dm, TAK_EN_8730A);

	_txgapk_dbg_diff_calculate_8730a(dm);
	...
	/*Write GapK Results*/
	if (table_sel == GAPK_TRACK_TABLE_8730A) {
		odm_set_rf_reg(dm, (enum rf_path)path, RF_0xee, BIT(15), 0x1);
		for (i = 0; i < 0xa; i++) {
			idx = 0x3 + i * 6;
			odm_set_rf_reg(dm, (enum rf_path)path, RF_0x5c, 0x3f800, idx);
		...
		odm_set_rf_reg(dm, (enum rf_path)path, RF_0xee, BIT(15), 0x0);
	} else if (table_sel == GAPK_POWER_TABLE_8730A) {
		odm_set_rf_reg(dm, (enum rf_path)path, RF_0xee, BIT(18), 0x1);
		for (i = 0; i < 0xa; i++) {
			idx = 0x1 + i * 3;
			odm_set_rf_reg(dm, (enum rf_path)path, RF_0x5e, 0x3f000, idx);
...
void _txgapk_track_gap_search_8730a(struct dm_struct *dm, u8 path)
{
	...
	for (i = 0; i < 0xa; i++) {
		...
		if (g1 != g2) {
			...
			/*point0 PSD one shot*/
			_txgapk_psd_one_shot_8730a(dm, 0);
			...
			RF_DBG(dm, DBG_RF_TXGAPK,
			       ...
			       odm_get_rf_reg(dm, path, RF_0x56, 0xfffff));
			/*point1 PSD one shot*/
			_txgapk_psd_one_shot_8730a(dm, 1);

			if (_txgapk_dbg_psd_pwr_8730a(dm)) {
				_txgapk_enablek_one_shot_8730a(dm, DIFFK_EN_8730A);
			}
...
void _txgapk_power_gap_search_8730a(struct dm_struct *dm, u8 path)
{
...
void _txgapk_track_table_offset_8730a(struct dm_struct *dm, u8 path)
{
	u8 track_idx, i, ta[10];

	RF_DBG(dm, DBG_RF_TXGAPK, "[TXGAPK] ======>%s\n", __func__);

	_txgapk_track_gap_search_8730a(dm, path);

	/*Read & Write GapK Results*/
	_txgapk_write_gain_table_8730a(dm, path, GAPK_TRACK_TABLE_8730A);

	RF_DBG(dm, DBG_RF_TXGAPK,
	       "[TXGAPK] ======>/****** PAD GAPK END ******/<======\n");
}

void _txgapk_power_table_offset_8730a(struct dm_struct *dm, u8 path)
{
	u8 power_idx, i, ta[10];

	RF_DBG(dm, DBG_RF_TXGAPK, "[TXGAPK] ======>%s\n", __func__);

	_txgapk_power_gap_search_8730a(dm, path);

	/*Read & Write GapK Results*/
	_txgapk_write_gain_table_8730a(dm, path, GAPK_POWER_TABLE_8730A);

	RF_DBG(dm, DBG_RF_TXGAPK,
	       "[TXGAPK] ======>/****** PA GAPK END ******/<======\n");
}

void _txgapk_config_offset_table_8730a(struct dm_struct *dm)
{
	u32 reg_rf18;
	...
	_txgapk_rf_gain_setting_8730a(dm, band, current_path);

	_txgapk_clear_gain_table_8730a(dm, current_path);

	_txgapk_enable_gapk_8730a(dm, current_path);

	_txgapk_track_table_offset_8730a(dm, current_path);
#if (GAPK_POWER_TABLE_8730A)
	_txgapk_power_table_offset_8730a(dm, current_path);
#endif
}
...
{
	u8 path;
	u32 bb_reg_backup[GAPK_BB_REG_NUM_8730A];
	u32 rf_reg_backup[GAPK_RF_REG_NUM_8730A][GAPK_RF_PATH_NUM_8730A];

	u32 bb_reg[GAPK_BB_REG_NUM_8730A] = {
		R_0x1b00, R_0x1b14, R_0x1b24, R_0x1b38, R_0x1b3c, R_0x1bcc
	};
	u32 rf_reg[GAPK_RF_REG_NUM_8730A] = {
	...
	_txgapk_backup_bb_registers_8730a(dm, bb_reg, bb_reg_backup);
	_txgapk_backup_rf_registers_8730a(dm, rf_reg, rf_reg_backup);
	_txgapk_afe_setting_8730a(dm, true);
	_txgapk_config_offset_table_8730a(dm);
	_txgapk_afe_setting_8730a(dm, false);
	_txgapk_reload_rf_registers_8730a(dm, rf_reg, rf_reg_backup);
	_txgapk_reload_bb_registers_8730a(dm, bb_reg, bb_reg_backup);
