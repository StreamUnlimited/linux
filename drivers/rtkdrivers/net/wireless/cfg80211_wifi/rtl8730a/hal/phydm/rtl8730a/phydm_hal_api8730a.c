...
#if (RTL8730A_SUPPORT)
#if (PHYDM_FW_API_ENABLE_8730A)
/* ======================================================================== */
/* These following functions can be used for PHY DM only*/
#ifdef CONFIG_TXAGC_DEBUG_8730A
__odm_func__
boolean phydm_set_pw_by_rate_8730a(struct dm_struct *dm, s8 *pw_idx,
				   u8 rate_idx)
{
	u32 pw_all = 0;
	u8 j = 0;
	...
	for (j = 0; j < 4; j++) {
		config_phydm_read_txagc_diff_8730a(dm, rate_idx + j);
	}
...
__odm_func__
void phydm_txagc_tab_buff_init_8730a(struct dm_struct *dm)
{
	u8 i;
...
__odm_func__
void phydm_sdm_reset_8730a(struct dm_struct *dm)
{
	/*reset HSSI*/
	/*phydm_rstb_3wire_8730a(dm, false);*/
	/*write RF-0x18*/
	odm_set_rf_reg(dm, RF_PATH_A, RF_0xbc, BIT(19), 0x0);
	odm_set_rf_reg(dm, RF_PATH_A, RF_0xbc, BIT(19), 0x1);
	odm_set_rf_reg(dm, RF_PATH_A, RF_0xbc, BIT(19), 0x0);
	/*reset HSSI*/
	/*phydm_rstb_3wire_8730a(dm, true);*/
}

__odm_func__
void phydm_bb_reset_8730a(struct dm_struct *dm)
{
	if (*dm->mp_mode) {
		return;
	}

	odm_set_mac_reg(dm, R_0x04, BIT(17), 1);
	odm_set_mac_reg(dm, R_0x04, BIT(17), 0);
	odm_set_mac_reg(dm, R_0x04, BIT(17), 1);
}
...
__odm_func__
void phydm_igi_toggle_8730a(struct dm_struct *dm)
{
	u32 igi = 0x20;
...
__odm_func__
u32 phydm_check_bit_mask_8730a(u32 bit_mask, u32 data_original, u32 data)
{
	u8 bit_shift = 0;
...
__odm_func__
boolean
config_phydm_direct_write_rf_reg_8730a(struct dm_struct *dm,
...
__odm_func__
boolean
config_phydm_write_rf_reg_8730a(struct dm_struct *dm, enum rf_path path,
				u32 reg_addr, u32 bit_mask, u32 data)
{
	...
	result = config_phydm_direct_write_rf_reg_8730a(dm, path, reg_addr,
			bit_mask, data);
	return true;
}
...
__odm_func__
boolean
phydm_write_txagc_1byte_8730a(struct dm_struct *dm, u32 pw_idx, u8 hw_rate)
{
#if (PHYDM_FW_API_FUNC_ENABLE_8730A)
...
__odm_func__
boolean
config_phydm_write_txagc_ref_8730a(struct dm_struct *dm, u8 power_index,
				   enum rf_path path,
				   enum PDM_RATE_TYPE rate_type)
{
	...
	if (rate_type == PDM_CCK) {
		if (path == RF_PATH_A) {
			odm_set_bb_reg(dm, txagc_ref, BIT(15), power_index);
		} else {
			odm_set_bb_reg(dm, txagc_ref, BIT(17), power_index);
		}
		PHYDM_DBG(dm, ODM_PHY_CONFIG,
		...
		if (path == RF_PATH_A) {
			odm_set_bb_reg(dm, txagc_ref, BIT(16), power_index);
		} else {
			odm_set_bb_reg(dm, txagc_ref, BIT(18), power_index);
		}
		PHYDM_DBG(dm, ODM_PHY_CONFIG,
			  "path-%d rate type %d (0x%x) = 0x%x\n",
			  path, rate_type, txagc_ref, power_index);
...
__odm_func__
boolean
config_phydm_write_txagc_diff_8730a(struct dm_struct *dm, s8 power_index1,
...
{
#if (defined(CONFIG_RUN_IN_DRV))
	s8 diff_tab[NUM_RATE_N_1SS]; /*power diff table of 2 paths*/
	u8 ref_pow_cck = dm->txagc_buff[RF_PATH_A][ODM_RATE11M];
	u8 ref_pow_ofdm = dm->txagc_buff[RF_PATH_A][ODM_RATEMCS7];
	u8 ref_pow_tmp = 0;
	enum rf_path path = 0;
	u8 i, j = 0;
	struct _hal_rf_ *rf = &dm->rf_table;
	...
	/* === [Reference base] =============================================*/
#ifdef CONFIG_TXAGC_DEBUG_8730A
	pr_debug("ref_pow_cck={%d}, ref_pow_ofdm={%d}\n",
		 ref_pow_cck, ref_pow_ofdm);
#endif
	/*Set OFDM/CCK Ref. power index*/
	config_phydm_write_txagc_ref_8730a(dm, ref_pow_cck, RF_PATH_A,
					   PDM_CCK);
	config_phydm_write_txagc_ref_8730a(dm, ref_pow_ofdm, RF_PATH_A,
					   PDM_OFDM);

	/* === [Power By Rate] ==============================================*/
	odm_move_memory(dm, diff_tab, dm->txagc_buff, NUM_RATE_N_1SS);
#ifdef CONFIG_TXAGC_DEBUG_8730A
	...
	/*CCK*/
	ref_pow_tmp = ref_pow_cck;
	for (j = ODM_RATE1M; j <= ODM_RATE11M; j++) {
		diff_tab[j] -= (s8)ref_pow_tmp;
		/**/
	}
	/*OFDM*/
	ref_pow_tmp = ref_pow_ofdm;
	...
	for (i = ODM_RATE1M; i <= ODM_RATEMCS7; i++) {
		if (i % 4 == 3) {
			config_phydm_write_txagc_diff_8730a(dm,
							    diff_tab[i - 3],
							    diff_tab[i - 2],
							    diff_tab[i - 1],
							    diff_tab[i],
...
__odm_func__
boolean config_phydm_write_txagc_8730a(struct dm_struct *dm, u32 pw_idx,
				    ...
				    struct txagc_table_8730a *tab)
{
	...
	/* === [Reference base] =============================================*/
	/*Set OFDM/CCK Ref. power index*/
	config_phydm_write_txagc_ref_8730a(dm, tab->ref_pow_cck, RF_PATH_A,
					   PDM_CCK);
	config_phydm_write_txagc_ref_8730a(dm, tab->ref_pow_ofdm, RF_PATH_A,
					   PDM_OFDM);
					...
					struct txagc_table_8730a *tab)
{
	...
	/* === [Power By Rate] ==============================================*/
	odm_move_memory(dm, diff_tab, txagc_buff, NUM_RATE_N_1SS);

	ref_pow_cck = diff_tab[ODM_RATE11M];

	ref_pow_ofdm = diff_tab[ODM_RATEMCS7];
	...
	odm_move_memory(dm, tab->ref_pow_cck, ref_pow_cck, 1);
	odm_move_memory(dm, tab->ref_pow_ofdm, ref_pow_ofdm, 1);
	odm_move_memory(dm, tab->diff_t, diff_tab, NUM_RATE_N_1SS);
}
#endif
				 ...
				 u8 hw_rate, enum PDM_RATE_TYPE rate_type)
{
	...
	read_back_data = (s8)config_phydm_read_txagc_diff_8730a(dm, hw_rate);
...
__odm_func__
boolean
config_phydm_trx_mode_8730a(struct dm_struct *dm, enum bb_path tx_path_en,
			    ...
			    enum bb_path tx_path_sel_1ss)
{
	...
	phydm_bb_reset_8730a(dm);
	phydm_igi_toggle_8730a(dm);

	return true;
}

void phydm_dis_cck_trx_8730a(struct dm_struct *dm, u8 set_type)
{
...
__odm_func__
void
phydm_cck_agc_tab_sel_8730a(struct dm_struct *dm, u8 table)
{
	odm_set_bb_reg(dm, R_0x18ac, 0xf000, table);
}
__odm_func__
void
phydm_ofdm_agc_tab_sel_8730a(struct dm_struct *dm, u8 table)
{
...
__odm_func__
boolean
config_phydm_switch_band_8730a(struct dm_struct *dm, u8 central_ch)
{
	u32 rf_reg18 = 0;
	u8 i = 0;
	...
	rf_reg18 = config_phydm_read_rf_reg_8730a(dm, RF_PATH_A, 0x18,
			RFREG_MASK);
	...
	if (central_ch <= 14) {
		...
		/* @Enable CCK TRx */
		phydm_dis_cck_trx_8730a(dm, PHYDM_SET);
		...
		/* 5G */
		/* @Enable CCK TRx */
		phydm_dis_cck_trx_8730a(dm, PHYDM_REVERT);
...
__odm_func__
void
phydm_cck_tx_shaping_filter_8730a(struct dm_struct *dm, u8 central_ch)
{
	if (central_ch == 14) {
...
__odm_func__
void
phydm_sco_trk_fc_setting_8730a(struct dm_struct *dm, u8 central_ch)
{
...
__odm_func__
boolean
config_phydm_switch_channel_8730a(struct dm_struct *dm, u8 central_ch)
{
	u32 rf_reg18 = 0;
	u32 rf_reg19 = 0;
	...
	/* @RF register setting */
	rf_reg18 = config_phydm_read_rf_reg_8730a(dm, RF_PATH_A, RF_0x18,
			RFREG_MASK);
	rf_reg19 = config_phydm_read_rf_reg_8730a(dm, RF_PATH_A, RF_0x19,
			RFREG_MASK);
	...
	if (central_ch <= 14) {
		...
		/* RX cck agc table 5*/
		phydm_cck_agc_tab_sel_8730a(dm, CCK_BW20_40_8730A);
		if (*dm->band_width == CHANNEL_WIDTH_20) {
			/* ofdm 20M agc table 4 */
			phydm_ofdm_agc_tab_sel_8730a(dm, OFDM_2G_BW20_40_8730A);
		} else {
		...
		phydm_ofdm_agc_tab_sel_8730a(dm, OFDM_5G_LOW_BAND_8730A);
	} else if ((central_ch >= 100) && (central_ch <= 144)) {
		/* 5G RX idle agc table = low band table = 2 */
		odm_set_bb_reg(dm, R_0x1ea8, BIT(7), 0x0);
		phydm_ofdm_agc_tab_sel_8730a(dm, OFDM_5G_MID_BAND_8730A);
	} else if (central_ch >= 149) {
		/* 5G RX idle agc table = low band table = 3 */
		odm_set_bb_reg(dm, R_0x1ea8, BIT(7), 0x0);
		phydm_ofdm_agc_tab_sel_8730a(dm, OFDM_5G_HIGH_BAND_8730A);
	}

	/* @3. Set central frequency for clock offset tracking */
	phydm_sco_trk_fc_setting_8730a(dm, central_ch);
	/* 4. Other 2.4G tx_shaping_fliter Settings*/
	if (is_2g_ch) {
		phydm_cck_tx_shaping_filter_8730a(dm, central_ch);
	}
...
__odm_func__
boolean
config_phydm_switch_bandwidth_8730a(struct dm_struct *dm, u8 pri_ch,
				    enum channel_width bw)
{
	...
	case CHANNEL_WIDTH_20:
		if (bw == CHANNEL_WIDTH_5) {
			...
			odm_set_bb_reg(dm, R_0x9f0, 0xf, 0x3);

			/* TX BPSK/QPSK BandEdge configure */
			odm_set_bb_reg(dm, R_0x81c, 0x3, 0x0);
			...
			odm_set_bb_reg(dm, R_0x9f0, 0xf, 0x4);
			...
			odm_set_bb_reg(dm, R_0x9f0, 0xf, 0x5);

			/* TX BPSK/QPSK BandEdge configure */
			odm_set_bb_reg(dm, R_0x81c, 0x3, 0x3);
		}
	...
	case CHANNEL_WIDTH_40:
		...
		/* TX BPSK/QPSK BandEdge configure */
		odm_set_bb_reg(dm, R_0x81c, 0x3, 0x1);
		break;
...
__odm_func__
boolean
config_phydm_switch_channel_bw_8730a(struct dm_struct *dm, u8 central_ch,
				     u8 primary_ch_idx,
				     enum channel_width bandwidth)
{
	/* @Switch band */
	if (!config_phydm_switch_band_8730a(dm, central_ch)) {
		return false;
	}

	/* @Switch channel */
	if (!config_phydm_switch_channel_8730a(dm, central_ch)) {
		return false;
	}

	/* @Switch bandwidth */
	if (!config_phydm_switch_bandwidth_8730a(dm, primary_ch_idx, bandwidth)) {
		return false;
	}
...
__odm_func__
void phydm_cck_gi_bound_8730a(struct dm_struct *dm)
{
...
__odm_func__
boolean
config_phydm_parameter_init_8730a(struct dm_struct *dm,
				  enum odm_parameter_init type)
{
	PHYDM_DBG(dm, ODM_PHY_CONFIG, "%s ======>\n", __func__);

	phydm_cck_gi_bound_8730a(dm);
	...
	phydm_bb_reset_8730a(dm);
#ifdef CONFIG_TXAGC_DEBUG_8730A
	/*phydm_txagc_tab_buff_init_8730a(dm);*/
#endif
...
__odm_func__
boolean
phydm_chk_bb_state_idle_8730a(struct dm_struct *dm)
{
	u32 dbgport = 0;
...
__odm_func_aon__
boolean
phydm_8730a_lps(struct dm_struct *dm, boolean enable_lps)
{
	u32 igi = 0x20;
...
/* ======================================================================== */
#endif /* PHYDM_FW_API_ENABLE_8730A */
