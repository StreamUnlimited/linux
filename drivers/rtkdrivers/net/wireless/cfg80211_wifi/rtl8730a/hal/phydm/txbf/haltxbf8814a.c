...
)
{
	...
	*dm->is_fw_dw_rsvd_page_in_progress = true;
	...
	tmp_reg422 = odm_read_1byte(dm, REG_FWHW_TXQ_HWSSN_CTRL_8814A + 2);
	odm_write_1byte(dm, REG_FWHW_TXQ_HWSSN_CTRL_8814A + 2,  tmp_reg422 & (~BIT(6)));
	...
	if (is_send_beacon) {
		odm_write_1byte(dm, REG_FWHW_TXQ_HWSSN_CTRL_8814A + 2, tmp_reg422);
	}
