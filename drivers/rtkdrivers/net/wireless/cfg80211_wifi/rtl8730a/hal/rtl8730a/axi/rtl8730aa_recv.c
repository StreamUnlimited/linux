...
static u32 rtl8730aa_wait_rxrdy(_adapter *padapter,
 ...
 */
static u16 rtl8730aa_check_rxdesc_remain(_adapter *padapter, int rx_queue_idx)
{
	struct recv_priv *r_priv = &padapter->recvpriv;
	...
	tmp_4bytes = rtw_read32(padapter, REG_RXQ_RXBD_IDX);
	...
	{
		num_rxdesc_to_handle = RX_BD_NUM_8730AA - desc_idx_host + desc_idx_hw;
	} else {
...
static void rtl8730aa_rx_mpdu(_adapter *padapter)
{
	struct recv_priv *r_priv = &padapter->recvpriv;
	...
	desc_size = rtl8730a_get_rx_desc_size(padapter);

	/* RX NORMAL PKT */

	remaing_rxdesc = rtl8730aa_check_rxdesc_remain(padapter, rx_q_idx);
	while (remaing_rxdesc) {
		...
		if (rtl8730aa_wait_rxrdy(padapter, rx_bd, rx_q_idx) !=
		    _SUCCESS)
...

		{
			...
			rtl8730a_query_rx_desc(precvframe, skb->data);
			...
			if (rtw_os_alloc_recvframe(padapter, precvframe,
				...
				RTW_INFO("rtl8730aa_rx_mpdu:can't allocate memory for skb copy\n");
				*((dma_addr_t *) skb->cb) =
					dma_map_single(&pdvobjpriv->pdev->dev,
			...
			if (pattrib->pkt_rpt_type == NORMAL_RX) {
				...
				if (pattrib->pkt_rpt_type == C2H_PACKET)
					rtl8730a_c2h_handler_no_io(padapter,
								   skb->data,
								   desc_size +
								   pattrib->pkt_len);
...
static void rtl8730aa_recv_tasklet(void *priv)
{
	_irqL	irqL;
	...
	rtl8730aa_rx_mpdu(padapter);
	_enter_critical(&pdvobjpriv->irq_th_lock, &irqL);
	pHalData->IntrMask[0] |= BIT_RXFF_FULL_INT_8730A;
	pHalData->IntrMask[1] |= (IMR_ROK_8730A | IMR_RDU_8730A);
	rtw_write32(padapter, REG_HIMR0_8730A, pHalData->IntrMask[0]);
	rtw_write32(padapter, REG_AXI_INTERRUPT_MASK, pHalData->IntrMask[1]);
	_exit_critical(&pdvobjpriv->irq_th_lock, &irqL);
}

static void rtl8730aa_xmit_beacon(PADAPTER Adapter)
{
...
static void rtl8730aa_prepare_bcn_tasklet(void *priv)
{
	_adapter *padapter = (_adapter *)priv;

	rtl8730aa_xmit_beacon(padapter);
}

s32 rtl8730aa_init_recv_priv(_adapter *padapter)
{
	...
	tasklet_init(&precvpriv->recv_tasklet,
		     (void(*)(unsigned long))rtl8730aa_recv_tasklet,
		     (unsigned long)padapter);

	tasklet_init(&precvpriv->irq_prepare_beacon_tasklet,
		     (void(*)(unsigned long))rtl8730aa_prepare_bcn_tasklet,
		     (unsigned long)padapter);
#endif
...
void rtl8730aa_free_recv_priv(_adapter *padapter)
{

}

int rtl8730aa_init_rxbd_ring(_adapter *padapter)
{
	struct recv_priv	*r_priv = &padapter->recvpriv;
	struct dvobj_priv	*pdvobjpriv = adapter_to_dvobj(padapter);
...
void rtl8730aa_free_rxbd_ring(_adapter *padapter)
{
	struct recv_priv *r_priv = &padapter->recvpriv;
	struct dvobj_priv *pdvobjpriv = adapter_to_dvobj(padapter);
	struct platform_device *pdev = pdvobjpriv->pdev;
