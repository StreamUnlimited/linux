 ...
 *
 *****************************************************************************/
#define _RTL8730AA_XMIT_C_

#include <drv_types.h>		/* PADAPTER, rtw_xmit.h and etc. */
#include <hal_data.h>		/* HAL_DATA_TYPE */
#include "../../halmac-rs/halmac_api.h"
#include "../rtl8730a.h"
#include "rtl8730aa.h"
...
static void rtl8730aa_xmit_tasklet(void *priv)
{
	_irqL irqL;
	...
	rtl8730aa_xmitframe_resume(padapter);

}

s32 rtl8730aa_init_xmit_priv(_adapter *padapter)
{
	s32 ret = _SUCCESS;
	...
	tasklet_init(&pxmitpriv->xmit_tasklet,
		     (void(*)(unsigned long))rtl8730aa_xmit_tasklet,
		     (unsigned long)padapter);
#endif
	rtl8730a_init_xmit_priv(padapter);

	return ret;
}

void rtl8730aa_free_xmit_priv(_adapter *padapter)
{
	struct dvobj_priv *pdvobjpriv = adapter_to_dvobj(padapter);
...
static s32 rtl8730aa_enqueue_xmitbuf(struct rtw_tx_ring	*ring,
...
{
	u16 txbd_reg_addr = REG_BEQ_TXBD_IDX_8730A;

	switch (q_idx) {

	case BK_QUEUE_INX:
		txbd_reg_addr = REG_BKQ_TXBD_IDX_8730A;
		break;

	case BE_QUEUE_INX:
		txbd_reg_addr = REG_BEQ_TXBD_IDX_8730A;
		break;

	case VI_QUEUE_INX:
		txbd_reg_addr = REG_VIQ_TXBD_IDX_8730A;
		break;

	case VO_QUEUE_INX:
		txbd_reg_addr = REG_VOQ_TXBD_IDX_8730A;
		break;

	case BCN_QUEUE_INX:
		txbd_reg_addr = REG_BEQ_TXBD_IDX_8730A;	/* need check */
		break;
	...
	case HIGH_QUEUE_INX:
		txbd_reg_addr = REG_MQ0_TXBD_IDX;
		break;
	default:
		break;
	}
...
struct xmit_frame *__rtw_alloc_cmdxmitframe_8814be(struct xmit_priv *pxmitpriv,
...
static void rtl8730aa_kick_bcnq(_adapter *padapter)
{
	u32 reg_bcnq;
	u32 bit_bcnq;

	reg_bcnq = REG_RX_RXBD_NUM;
	bit_bcnq = BIT_POLL_BCNQ;

	/* kick start */
	rtw_write16(padapter, reg_bcnq,
		    rtw_read16(padapter, reg_bcnq) | bit_bcnq);
}
 ...
 */
static void rtl8730aa_update_txbd(struct xmit_frame *pxmitframe,
				  u8 *txbd, s32 sz)
{
	...
	mapping = dma_map_single(&pdvobjpriv->pdev->dev, pxmitframe->buf_addr,
...
#if 1
		page_size_length = (sz + TX_WIFI_INFO_SIZE + (tx_page_size_reg * 128) - 1) /
	 ...
	 * For 8814be, it is required that TX_WIFI_INFO is put in first segment,
...
static s32 update_txdesc(struct xmit_frame *pxmitframe, s32 sz)
{
	...
	rtl8730a_update_txdesc(pxmitframe, ptxdesc);
	rtl8730a_dbg_dump_tx_desc(padapter, pxmitframe->frame_tag, ptxdesc);
	return 0;
}

static u32 rtw_ext_hiq_hwaddr(_adapter *padapter, u32 ff_hwaddr)
{
#ifdef CONFIG_PORT_BASED_HIQ
	if (ff_hwaddr != HIGH_QUEUE_INX) {
		return ff_hwaddr;
	}

	ff_hwaddr = HI_QUEUE_INX(padapter->hw_port);

	if (ff_hwaddr >= HW_QUEUE_ENTRY) {
		RTW_ERR("HIQ hwaddr=%d hw_port=%d\n", ff_hwaddr, padapter->hw_port);
		ff_hwaddr = HW_QUEUE_ENTRY - 1;
	}
...
s32 rtl8730aa_dump_xframe(_adapter *padapter, struct xmit_frame *pxmitframe)
{
	s32 ret = _SUCCESS;
	...
	for (t = 0; t < pattrib->nr_frags; t++) {
		...
		ff_hwaddr = rtw_ext_hiq_hwaddr(padapter, ff_hwaddr);
		...
		/* rtl8730aa_update_txbd() must be called after update_txdesc().
		 * It rely on rtl8730aa_update_txbd() to map it into non cache
		 * memory */

		rtl8730aa_update_txbd(pxmitframe, txbd, sz);

		if (pxmitbuf->buf_tag != XMITBUF_CMD) {
			rtl8730aa_enqueue_xmitbuf(ptx_ring, pxmitbuf);
		}
 ...
 */
s32 rtl8730aa_xmit_buf_handler(_adapter *padapter)
{
	PHAL_DATA_TYPE phal;
	...
	do {
		...
		rtl8730aa_dump_xframe(padapter, pxmitframe);
	} while (1);
...
static s32 xmitframe_amsdu_direct(_adapter *padapter, struct xmit_frame *pxmitframe)
{
	...
	if (res == _SUCCESS) {
		...
		enqueue_pending_xmitbuf(pxmitpriv, pxmitframe->pxmitbuf);
#else
		res = rtl8730aa_dump_xframe(padapter, pxmitframe);
#endif
	} else {
		rtw_free_xmitbuf(pxmitpriv, pxmitbuf);
		rtw_free_xmitframe(pxmitpriv, pxmitframe);
...
#else
					rtl8730aa_dump_xframe(padapter, pxmitframe_next);
#endif
...
s32 rtl8730aa_mgnt_xmit(_adapter *padapter, struct xmit_frame *pmgntframe)
{
	...
	if ((pattrib->qsel == QSLT_CMD) || (pattrib->qsel == QSLT_BEACON)) {
		ret = rtl8730aa_dump_xframe(padapter, pmgntframe);
	} else {
...
#else
	return rtl8730aa_dump_xframe(padapter, pmgntframe);
#endif
}
 ...
 */
s32 rtl8730aa_hal_xmit(_adapter *padapter, struct xmit_frame *pxmitframe)
{
	return pre_xmitframe(padapter, pxmitframe);
}

#ifdef CONFIG_RTW_MGMT_QUEUE
s32 rtl8730aa_hal_mgmt_xmitframe_enqueue(_adapter *padapter,
		struct xmit_frame *pxmitframe)
{
	struct xmit_priv *pxmitpriv = &padapter->xmitpriv;
	s32 err;
...
s32 rtl8730aa_hal_xmitframe_enqueue(_adapter *padapter,
				    struct xmit_frame *pxmitframe)
{
	struct xmit_priv *pxmitpriv = &padapter->xmitpriv;
	s32 err;
...
int rtl8730aa_init_txbd_ring(_adapter *padapter, unsigned int q_idx,
			     unsigned int entries)
{
	struct xmit_priv *t_priv = &padapter->xmitpriv;
	struct dvobj_priv *pdvobjpriv = adapter_to_dvobj(padapter);
	struct platform_device *pdev = pdvobjpriv->pdev;
	...
	txbd = dma_alloc_coherent(&pdev->dev, sizeof(struct tx_buf_desc) * entries, &dma, GFP_KERNEL);
	...
	_rtw_memset(txbd, 0, sizeof(struct tx_buf_desc) * entries);
	...
	RTW_INFO("%s queue:%d, ring_addr:0x%08x\n", __func__, q_idx, txbd);


	return _SUCCESS;
}

void rtl8730aa_free_txbd_ring(_adapter *padapter, unsigned int prio)
{
	struct xmit_priv *t_priv = &padapter->xmitpriv;
	struct dvobj_priv *pdvobjpriv = adapter_to_dvobj(padapter);
	struct platform_device *pdev = pdvobjpriv->pdev;
	...
	while (ring->qlen) {
		...
		if (pxmitbuf) {
			...
			dma_unmap_single(&pdev->dev, mapping, pxmitbuf->len,
					 DMA_TO_DEVICE);
	...
	dma_free_coherent(&pdev->dev, sizeof(struct tx_buf_desc) * ring->entries,
...
static void _draw_queue(PADAPTER Adapter, int prio)
{
	...
	index = ring->idx;
	_rtw_memset(line, '.', TX_BD_NUM_8730AA);
	...
	if (index <= hw) {
		...
		for (i = index; i < TX_BD_NUM_8730AA; i++) {
			line[i] = ':';
		}
		for (i = 0; i < hw; i++) {
			line[i] = ':';
 ...
 */
static u32 rtl8730aa_check_txdesc_closed(PADAPTER Adapter, u32 queue_idx,
		struct rtw_tx_ring *ring)
{
	/*
...
void rtl8730aa_tx_isr(PADAPTER Adapter, int prio)
{
	struct xmit_priv *t_priv = &Adapter->xmitpriv;
	...
	while (ring->qlen) {
		...
		if (!rtl8730aa_check_txdesc_closed(Adapter, prio, ring)) {
			return;
		}
		...
		if (pxmitbuf) {
			...
			dma_unmap_single(pdvobjpriv->pdev, mapping,
...
void rtl8730aa_tx_isr(PADAPTER Adapter, int prio)
{
	...
	while (ring->qlen) {
		 ...
		 */
		if (prio != BCN_QUEUE_INX) {
			if (!rtl8730aa_check_txdesc_closed(Adapter,
							   prio, ring)) {
				return;
			}
...
#if 0 /* 8814b change 00[31] to DISQSELSEQ */
		else if (prio == BCN_QUEUE_INX) {
			SET_TX_DESC_BUFFER_OWN_92E(tx_desc, 0);
		}
#endif
		...
		if (pxmitbuf) {
			mapping = GET_TX_BD_PHYSICAL_ADDR0_LOW(tx_desc);
			dma_unmap_single(&pdvobjpriv->pdev->dev, mapping,
					 pxmitbuf->len, DMA_TO_DEVICE);
			rtw_sctx_done(&pxmitbuf->sctx);
			rtw_free_xmitbuf(&(pxmitbuf->padapter->xmitpriv),
					 pxmitbuf);
...
s32 rtl8730aa_hostap_mgnt_xmit_entry(_adapter *padapter, _pkt *pkt)
{
#ifdef PLATFORM_LINUX
	u16 fc;
	int rc, len, pipe;
