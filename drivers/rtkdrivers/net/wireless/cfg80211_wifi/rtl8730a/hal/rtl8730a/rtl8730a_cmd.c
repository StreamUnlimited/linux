 ...
 *
 ******************************************/
s32 rtl8730a_fillh2ccmd(PADAPTER adapter, u8 id, u32 buf_len, u8 *pbuf)
{
...
void rtl8730a_req_txrpt_cmd(PADAPTER adapter, u8 macid)
{
	u8 h2c[RTW_HALMAC_H2C_MAX_SIZE] = {0};
...
{
	return _rtl8730a_set_FwPwrMode_cmd(adapter, psmode, rf_off);
}
...
void rtl8730a_set_FwPwrMode_rfon_ctrl_cmd(PADAPTER adapter, u8 rfon_ctrl)
{
	...
	return _rtl8730a_set_FwPwrMode_cmd(adapter, pwrpriv->power_mgnt, rfon_ctrl);
}
...
void rtl8730a_set_fw_pwrmode_inips_cmd_wowlan(PADAPTER padapter, u8 ps_mode)
{
	...
	rtl8730a_fillh2ccmd(padapter, H2C_INACTIVE_PS_, sizeof(param), param);
}
#endif /* CONFIG_WOWLAN */
...
#ifdef CONFIG_BT_COEXIST
void rtl8730a_download_BTCoex_AP_mode_rsvd_page(PADAPTER adapter)
{
...
static void process_c2h_event(PADAPTER adapter, u8 *c2h, u32 size)
{
	...
	desc_size = rtl8730a_get_rx_desc_size(adapter);
...
void rtl8730a_c2h_handler(PADAPTER adapter, u8 *pbuf, u16 length)
{
#ifdef CONFIG_WOWLAN
	struct pwrctrl_priv *pwrpriv = adapter_to_pwrctl(adapter);
 ...
 */
void rtl8730a_c2h_handler_no_io(PADAPTER adapter, u8 *pbuf, u16 length)
{
	u32 desc_size;
	u8 id, seq;
	u8 *pc2h_content;
