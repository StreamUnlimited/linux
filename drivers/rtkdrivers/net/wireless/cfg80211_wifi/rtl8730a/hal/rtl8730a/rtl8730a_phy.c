 ...
 *	_FALSE	Fail
 */
u8 rtl8730a_phy_init_mac_register(PADAPTER adapter)
{
	PHAL_DATA_TYPE hal;
	u8 ret = _TRUE;
	int res;
	enum hal_status status;
	u32 RRSC_BITMAP = 0x1FF; /*from halmac*/
...
static u8 init_bb_reg(PADAPTER adapter)
{
	...
	//hal->crystal_cap &= 0x7F;
	//phydm_set_crystal_cap(phydm, hal->crystal_cap);
...
static u8 _init_rf_reg(PADAPTER adapter)
{
	...
	value32 = rtw_sys_read32(adapter, SYSTEM_CTRL_BASE_LP, XTAL_ANAPAR_XTAL_OFF_0);
	value32 &= ~XTAL_BIT_DRV_RF_LATCH;
	rtw_sys_write32(adapter, SYSTEM_CTRL_BASE_LP, XTAL_ANAPAR_XTAL_OFF_0, value32);
...
u8 rtl8730a_phy_init(PADAPTER adapter)
{
	...
	ret = config_phydm_parameter_init_8730a(phydm, ODM_PRE_SETTING);
	if (FALSE == ret) {
		return _FALSE;
	}
	...
	ret = config_phydm_parameter_init_8730a(phydm, ODM_POST_SETTING);
	if (FALSE == ret) {
		return _FALSE;
	}
...
static void init_phydm_cominfo(PADAPTER adapter)
{
	...
	odm_cmn_info_init(p_dm_odm, ODM_CMNINFO_IC_TYPE, ODM_RTL8730A);
...
void rtl8730a_phy_init_dm_priv(PADAPTER adapter)
{
	struct dm_struct *podmpriv = adapter_to_phydm(adapter);

	init_phydm_cominfo(adapter);
	odm_cmn_info_init(podmpriv, ODM_CMNINFO_IC_TYPE, ODM_RTL8730A);
	odm_init_all_timers(podmpriv);
}

void rtl8730a_phy_deinit_dm_priv(PADAPTER adapter)
{
	struct dm_struct *podmpriv = adapter_to_phydm(adapter);
...
void rtl8730a_phy_init_haldm(PADAPTER adapter)
{
...
static void check_rxfifo_full(PADAPTER adapter)
{
	...
	if (regsty->check_hw_status == 1) {
		...
		val8 = rtw_read8(adapter, REG_RXERR_RPT_8730A + 3);
		rtw_write8(adapter, REG_RXERR_RPT_8730A + 3, (val8 | 0xa0));

		pdbgpriv->dbg_rx_fifo_last_overflow = pdbgpriv->dbg_rx_fifo_curr_overflow;
		pdbgpriv->dbg_rx_fifo_curr_overflow = rtw_read16(adapter, REG_RXERR_RPT_8730A);
		pdbgpriv->dbg_rx_fifo_diff_overflow =
			pdbgpriv->dbg_rx_fifo_curr_overflow - pdbgpriv->dbg_rx_fifo_last_overflow;
	}
}

void rtl8730a_phy_haldm_watchdog(PADAPTER adapter)
{
...
u32 rtl8730a_read_bb_reg(PADAPTER adapter, u32 addr, u32 mask)
{
	...
	return 0;
#endif
	/* testchip need to acccess some regs indirectly, DD will modify default access in mpchip */
	if (addr >= 0x4000 && addr <= 0x4fff) {
		while (rtw_read32(adapter, 0x1e8) & (BIT(30) | BIT(31))) {
			rtw_udelay_os(1);
		}
		/* @0x1e8[14:2] for reg addr,0x1e8[18:15] for bytes ctrl,0x1e8[31:30] for write or read */
		addr &= 0x7fff;
		ctrl = addr;
		ctrl |= BIT(31);
		rtw_write32(adapter, 0x1e8, ctrl);
		val = rtw_read32(adapter, 0x1ec);
	} else {
...
void rtl8730a_write_bb_reg(PADAPTER adapter, u32 addr, u32 mask, u32 val)
{
	u32 val_org, shift;
	u32 ctrl = 0;
	...
	if (addr >= 0x4000 && addr <= 0x4fff) {
		...
		addr &= 0x7fff;
		ctrl |= (addr | BIT(30));
		if (mask != 0xFFFFFFFF) { //if not "double word" write
			shift = phy_calculatebitshift(mask);
			val = val << ((shift - 15) * 8);
			ctrl |= mask;
		} else { // byte write
...
u32 rtl8730a_read_rf_reg(PADAPTER adapter, enum rf_path path, u32 addr, u32 mask)
{
	...
	val = config_phydm_read_rf_reg_8730a(phydm, path, addr, mask);
	if (!config_phydm_read_rf_check_8730a(val))
...
void rtl8730a_write_rf_reg(PADAPTER adapter, enum rf_path path, u32 addr, u32 mask, u32 val)
{
	...
	ret = config_phydm_write_rf_reg_8730a(phydm, path, addr, mask, val);
	if (_FALSE == ret)
...
void rtl8730a_set_tx_power_level(PADAPTER adapter, u8 channel)
{
	...
	for (path = RF_PATH_A; path < hal_spec->rf_reg_path_num; ++path) {
		...
		if (hal_data->txpwr_pg_mode == TXPWR_PG_WITH_TSSI_OFFSET) {
			_halrf_tssi_set_powerlevel_8730a(phydm, path);
		} else
#endif
		{
			/*
			* can't bypass unused path
			* because phydm need all path value to calculate min diff
			*/
...
void rtl8730a_set_txpwr_done(_adapter *adapter)
{
	...
	if (hal_data->txpwr_pg_mode == TXPWR_PG_WITH_PWR_IDX
		...
		config_phydm_set_txagc_to_hw_8730a(phydm);
	}
}
...
void rtl8730a_set_tx_power_index(PADAPTER adapter, u32 powerindex, enum rf_path rfpath, u8 rate)
{
	...
	if (rate > DESC_RATEMCS7 || rfpath > RF_PATH_A) {
		RTW_ERR(FUNC_ADPT_FMT"%s (rfpath:%d, power index:0x%02x) write TX-AGC failed!!!\n",  __func__, rfpath, powerindex);
		return;
	}

	/*
	* For 8730A, phydm api use 4 bytes txagc value
	...
	*/
	...
	write_ret = config_phydm_write_txagc_8730a(phydm, hal->txagc_set_buf, rfpath, rate);
...
static void switch_chnl_and_set_bw_by_drv(PADAPTER adapter, u8 switch_band)
{
	...
	if (switch_band) {
		...
		ret = config_phydm_switch_band_8730a(p_dm_odm, hal->current_channel);

		if (!ret) {
			RTW_INFO("%s: config_phydm_switch_band_8730a fail\n", __FUNCTION__);
			rtw_warn_on(1);
			return;
		}
	}
	...
	if (hal->bSwChnl) {
		ret = config_phydm_switch_channel_8730a(p_dm_odm, hal->current_channel);
		hal->bSwChnl = _FALSE;

		if (!ret) {
			RTW_INFO("%s: config_phydm_switch_channel_8730a fail\n", __FUNCTION__);
			rtw_warn_on(1);
			return;
		}
	}
	...
	if (hal->bSetChnlBW) {
		...
		ret = config_phydm_switch_bandwidth_8730a(p_dm_odm, pri_ch_idx, hal->current_channel_bw);
		hal->bSetChnlBW = _FALSE;

		if (!ret) {
			RTW_INFO("%s: config_phydm_switch_bandwidth_8730a fail\n", __FUNCTION__);
			rtw_warn_on(1);
			return;
		}
	}
...
void phy_set_bb_swing_by_band_8730a(_adapter *adapter, u8 band, u8 previous_band)
{
	PHAL_DATA_TYPE hal = GET_HAL_DATA(adapter);
	struct dm_struct *p_dm_odm = &hal->odmpriv;
	s8 bb_diff_between_band = 0;
	struct dm_rf_calibration_struct *p_rf_cal_info = &(p_dm_odm->rf_calibrate_info);

	if (band != previous_band) {
		bb_diff_between_band = p_rf_cal_info->bb_swing_diff_2g - p_rf_cal_info->bb_swing_diff_5g;
		bb_diff_between_band = (band == BAND_ON_2_4G) ? bb_diff_between_band : (-1 * bb_diff_between_band);
		p_rf_cal_info->default_ofdm_index += bb_diff_between_band * 2;
	}
 ...
 */
void rtl8730a_switch_chnl_and_set_bw(PADAPTER adapter)
{
	PHAL_DATA_TYPE hal = GET_HAL_DATA(adapter);
	struct dm_struct *p_dm_odm = &hal->odmpriv;
	u8 center_ch = 0, ret = 0, switch_band = _FALSE;
	u8 current_band = hal->current_band_type;
	...
	if (switch_band) {
		...
		ret = config_phydm_switch_band_8730a(p_dm_odm, hal->current_channel);
		if (!ret) {
			RTW_ERR("%s:config_phydm_switch_band_8730a fail!\n", __func__);
			return;
		}
		phy_set_bb_swing_by_band_8730a(adapter, hal->current_band_type, current_band);
	}
	...
	if (hal->bSwChnl) {
		...
		hal->bSwChnl = _FALSE;
		if (!ret) {
			RTW_ERR("%s:config_phydm_switch_channel_8730a fail!\n", __func__);
			return;
		}
	}
	...
	if (hal->bSetChnlBW) {
		...
		/* 3.1 AmebaSmart is 20M BW only, no need to set mac */
		/* 3.2 set BB/RF register */
		ret = config_phydm_switch_bandwidth_8730a(p_dm_odm, pri_ch_idx, hal->current_channel_bw);
		hal->bSetChnlBW = _FALSE;
		if (!ret) {
			RTW_ERR("%s config_phydm_switch_bandwidth_8730a fail!\n", __func__);
			return;
		}
	}

#if 0//def CONFIG_TXPWR_PG_WITH_PWR_IDX
	...
	/* TX Power Setting */
	rtl8730a_set_tx_power_level(adapter, hal->current_channel);
...
void rtl8730a_handle_sw_chnl_and_set_bw(
	...
	u8 ChnlOffsetOf80MHz, u8 CenterFrequencyIndex1)
{
	...
	if (!RTW_CANNOT_RUN(Adapter)) {
		rtl8730a_switch_chnl_and_set_bw(Adapter);
	} else {
		if (hal->bSwChnl) {
...
{
	rtl8730a_handle_sw_chnl_and_set_bw(adapter, _TRUE, _TRUE, center_ch, bw, offset40, offset80, center_ch);
}

void rtl8730a_notch_filter_switch(PADAPTER adapter, bool enable)
{
	if (enable) {
 ...
 */
void rtl8730a_mp_config_rfpath(PADAPTER adapter)
{
	PHAL_DATA_TYPE hal;
	PMPT_CONTEXT mpt;
...
static void _sounding_reset_all(PADAPTER adapter)
{
	...
	rtw_write8(adapter, REG_TXBF_CTRL_8730A + 3, 0);
	...
	mu_tx_ctl = rtw_read32(adapter, REG_MU_TX_CTRL_8730A);
	for (i = 0; i < 6; i++) {
		mu_tx_ctl = BIT_SET_MU_TAB_SEL_8730A(mu_tx_ctl, i);
		rtw_write32(adapter, REG_MU_TX_CTRL_8730A, mu_tx_ctl);
		/* set MU STA gid valid table */
		rtw_write32(adapter, REG_MU_STA_GID_VLD_8730A, 0);
	}

	/* Disable TxMU PPDU */
	mu_tx_ctl &= ~BIT_MU_EN_8730A;
	rtw_write32(adapter, REG_MU_TX_CTRL_8730A, mu_tx_ctl);
}
...
static void _sounding_config_su(PADAPTER adapter, struct beamformee_entry *bfee, enum _HW_CFG_SOUNDING_TYPE cfg_type)
{
	...
	txbf_ctrl = rtw_read32(adapter, REG_TXBF_CTRL_8730A);
	new_ctrl = txbf_ctrl;

	/* Clear TxBF status at 20M/40/80M first */
	switch (bfee->su_reg_index) {
	case 0:
		new_ctrl &= ~(BIT_TXBF0_20M_8730A | BIT_TXBF0_40M_8730A | BIT_TXBF0_80M_160M_8730A);
		break;
	case 1:
		new_ctrl &= ~(BIT_TXBF1_20M_8730A | BIT_TXBF1_40M_8730A | BIT_TXBF1_80M_160M_8730A);
		break;
	}
	...
	case HW_CFG_SOUNDING_TYPE_SOUNDDOWN:
		...
		case CHANNEL_WIDTH_80:
			if (0 == bfee->su_reg_index) {
				new_ctrl |= BIT_TXBF0_80M_160M_8730A;
			} else if (1 == bfee->su_reg_index) {
				new_ctrl |= BIT_TXBF1_80M_160M_8730A;
			}
		/* fall through */
		case CHANNEL_WIDTH_40:
			if (0 == bfee->su_reg_index) {
				new_ctrl |= BIT_TXBF0_40M_8730A;
			} else if (1 == bfee->su_reg_index) {
				new_ctrl |= BIT_TXBF1_40M_8730A;
			}
		/* fall through */
		case CHANNEL_WIDTH_20:
			if (0 == bfee->su_reg_index) {
				new_ctrl |= BIT_TXBF0_20M_8730A;
			} else if (1 == bfee->su_reg_index) {
				new_ctrl |= BIT_TXBF1_20M_8730A;
			}
			break;
		}
		break;
	...
	if (new_ctrl != txbf_ctrl) {
		rtw_write32(adapter, REG_TXBF_CTRL_8730A, new_ctrl);
	}
}
...
static void _sounding_config_mu(PADAPTER adapter, struct beamformee_entry *bfee, enum _HW_CFG_SOUNDING_TYPE cfg_type)
{
	...
	case HW_CFG_SOUNDING_TYPE_LEAVE:
		...
		if (TEST_FLAG(bfee->cap, BEAMFORMEE_CAP_VHT_MU)) {
			...
			mu_tx_ctl = BIT_SET_MU_TAB_SEL_8730A(mu_tx_ctl, bfee->mu_reg_index);
			rtw_write32(adapter, REG_MU_TX_CTRL_8730A, mu_tx_ctl);
			/* Set MU STA gid valid table */
			rtw_write32(adapter, REG_MU_STA_GID_VLD_8730A, 0);
		} else {
	...
	case HW_CFG_SOUNDING_TYPE_SOUNDDOWN:
		...
		for (i = 0; i < 6; i++) {
			...
			rtw_write32(adapter, REG_MU_TX_CTRL_8730A, mu_tx_ctl);
			/* Set MU STA gid valid table */
			rtw_write32(adapter, REG_MU_STA_GID_VLD_8730A, gid_valid[i]);
		}
...
static void _config_beamformer_su(PADAPTER adapter, struct beamformer_entry *bfer)
{
	...
	if (bfer->su_reg_index == 0) {
		addr_bfer_info = REG_ASSOCIATED_BFMER0_INFO_8730A;
		addr_csi_rpt = REG_TX_CSI_RPT_PARAM_BW20_8730A;
	} else {
		addr_bfer_info = REG_ASSOCIATED_BFMER1_INFO_8730A;
		addr_csi_rpt = REG_TX_CSI_RPT_PARAM_BW20_8730A + 2;
	}

	/* Sounding protocol control */
	rtw_write8(adapter, REG_SND_PTCL_CTRL_8730A, 0xDB);
	...
	rtw_write8(adapter, REG_SND_PTCL_CTRL_8730A + 3, 0x70);
}
...
static void _config_beamformer_mu(PADAPTER adapter, struct beamformer_entry *bfer)
{
	...
	val8 = rtw_read8(adapter, REG_RXFLTMAP0_8730A + 1);
	val8 |= (BIT_MGTFLT14EN_8730A >> 8);
	rtw_write8(adapter, REG_RXFLTMAP0_8730A + 1, val8);

	/* Set 0x6A2[5:4] = 1 to NDPA and BF report poll */
	val8 = rtw_read8(adapter, REG_RXFLTMAP1_8730A);
	val8 |= BIT_CTRLFLT4EN_8730A | BIT_CTRLFLT5EN_8730A;
	rtw_write8(adapter, REG_RXFLTMAP1_8730A, val8);
}
...
static void _config_beamformee_su(PADAPTER adapter, struct beamformee_entry *bfee)
{
	...
	val32 = rtw_read32(adapter, REG_TXBF_CTRL_8730A);
	if (idx == 0) {
		val32 = BIT_SET_TXBF0_AID_8730A(val32, p_aid);
		val32 &= ~(BIT_TXBF0_20M_8730A | BIT_TXBF0_40M_8730A | BIT_TXBF0_80M_160M_8730A);
	} else {
		val32 = BIT_SET_TXBF1_AID_8730A(val32, p_aid);
		val32 &= ~(BIT_TXBF1_20M_8730A | BIT_TXBF1_40M_8730A | BIT_TXBF1_80M_160M_8730A);
	}
	val32 |= BIT_EN_NDPA_INT_8730A | BIT_NDPA_PARA_8730A | BIT_ENABLE_NDPA_8730A;
	rtw_write32(adapter, REG_TXBF_CTRL_8730A, val32);

	/* CSI report parameters of Beamformee */
	val32 = rtw_read32(adapter, REG_ASSOCIATED_BFMEE_SEL_8730A);
	if (idx == 0) {
		val32 = BIT_SET_AID0_8730A(val32, p_aid);
		val32 |= BIT_TXUSER_ID0_8730A;

		/* unknown? */
		val32 &= 0x03FFFFFF;
		val32 |= 0x60000000;
	} else {
		val32 = BIT_SET_AID1_8730A(val32, p_aid);
		val32 |= BIT_TXUSER_ID1_8730A;

		/* unknown? */
		val32 &= 0x03FFFFFF;
		val32 |= 0xE0000000;
	}
	rtw_write32(adapter, REG_ASSOCIATED_BFMEE_SEL_8730A, val32);
}
...
static void _config_beamformee_mu(PADAPTER adapter, struct beamformee_entry *bfee)
{
	...
	u32 mu_reg[6] = {REG_WMAC_ASSOCIATED_MU_BFMEE2_8730A,
			 REG_WMAC_ASSOCIATED_MU_BFMEE3_8730A,
			 REG_WMAC_ASSOCIATED_MU_BFMEE4_8730A,
			 REG_WMAC_ASSOCIATED_MU_BFMEE5_8730A,
			 REG_WMAC_ASSOCIATED_MU_BFMEE6_8730A,
			 REG_WMAC_ASSOCIATED_MU_BFMEE7_8730A
			};
	/* Misc */
	u8 i, val8;
	u16 val16;
	...
	/* select MU STA table */
	val32 = rtw_read32(adapter, REG_MU_TX_CTRL_8730A);
	val32 = BIT_SET_MU_TAB_SEL_8730A(val32, idx);
	rtw_write32(adapter, REG_MU_TX_CTRL_8730A, val32);

	/* Reset gid_valid table */
	rtw_write32(adapter, REG_MU_STA_GID_VLD_8730A, 0);
	rtw_write32(adapter, REG_MU_STA_USER_POS_INFO_8730A, user_position_l);
	rtw_write32(adapter, REG_MU_STA_USER_POS_INFO_8730A + 4, user_position_h);

	/* set validity of MU STAs */
	val32 = BIT_SET_MU_TAB_VALID_8730A(val32, info->beamformee_mu_reg_maping);
	rtw_write32(adapter, REG_MU_TX_CTRL_8730A, val32);
	...
	val8 = rtw_read8(adapter, REG_TXBF_CTRL_8730A + 3);
	val8 |= 0xD0; /* Set bit 28, 30, 31 to 3b'111 */
	rtw_write8(adapter, REG_TXBF_CTRL_8730A + 3, val8);

	/* Set NDPA rate*/
	val8 = phydm_get_ndpa_rate(GET_PDM_ODM(adapter));
	rtw_write8(adapter, REG_NDPA_RATE_8730A, val8);

	val8 = rtw_read8(adapter, REG_NDPA_OPT_CTRL_8730A);
	val8 = BIT_SET_NDPA_BW_8730A(val8, 0); /* Clear bit 0, 1 */
	rtw_write8(adapter, REG_NDPA_OPT_CTRL_8730A, val8);

	val32 = rtw_read32(adapter, REG_SND_PTCL_CTRL_8730A);
	val32 = (val32 & 0xFF0000FF) | 0x020200; /* Set [23:8] to 0x0202 */
	rtw_write32(adapter, REG_SND_PTCL_CTRL_8730A, val32);
	...
	val8 = rtw_read8(adapter, REG_SND_PTCL_CTRL_8730A + 1);
	val8 |= (BIT_VHTNDP_RPTPOLL_CSI_STR_OFFSET_SEL_8730A >> 8);
	rtw_write8(adapter, REG_SND_PTCL_CTRL_8730A + 1, val8);
...
static void _reset_beamformer_su(PADAPTER adapter, struct beamformer_entry *bfer)
{
	...
	if (idx == 0) {
		rtw_write32(adapter, REG_ASSOCIATED_BFMER0_INFO_8730A, 0);
		rtw_write16(adapter, REG_ASSOCIATED_BFMER0_INFO_8730A + 4, 0);
		rtw_write16(adapter, REG_TX_CSI_RPT_PARAM_BW20_8730A, 0);
	} else {
		rtw_write32(adapter, REG_ASSOCIATED_BFMER1_INFO_8730A, 0);
		rtw_write16(adapter, REG_ASSOCIATED_BFMER1_INFO_8730A + 4, 0);
		rtw_write16(adapter, REG_TX_CSI_RPT_PARAM_BW20_8730A + 2, 0);
	}
...
static void _reset_beamformee_su(PADAPTER adapter, struct beamformee_entry *bfee)
{
	...
	/* clear P_AID */
	txbf_ctrl = rtw_read32(adapter, REG_TXBF_CTRL_8730A);
	bfmee_sel = rtw_read32(adapter, REG_ASSOCIATED_BFMEE_SEL_8730A);
	if (idx == 0) {
		txbf_ctrl = BIT_SET_TXBF0_AID_8730A(txbf_ctrl, 0);
		txbf_ctrl &= ~(BIT_TXBF0_20M_8730A | BIT_TXBF0_40M_8730A | BIT_TXBF0_80M_160M_8730A);

		bfmee_sel = BIT_SET_AID0_8730A(bfmee_sel, 0);
		bfmee_sel &= ~BIT_TXUSER_ID0_8730A;
	} else {
		txbf_ctrl = BIT_SET_TXBF1_AID_8730A(txbf_ctrl, 0);
		txbf_ctrl &= ~(BIT_TXBF1_20M_8730A | BIT_TXBF1_40M_8730A | BIT_TXBF1_80M_160M_8730A);

		bfmee_sel = BIT_SET_AID1_8730A(bfmee_sel, 0);
		bfmee_sel &= ~BIT_TXUSER_ID1_8730A;
	}
	txbf_ctrl |= BIT_EN_NDPA_INT_8730A | BIT_NDPA_PARA_8730A | BIT_ENABLE_NDPA_8730A;
	rtw_write32(adapter, REG_TXBF_CTRL_8730A, txbf_ctrl);
	rtw_write32(adapter, REG_ASSOCIATED_BFMEE_SEL_8730A, bfmee_sel);
...
void rtl8730a_phy_bf_reset_all(PADAPTER adapter)
{
	struct beamforming_info *info;
	u8 i, val8;
	u32 val32;
	...
	val32 = rtw_read32(adapter, REG_MU_TX_CTRL_8730A);
	val32 = BIT_SET_MU_TAB_VALID_8730A(val32, 0);
	rtw_write32(adapter, REG_MU_TX_CTRL_8730A, val32);
	...
	for (i = 0; i < MAX_NUM_BEAMFORMEE_MU; i++) {
		rtw_write16(adapter, REG_WMAC_ASSOCIATED_MU_BFMEE2_8730A + (i * 2), 0);
	}

	/* set validity of MU STA */
	rtw_write32(adapter, REG_MU_TX_CTRL_8730A, 0);

	/* Reset SU BFee entry setting */
	/* SU BF0 and BF1 */
	val32 = BIT_EN_NDPA_INT_8730A | BIT_NDPA_PARA_8730A | BIT_ENABLE_NDPA_8730A;
	rtw_write32(adapter, REG_TXBF_CTRL_8730A, val32);
	rtw_write32(adapter, REG_ASSOCIATED_BFMEE_SEL_8730A, 0);
	...
	if (_TRUE == info->bEnableSUTxBFWorkAround) {
		rtl8730a_phy_bf_set_csi_report(adapter, &info->TargetCSIInfo);
	}
#endif
...
void rtl8730a_phy_bf_init(PADAPTER adapter)
{
	u8 v8;
	u32 v32;

	v32 = rtw_read32(adapter, REG_MU_TX_CTRL_8730A);
	/* Enable P1 aggr new packet according to P0 transfer time */
	v32 |= BIT_MU_SECONDARY_WAITMODE_EN_8730A;
	/* MU Retry Limit */
	v32 = BIT_SET_MU_RL_8730A(v32, 0xA);
	/* Disable Tx MU-MIMO until sounding done */
	v32 &= ~BIT_MU_EN_8730A;
	/* Clear validity of MU STAs */
	v32 = BIT_SET_MU_TAB_VALID_8730A(v32, 0);
	rtw_write32(adapter, REG_MU_TX_CTRL_8730A, v32);

	/* MU-MIMO Option as default value */
	v8 = BIT_WMAC_TXMU_ACKPOLICY_8730A(3);
	v8 |= BIT_WMAC_TXMU_ACKPOLICY_EN_8730A;
	rtw_write8(adapter, REG_MU_BF_OPTION_8730A, v8);
	/* MU-MIMO Control as default value */
	rtw_write16(adapter, REG_WMAC_MU_BF_CTL_8730A, 0);

	/* Set MU NDPA rate & BW source */
	/* 0x42C[30] = 1 (0: from Tx desc, 1: from 0x45F) */
	v8 = rtw_read8(adapter, REG_TXBF_CTRL_8730A + 3);
	v8 |= (BIT_NDPA_PARA_8730A >> 24);
	rtw_write8(adapter, REG_TXBF_CTRL_8730A + 3, v8);
	/* 0x45F[7:0] = 0x10 (Rate=OFDM_6M, BW20) */
	rtw_write8(adapter, REG_NDPA_OPT_CTRL_8730A, 0x10);
...
void rtl8730a_phy_bf_enter(PADAPTER adapter, struct sta_info *sta)
{
	struct beamforming_info *info;
...
void rtl8730a_phy_bf_leave(PADAPTER adapter, u8 *addr)
{
	struct beamforming_info *info;
	struct beamformer_entry *bfer;
	struct beamformee_entry *bfee;
	...
	if (bfer) {
		...
		rtw_write8(adapter, REG_SND_PTCL_CTRL_8730A, 0xD8);
...
void rtl8730a_phy_bf_set_gid_table(PADAPTER adapter,
...
#if 0
void rtl8730a_phy_bf_set_csi_report(PADAPTER adapter, struct _RT_CSI_INFO *csi)
{
	PHAL_DATA_TYPE hal;
	struct beamforming_info *info;
	...
	if (IS_A_CUT(hal->version_id) || IS_B_CUT(hal->version_id) || IS_C_CUT(hal->version_id)) {
		...
		phydm_8814b_sutxbfer_workaroud(
			GET_PDM_ODM(adapter),
			enable_su,
			csi->Nc,
			csi->Nr,
...
void rtl8730a_phy_bf_sounding_status(PADAPTER adapter, u8 status)
{
	struct beamforming_info	*info;
	...
	if (sounding->state == SOUNDING_STATE_SU_SOUNDDOWN) {
		...
		val16 = rtw_read16(adapter, REG_TXBF_CTRL_8730A);
		val16 |= BIT_DIS_NDP_BFEN_8730A;
		rtw_write16(adapter, REG_TXBF_CTRL_8730A, val16);
	} else if (sounding->state == SOUNDING_STATE_MU_SOUNDDOWN) {
		/* MU sounding done */
		RTW_INFO("%s: Config MU sound down HW settings\n", __FUNCTION__);

		val32 = rtw_read32(adapter, REG_WMAC_ASSOCIATED_MU_BFMEE2_8730A);
		is_sounding_success[0] = (val32 & BIT_STATUS_BFEE2_8730A) ? 1 : 0;
		is_sounding_success[1] = ((val32 >> 16) & BIT_STATUS_BFEE3_8730A) ? 1 : 0;
		val32 = rtw_read32(adapter, REG_WMAC_ASSOCIATED_MU_BFMEE4_8730A);
		is_sounding_success[2] = (val32 & BIT_STATUS_BFEE4_8730A) ? 1 : 0;
		is_sounding_success[3] = ((val32 >> 16) & BIT_BIT_STATUS_BFEE5_8730A) ? 1 : 0;
		val32 = rtw_read32(adapter, REG_WMAC_ASSOCIATED_MU_BFMEE6_8730A);
		is_sounding_success[4] = (val32 & BIT_STATUS_BFEE6_8730A) ? 1 : 0;
		is_sounding_success[5] = ((val32 >> 16) & BIT_STATUS_BFEE7_8730A) ? 1 : 0;
...
void rtw_lps_pwr_tracking(_adapter *adapter, u8 thermal_value)
{
	...
	rtl8730a_set_fw_thermal_rpt_cmd(adapter, _TRUE, thermal_value);
}
#endif
