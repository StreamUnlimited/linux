	       ...
	       struct iw_point *wrqu, char *extra)
{
	...
	if (bStartRx) {
		...
		sprintf(extra, "Received packet OK:%d CRC error:%d ,Filter out:%d", padapter->mppriv.rx_pktcount, padapter->mppriv.rx_crcerrpktcount,
		...
		sprintf(extra, "Phy Received packet OK:%d CRC error:%d FA Counter: %d", rx_counter.rx_pkt_ok, rx_counter.rx_pkt_crc_error,
		   ...
		   struct iw_point *wrqu, char *extra)
{
	...
	ther_path_addr[0] = EEPROM_THERMAL_METER_8730A;
#endif
		...
		union iwreq_data *wrqu, char *extra)
{
	...
	|| defined(CONFIG_RTL8821C) || defined(CONFIG_RTL8822C) || defined(CONFIG_RTL8730A)
	if (copy_from_user(extra, wrqu->data.pointer, wrqu->data.length)) {
		return -EFAULT;
	}
	*(extra + wrqu->data.length) = '\0';
