...
#else
uint rtw_drv_log_level = _DRV_MAX_;
#endif
...
int rtw_os_ndev_alloc(_adapter *adapter)
{
	...
	ndev->irq = adapter_to_dvobj(adapter)->irq_dma;
#endif
...
#endif /* LINUX_VERSION_CODE >= 3.7.8 */
#endif
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 19, 0)) && defined(CONFIG_AXI_HCI)
...
#if defined(CONFIG_SDIO_HCI) || defined(CONFIG_AXI_HCI)
