...
int proc_get_ap_info(struct seq_file *m, void *v)
{
	...
	if (psta) {
		...
		RTW_PRINT_SEL(m, "sta's macaddr:" MAC_FMT "\n", MAC_ARG(psta->phl_sta->mac_addr));
		RTW_PRINT_SEL(m, "cur_channel=%d, cur_bwmode=%d(%s), cur_ch_offset=%d\n", pmlmeext->chandef.chan, pmlmeext->chandef.bw, ch_width_str(pmlmeext->chandef.bw),
...
int proc_get_tx_ring(struct seq_file *m, void *v)
{
	...
	_rtw_spinlock_irq(&dvobj_to_axi(pdvobjpriv)->irq_th_lock, &sp_flags);
	...
	_rtw_spinunlock_irq(&dvobj_to_axi(pdvobjpriv)->irq_th_lock, &sp_flags);
