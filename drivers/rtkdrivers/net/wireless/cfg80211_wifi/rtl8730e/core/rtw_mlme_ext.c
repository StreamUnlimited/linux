...
unsigned int send_beacon(_adapter *padapter)
{
	...
	return _SUCCESS;
#else
#if (defined(CONFIG_PCI_HCI) || defined(CONFIG_AXI_HCI)) && !defined(CONFIG_BCN_POLLING)
	...
	/* CONFIG_BCN_POLLING is for pci interface beacon polling mode */
#if defined(CONFIG_BCN_POLLING)
	u8 bxmitok = _FALSE;
	int issue = 0;
	int poll = 0;
	systime start = rtw_get_current_time();
...
#endif /*(defined(CONFIG_PCI_HCI) || defined(CONFIG_AXI_HCI)) && !defined(CONFIG_BCN_POLLING)*/
#endif /* RTW_PHL_BCN */
}
