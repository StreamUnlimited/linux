...
void rtw_hw_intf_cfg(struct dvobj_priv *dvobj, struct hal_com_t *hal_com)
{
	...
	if (dvobj->interface_type == RTW_HCI_AXI) {
		PPCI_DATA axi = dvobj_to_axi(dvobj);
		//hci_info.
	}
#endif
...
#ifdef CONFIG_AXI_HCI
	if (dvobj->interface_type == RTW_HCI_AXI) {
		PAXI_DATA axi = dvobj_to_axi(dvobj);
...
#ifdef CONFIG_AXI_HCI
	//intr init flag
	dvobj_to_axi(dvobj)->irq_enabled = 1;
#endif
#ifdef CONFIG_CMD_GENERAL
...
#ifdef CONFIG_AXI_HCI
	//intr init flag
	dvobj_to_axi(dvobj)->irq_enabled = 0;
#endif
