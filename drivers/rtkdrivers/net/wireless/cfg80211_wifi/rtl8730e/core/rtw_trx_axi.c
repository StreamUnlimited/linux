...
/********************************xmit section*****************************/
static void axi_xmit_tasklet(_adapter *padapter)
{
...
s32 axi_init_xmit_priv(_adapter *adapter)
{
	s32 ret = _SUCCESS;
	struct xmit_priv *pxmitpriv = &adapter->xmitpriv;
	struct dvobj_priv *dvobj = adapter_to_dvobj(adapter);

	_rtw_spinlock_init(&dvobj_to_axi(dvobj)->irq_th_lock);

	rtw_tasklet_init(&pxmitpriv->xmit_tasklet,
			 (void(*)(unsigned long))axi_xmit_tasklet,
			 (unsigned long)adapter);

	return ret;
}

void axi_free_xmit_priv(_adapter *adapter)
{
	struct dvobj_priv *dvobj = adapter_to_dvobj(adapter);

	_rtw_spinlock_free(&dvobj_to_axi(dvobj)->irq_th_lock);
}

static s32 axi_xmit_direct(_adapter *adapter, struct xmit_frame *pxmitframe)
{
	s32 res = _SUCCESS;
#if 0
#ifdef CONFIG_XMIT_THREAD_MODE
	...
	if (res == _SUCCESS) {
		...
		enqueue_pending_xmitbuf(pxmitpriv, pxmitframe->pxmitbuf);
#else
		axi_dump_xframe(adapter, pxmitframe);
#endif
	}
#endif
	return res;
...
static s32 axi_data_xmit(_adapter *adapter, struct xmit_frame *pxmitframe)
{
	s32 res;
	...
	if (rtw_hal_axi_check_enough_txdesc(GET_PHL_COM(dvobj), pattrib->qsel) == _FALSE) {
		goto enqueue;
	}
	...
	_rtw_spinunlock_bh(&pxmitpriv->lock);
#endif
	if (axi_xmit_direct(adapter, pxmitframe) != _SUCCESS) {
#if 0 /*CONFIG_CORE_XMITBUF*/
		rtw_free_xmitbuf(pxmitpriv, pxmitbuf);
#endif
		rtw_free_xmitframe(pxmitpriv, pxmitframe);
 ...
 */
s32 axi_xmit_buf_handler(_adapter *adapter)
{
	struct dvobj_priv *dvobj = adapter_to_dvobj(adapter);
	...
	do {
		...
		if (rtw_hal_axi_check_enough_txdesc(GET_PHL_COM(dvobj),
...
#endif
s32 axi_xmitframe_enqueue(_adapter *adapter,
			  struct xmit_frame *pxmitframe)
{
	struct dvobj_priv *dvobj = adapter_to_dvobj(adapter);
	struct xmit_priv *pxmitpriv = &adapter->xmitpriv;
...
/******************************** recv section*******************************/
s32 axi_init_recv_priv(struct dvobj_priv *dvobj)
{
	s32 ret = _SUCCESS;
...
void axi_free_recv_priv(struct dvobj_priv *dvobj)
{

}


struct rtw_intf_ops axi_ops = {
	.init_xmit_priv = axi_init_xmit_priv,
	.free_xmit_priv = axi_free_xmit_priv,
	.data_xmit	= axi_data_xmit,
	.xmitframe_enqueue = axi_xmitframe_enqueue,
#if 0 /*def CONFIG_XMIT_THREAD_MODE*/
	.xmit_buf_handler = axi_xmit_buf_handler
#endif

	.init_recv_priv = axi_init_recv_priv,
	.free_recv_priv = axi_free_recv_priv,
};
