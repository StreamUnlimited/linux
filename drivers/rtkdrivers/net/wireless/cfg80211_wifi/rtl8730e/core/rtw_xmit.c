...
static void set_qos(struct sk_buff *pkt, struct pkt_attrib *pattrib)
{
	...
	if (pattrib->ether_type == ETH_P_IP) {
		...
		_rtw_pktfile_read(&ppktfile, (u8 *)&ipv6_hdr, sizeof(struct ipv6hdr));
		UserPriority = ipv6_hdr.priority;
...
void core_recycle_txreq_phyaddr(_adapter *padapter, struct rtw_xmit_req *txreq)
{
	...
	PAXI_DATA axi_data = dvobj_to_axi(padapter->dvobj);
...
#endif
#ifdef CONFIG_AXI_HCI
		axi_unmap_bus_addr(&pdev->dev, &phy_addr, pkt_list->length, DMA_TO_DEVICE);
#else
...
#ifdef CONFIG_AXI_HCI
			axi_get_bus_addr(&pdev->dev, pkt_list->vir_addr, &phy_addr, pkt_list->length, DMA_TO_DEVICE);
#else
...
void fill_txreq_mdata(_adapter *padapter, struct xmit_frame *pxframe)
{
	...
	if (pxframe->xftype == RTW_TX_DRV_MGMT) {
		mdata->userate_sel = 1;
		mdata->mbssid = pxframe->attrib.mbssid & 0xF;

		if (pxframe->attrib.rate) {
			mdata->f_rate = _rate_mrate2phl(pxframe->attrib.rate);
		} else {
			mdata->f_rate = _rate_mrate2phl(pmlmeext->tx_rate);
		}

		mdata->rty_use_des = 1;

		if (pxframe->attrib.retry_ctrl == _TRUE) {
			mdata->retry_limit = 6;
		} else {
			mdata->retry_limit = 12;
		}
	} else {
		...
		if (mdata->mc) {
			if ((pxframe->attrib.encrypt > 0) && (!pxframe->attrib.bswenc)
			    && (pxframe->attrib.bmc_camid != INVALID_SEC_MAC_CAM_ID)) {
				mdata->en_desc_id = 1;
				mdata->macid = pxframe->attrib.bmc_camid;
			}
		}
	}
	mdata->f_er = 0;
	...
	if (pxframe->ack_report) {
		...
		mdata->sw_define_l = mdata->sw_define & 0x7F;
		mdata->sw_define_h = (mdata->sw_define >> 7) & 0xF;
