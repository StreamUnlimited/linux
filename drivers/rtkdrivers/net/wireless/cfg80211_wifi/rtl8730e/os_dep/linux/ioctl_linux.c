			 ...
			 union iwreq_data *wrqu, char *extra)
{
	...
	case 4:
		...
		break;
	case 21:
		data32 = rtw_phl_sys_read8(dvobj->phl, 0x42008000, addr);
		sprintf(extra, "0x%02X", data32);
		break;
	case 22:
		data32 = rtw_phl_sys_read16(dvobj->phl, 0x42008000, addr);
		sprintf(extra, "0x%04X", data32);
		break;
	case 24:
		data32 = rtw_phl_sys_read32(dvobj->phl, 0x42008000, addr);
		sprintf(extra, "0x%08X", data32);
		break;
	default:
			  ...
			  union iwreq_data *wrqu, char *extra)
{
	...
	case 4:
		...
		break;
	case 21:
		rtw_phl_sys_write8(dvobj->phl, 0x42008000, addr, (u8)data32);
		RTW_INFO("%s: addr=0x%08X data=0x%02X\n", __func__, 0x42008000 + addr, (u8)data32);
		break;
	case 22:
		rtw_phl_sys_write16(dvobj->phl, 0x42008000, addr, (u16)data32);
		RTW_INFO("%s: addr=0x%08X data=0x%04X\n", __func__, 0x42008000 +addr, (u16)data32);
		break;
	case 24:
		rtw_phl_sys_write32(dvobj->phl, 0x42008000, addr, data32);
		RTW_INFO("%s: addr=0x%08X data=0x%08X\n", __func__, 0x42008000 + addr, data32);
		break;
	default:
