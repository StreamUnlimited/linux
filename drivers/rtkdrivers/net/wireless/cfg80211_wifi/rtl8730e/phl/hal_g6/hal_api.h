...
/**
 * rtw_hal_trxring_num_query - query ring number with specifc dma channel
 * @hal: see struct hal_info_t
 * @dma_ch: the target dma channel
 *
 * this function returns the number of available trx resource
 * NOTE, input host_idx and hw_idx ptr shall NOT be NULL
 */
int rtw_hal_trxring_num_query(void *hal, u8 dma_ch);
...
enum rtw_hal_status rtw_hal_trigger_bcn(void *hal, u32 port);
#endif
...
enum rtw_hal_status
rtw_hal_rf_set_ch_bw(struct rtw_hal_com_t *hal_com, enum phl_phy_idx phy, u8 center_ch,
