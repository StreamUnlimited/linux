...
hal_mac_set_chip_id(struct rtw_hal_com_t *hal_com,
	...
	case MAC_AX_CHIP_ID_8730E:
		hal_com->chip_id = CHIP_WIFI6_8730E;
		break;
	default:
...
#ifdef CONFIG_AXI_HCI
static u8 hal_sys_reg_r8(void *h, u32 base, u32 addr)
{
	return hal_sys_read8((struct rtw_hal_com_t *)h, base, addr);
}
static u16 hal_sys_reg_r16(void *h, u32 base, u32 addr)
{
	return hal_sys_read16((struct rtw_hal_com_t *)h, base, addr);
}
static u32 hal_sys_reg_r32(void *h, u32 base, u32 addr)
{
	return hal_sys_read32((struct rtw_hal_com_t *)h, base, addr);
}
static void hal_sys_reg_w8(void *h, u32 base, u32 addr, u8 val)
{
	hal_sys_write8((struct rtw_hal_com_t *)h, base, addr, val);
}
static void hal_sys_reg_w16(void *h, u32 base, u32 addr, u16 val)
{
	hal_sys_write16((struct rtw_hal_com_t *)h, base, addr, val);
}
static void hal_sys_reg_w32(void *h, u32 base, u32 addr, u32 val)
{
	hal_sys_write32((struct rtw_hal_com_t *)h, base, addr, val);
}
#endif
...
#ifdef CONFIG_AXI_HCI
	rtw_plt_cb.sys_reg_r8 = hal_sys_reg_r8;
	rtw_plt_cb.sys_reg_r16 = hal_sys_reg_r16;
	rtw_plt_cb.sys_reg_r32 = hal_sys_reg_r32;
	rtw_plt_cb.sys_reg_w8 = hal_sys_reg_w8;
	rtw_plt_cb.sys_reg_w16 = hal_sys_reg_w16;
	rtw_plt_cb.sys_reg_w32 = hal_sys_reg_w32;
#endif
		     ...
		     struct hal_info_t *hal_info)
{
...
#else
	{
		...
		} else if (phl_com->hci_type ==  RTW_HCI_AXI) {
			intf = MAC_AX_INTF_AXI;
...
#ifdef CONFIG_AXI_HCI
u32 rtw_hal_mac_send_h2c_ameba(struct rtw_hal_com_t *hal_com, u8 id,
			       u32 buf_len, u8 *pbuf)
{
	struct hal_info_t *hal = hal_com->hal_priv;
	struct mac_ax_adapter *adapter = hal_to_mac(hal);
	struct dvobj_priv *dvobj = (struct dvobj_priv *)(hal_com->drv_priv);
	struct mac_ax_ops *ops = adapter->ops;
	u8 h2c[FWCMD_H2C_MAX_SIZE] = {0};
	u32 mac_err = 0;
	...
	if (buf_len > (FWCMD_H2C_MAX_SIZE - 1)) {
		return RTW_HAL_STATUS_FAILURE;
	}
	...
	h2c[0] = id;
	PLTFM_MEMCPY((u8 *)(h2c + 1), pbuf, buf_len);

	mac_err = ops->send_h2c_hmebox(adapter, h2c);
				     ...
				     bool is_connect)
{
	...
	mac->ops->cfg_macid(mac, sta);
}

enum rtw_hal_status
rtw_hal_mac_cfg_macid(void *hal,
		      struct rtw_phl_stainfo_t *sta)
{
	struct hal_info_t *hal_info = (struct hal_info_t *)hal;
	struct mac_ax_adapter *mac = hal_to_mac(hal_info);

	if (mac->ops->cfg_macid(mac, sta) == MACSUCCESS) {
		return RTW_HAL_STATUS_SUCCESS;
	}
...
#else
	mac_status = mac->ops->hal_init(mac, &init_info->trx_info, NULL, &init_info->intf_info);
#endif
...
#else
	mac_status = mac->ops->hal_fast_init(mac, &init_info->trx_info, NULL, &init_info->intf_info);
#endif
	if (mac_status == MACSUCCESS)
	{
		hstatus = RTW_HAL_STATUS_SUCCESS;
...
rtw_hal_mac_cfg_keep_alive(struct hal_info_t *hal_info, u16 macid, u8 en,
	...
	if (en)
	{
		...
		info.packet_type = cfg->keep_pkt_type;
	}
...
static bool _wow_is_txq_empty(struct mac_ax_tx_queue_empty *val)
{
	...
	for (i = 0; i < size; i++) {
		if (val->macid_txq_empty[i] != 1) {
			return false;
		}
	}

	PHL_INFO("%s : others_empty %d.\n", __func__, val->others_empty);
	PHL_INFO("%s : mgnt_empty %d.\n", __func__, val->mgnt_empty);
	for (i = 0; i < size; i++) {
...
enum rtw_hal_status rtw_hal_mac_wow_wde_drop(struct hal_info_t *hal, u8 band)
{
	...
	info.sel = MAC_AX_PKT_DROP_SEL_MACID_ALL;
	info.band = band;
...
#else
enum rtw_hal_status
rtw_hal_mac_enable_fw(struct hal_info_t *hal_info, u8 enable) {
	struct mac_ax_adapter *mac = hal_to_mac(hal_info);
	u32 mac_err;

	mac_err = mac->ops->enable_fw(mac, enable);
	if (mac_err != MACSUCCESS)
	{
		PHL_ERR("%s : mac status %d.\n", __func__, mac_err);
		return RTW_HAL_STATUS_FAILURE;
...
rtw_hal_mac_ax_deinit_bfee(struct rtw_hal_com_t *hal_com, u8 band) {
	...
	hal_status = mac_info->ops->deinit_mee(mac_info);
...
rtw_hal_mac_ax_bfee_set_csi_rrsc(void *mac, u8 band, u32 rrsc) {
	...
	hal_status = mac_info->ops->csi_rrsc(mac_info, rrsc);
...
rtw_hal_mac_ax_bfee_forced_csi_rate(void *mac, struct rtw_phl_stainfo_t *sta,
	...
	hal_status = mac_info->ops->csi_force_rate(mac_info, ht_rate, vht_rate,
			he_rate);
...
rtw_hal_mac_ax_set_bf_entry(void *mac, u8 band,
	...
	hal_status = mac_info->ops->set_csi_buffer_index(mac_info, buffer_idx);
				 ...
				 struct mac_ax_ppdu_rpt *mac_ppdu, void *rx_mdata)
{
	...
	for (i = 0; i < mac_ppdu->usr_num; i++) {
		if (mdata->macid_rxdesc != 0x7F) {
			...
			mac_ppdu->usr[i].macid = mdata->macid_rxdesc;
			mac_ppdu->usr[i].has_data = (mdata->frame_type == RTW_FRAME_TYPE_DATA);
			mac_ppdu->usr[i].has_ctrl = (mdata->frame_type == RTW_FRAME_TYPE_CTRL);
			mac_ppdu->usr[i].has_mgnt = (mdata->frame_type == RTW_FRAME_TYPE_MGNT);
			mac_ppdu->usr[i].has_bcn = (mac_ppdu->usr[i].has_mgnt) && (mdata->bc);
		} else {
...
rtw_hal_mac_ppdu_stat_cfg(struct hal_info_t *hal_info,
	...
	if (ppdu_stat_en) {
		if (appen_info & HAL_PPDU_PHYSTS) {
			cfg.sts_append_en = 1;
		} else {
			cfg.sts_append_en = 0;
		}

		if (appen_info & HAL_PHYSTS_PER_SUBMPDU) {
			cfg.sts_append_mpdu = 1;
		} else {
			cfg.sts_append_mpdu = 0;
		}
	} else {
		cfg.sts_append_en = 0;
		cfg.sts_append_mpdu = 0;
		...
		struct pkt_ofld_entry *entry)
{
	...
	mac_ids.arp_rsp = entry->pkt_info[PKT_TYPE_ARP_RSP].id;

	PHL_PRINT("probersp %d, pspoll %d, nulldata %d, qosnull %d,"
		  " btqosnull %d, cts2self %d, arprsp %d.\n",
		  mac_ids.probersp,
		  mac_ids.pspoll,
		  mac_ids.nulldata,
		  mac_ids.qosnull,
...
rtw_hal_mac_read_log_efuse_map(struct rtw_hal_com_t *hal_com, u8 *map,
	...
	if (mac->ops->dump_log_efuse(mac, map) != MACSUCCESS)
	{
...
rtw_hal_mac_read_phy_efuse(struct rtw_hal_com_t *hal_com,
	...
	if (mac->ops->read_efuse(mac, addr, size, value) != MACSUCCESS)
	{
		return RTW_HAL_STATUS_FAILURE;
	}
	return RTW_HAL_STATUS_SUCCESS;
	...
	struct rtw_hal_com_t *hal_com, u8 band, u32 offset, u32 mask, u32 val)
{
	...
	result = ops->write_msk_pwr_reg(mac, offset, mask, val);
...
u32 rtw_hal_mac_set_pwr_reg(struct rtw_hal_com_t *hal_com, u8 band, u32 offset, u32 val)
{
	...
	result = ops->write_pwr_reg(mac, offset, val);
...
u32 rtw_hal_mac_get_pwr_reg(struct rtw_hal_com_t *hal_com, u8 band, const u32 offset, u32 *val)
{
	struct hal_info_t *hal = hal_com->hal_priv;
	struct mac_ax_adapter *mac = hal_to_mac(hal);
	struct mac_ax_ops *ops = mac->ops;
	...
	result = ops->read_pwr_reg(mac, offset, val);
...
rtw_hal_mac_ps_set_32k(struct hal_info_t *hal_info, bool en_32k, bool en_ack) {
	...
	if (mac->ops->ps_set_32k(mac, en_32k, en_ack) == MACSUCCESS)
	{
		return RTW_HAL_STATUS_SUCCESS;
	} else
	{
...
rtw_hal_mac_set_sch_tx_en(struct rtw_hal_com_t *hal_com, u8 band_idx,
	...
	cfg.tx_en.be = ((tx_en & RTW_TXEN_BE) ? 1 : 0);
	cfg.tx_en.bk = ((tx_en & RTW_TXEN_BK) ? 1 : 0);
	cfg.tx_en.vi = ((tx_en & RTW_TXEN_VI) ? 1 : 0);
	cfg.tx_en.vo = ((tx_en & RTW_TXEN_VO) ? 1 : 0);
	cfg.tx_en.mgnt = ((tx_en & RTW_TXEN_MG) ? 1 : 0);
	cfg.tx_en.hiq = ((tx_en & RTW_TXEN_HI) ? 1 : 0);
	cfg.tx_en.bcnq = ((tx_en & RTW_TXEN_BCN) ? 1 : 0);
	cfg.tx_en.cpumgnt = ((tx_en & RTW_TXEN_CPUMG) ? 1 : 0);

	cfg.tx_en_mask.be = ((tx_en_mask & RTW_TXEN_BE) ? 1 : 0);
	cfg.tx_en_mask.bk = ((tx_en_mask & RTW_TXEN_BK) ? 1 : 0);
	cfg.tx_en_mask.vi = ((tx_en_mask & RTW_TXEN_VI) ? 1 : 0);
	cfg.tx_en_mask.vo = ((tx_en_mask & RTW_TXEN_VO) ? 1 : 0);
	cfg.tx_en_mask.mgnt = ((tx_en_mask & RTW_TXEN_MG) ? 1 : 0);
	cfg.tx_en_mask.hiq = ((tx_en_mask & RTW_TXEN_HI) ? 1 : 0);
	cfg.tx_en_mask.bcnq = ((tx_en_mask & RTW_TXEN_BCN) ? 1 : 0);
	cfg.tx_en_mask.cpumgnt = ((tx_en & RTW_TXEN_CPUMG) ? 1 : 0);
...
rtw_hal_mac_set_rxfltr_by_mode(struct rtw_hal_com_t *hal_com,
	...
	err = mac->ops->set_rx_fltr_opt(mac, &ctrl, &mask);
	if (err)
	{
		return RTW_HAL_STATUS_FAILURE;
	}
	...
	struct rtw_hal_com_t *hal_com, u8 band, u8 type, u8 target)
{
	...
	if (target > MAC_AX_FWD_ACPT) {
		return RTW_HAL_STATUS_FAILURE;
	}
	fwd = (enum mac_ax_fwd_target)target;

	err = mac->ops->set_rx_fltr_typ_opt(mac, ftype, fwd);
	if (err) {
		return RTW_HAL_STATUS_FAILURE;
	}
	...
	u8 type, u8 subtype, u8 target)
{
	...
	err = mac->ops->set_rx_fltr_typstyp_opt(mac, ftype, subtype, fwd);
	if (err) {
		return RTW_HAL_STATUS_FAILURE;
	}
	return RTW_HAL_STATUS_SUCCESS;
...
rtw_hal_mac_is_tx_mgnt_empty(struct hal_info_t *hal_info, u8 band, u8 *st) {
	...
	if (q_st.mgnt_empty == DLE_QUEUE_EMPTY)
	{
		*st = 1;
	} else
	{
...
rtw_hal_mac_write_pwr_ofst_mode(struct rtw_hal_com_t *hal_com, u8 band) {
	...
	mac->ops->write_pwr_ofst_mode(mac, tpu);
...
rtw_hal_mac_write_pwr_ofst_bw(struct rtw_hal_com_t *hal_com, u8 band) {
	...
	mac->ops->write_pwr_ofst_bw(mac, tpu);
...
rtw_hal_mac_write_pwr_ref_reg(struct rtw_hal_com_t *hal_com, u8 band) {
	...
	mac->ops->write_pwr_ref_reg(mac, tpu);
...
rtw_hal_mac_write_pwr_limit_en(struct rtw_hal_com_t *hal_com, u8 band) {
	...
	mac->ops->write_pwr_limit_en(mac, tpu);
...
rtw_hal_mac_write_pwr_limit_rua_reg(struct rtw_hal_com_t *hal_com, u8 band) {
	...
	mac->ops->write_pwr_limit_rua_reg(mac, tpu);
...
rtw_hal_mac_write_pwr_limit_reg(struct rtw_hal_com_t *hal_com, u8 band) {
	...
	mac->ops->write_pwr_limit_reg(mac, lmt);
...
rtw_hal_mac_write_pwr_by_rate_reg(struct rtw_hal_com_t *hal_com, u8 band) {
	...
	mac->ops->write_pwr_by_rate_reg(mac, by_rate);
				  ...
				  struct mac_ax_mcc_start *info)
{
	...
	PHL_TRACE(COMP_PHL_MCC, _PHL_INFO_,
		  "_hal_mac_mcc_fill_start_info(): info->group(%d), btc_in_group(%d), old_group_action(%d), old_group(%d), macid(%d), Tsf(0x%08x %08x)\n",
