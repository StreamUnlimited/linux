...
rtw_hal_reset(struct rtw_hal_com_t *hal_com, enum phl_phy_idx phy_idx, u8 band_idx, bool reset) {
	...
	if (reset)
	{
		...
		rtw_hal_acpt_crc_err_pkt(hal_info, band_idx, false);
...
enum rtw_hal_status rtw_hal_ppdu_sts_cfg(void *hal, u8 band_idx, bool en)
{
	...
	if (en) {
		...
		if (hal_com->band[band_idx].ppdu_sts_filter | HAL_PPDU_HAS_CRC_OK) {
			rtw_hal_acpt_crc_err_pkt(hal_info, HW_BAND_0, true);
		}
