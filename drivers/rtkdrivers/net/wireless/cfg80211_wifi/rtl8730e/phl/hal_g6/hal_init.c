...
u8 rtw_hal_sys_read8(void *h, u32 base, u32 addr)
{
	struct hal_info_t *hal_info = (struct hal_info_t *)h;
	struct rtw_hal_com_t *hal_com = hal_info->hal_com;

	return hal_sys_read8(hal_com, base, addr);
}
u16 rtw_hal_sys_read16(void *h, u32 base, u32 addr)
{
	struct hal_info_t *hal_info = (struct hal_info_t *)h;
	struct rtw_hal_com_t *hal_com = hal_info->hal_com;

	return hal_sys_read16(hal_com, base, addr);
}
u32 rtw_hal_sys_read32(void *h, u32 base, u32 addr)
{
	struct hal_info_t *hal_info = (struct hal_info_t *)h;
	struct rtw_hal_com_t *hal_com = hal_info->hal_com;

	return hal_sys_read32(hal_com, base, addr);
}
void rtw_hal_sys_write8(void *h, u32 base, u32 addr, u8 val)
{
	struct hal_info_t *hal_info = (struct hal_info_t *)h;
	struct rtw_hal_com_t *hal_com = hal_info->hal_com;

	hal_sys_write8(hal_com, base, addr, val);
}
void rtw_hal_sys_write16(void *h, u32 base, u32 addr, u16 val)
{
	struct hal_info_t *hal_info = (struct hal_info_t *)h;
	struct rtw_hal_com_t *hal_com = hal_info->hal_com;

	hal_sys_write16(hal_com, base, addr, val);
}
void rtw_hal_sys_write32(void *h, u32 base, u32 addr, u32 val)
{
	struct hal_info_t *hal_info = (struct hal_info_t *)h;
	struct rtw_hal_com_t *hal_com = hal_info->hal_com;

	hal_sys_write32(hal_com, base, addr, val);
}
...
void rtw_hal_bb_reg_dump(void *sel, void *h)
{
	...
	for (i = 0x0; i < 0x500; i += 4) {
		if (j % 4 == 1) {
			RTW_PRINT_SEL(sel, "0x%04x", i);
		}
		_RTW_PRINT_SEL(sel, " 0x%08x ", hal_read32(hal, i | 0x10000));
		if ((j++) % 4 == 0) {
			_RTW_PRINT_SEL(sel, "\n");
		}
	}

	for (i = 0x600; i < 0xf00; i += 4) {
		if (j % 4 == 1) {
			RTW_PRINT_SEL(sel, "0x%04x", i);
		}
		_RTW_PRINT_SEL(sel, " 0x%08x ", hal_read32(hal, i | 0x10000));
...
void rtw_hal_rf_reg_dump(void *sel, void *h)
{
	...
	for (path = 0; path < path_nums; path++) {
		...
		for (i = 0; i < 0x100; i++) {
			value = rtw_hal_read_rfreg(hal_info, path, i, 0xffffffff);
			if (j % 4 == 1) {
 ...
 *
 * Return rtw_hal_mac_get_sec_cam's return value in enum rtw_hal_status type.
 */
enum rtw_hal_status
...
static enum rtw_hal_status hal_ops_check(struct hal_info_t *hal)
{
...
#endif /*defined(CONFIG_PCI_HCI) || defined(CONFIG_AXI_HCI) */
	...
	if (!trx_ops->query_trxring_num) {
		hal_error_msg("trx query_trxring_num");
		status = RTW_HAL_STATUS_FAILURE;
	}
	if (!trx_ops->query_tx_res) {
		hal_error_msg("trx query_tx_res");
	...
	if (!trx_ops->check_bcn_status) {
		hal_error_msg("trx check_bcn_status");
		status = RTW_HAL_STATUS_FAILURE;
	}
	if (!trx_ops->check_tx_done) {
		hal_error_msg("trx check_tx_done");
		status = RTW_HAL_STATUS_FAILURE;
	}
	if (!trx_ops->handle_rxbd_info) {
		hal_error_msg("trx handle_rxbd_info");
...
#ifdef CONFIG_AXI_HCI
	if (phl_get_hci_type(phl_com) == RTW_HCI_AXI) {
		hal_set_ops_axi(phl_com, hal_info);
	}
#endif
				 ...
				 struct rtw_phl_com_t *phl_com, void **hal, enum rtl_ic_id ic_id)
{
	...
	} else if (ic_id == RTL8730E) {
		chip_id = CHIP_WIFI6_8730E;
	} else {
...
#ifdef CONFIG_AXI_HCI
	if (phl_com->hci_type == RTW_HCI_AXI) {
		set_intf_ops = hal_axi_set_io_ops;
	}
#endif
