...
u8 _hal_sys_read8(struct rtw_hal_com_t *hal, u32 base, u32 addr)
{
	u8 r_val;
	struct hal_io_priv *io_priv = &hal->iopriv;
	u8(*_read8)(struct rtw_hal_com_t *hal, u32 base, u32 addr);

	_read8 = io_priv->io_ops._sys_read8;
	r_val = _read8(hal, base, addr);
	return r_val;
}

u16 _hal_sys_read16(struct rtw_hal_com_t *hal, u32 base, u32 addr)
{
	u16 r_val;
	struct hal_io_priv *io_priv = &hal->iopriv;
	u16(*_read16)(struct rtw_hal_com_t *hal, u32 base, u32 addr);

	_read16 = io_priv->io_ops._sys_read16;
	r_val = _read16(hal, base, addr);
	return r_val;
}

u32 _hal_sys_read32(struct rtw_hal_com_t *hal, u32 base, u32 addr)
{
	u32 r_val;
	struct hal_io_priv *io_priv = &hal->iopriv;
	u32(*_read32)(struct rtw_hal_com_t *hal, u32 base, u32 addr);

	_read32 = io_priv->io_ops._sys_read32;
	r_val = _read32(hal, base, addr);
	return r_val;
}

int _hal_sys_write8(struct rtw_hal_com_t *hal, u32 base, u32 addr, u8 val)
{
	struct hal_io_priv *io_priv = &hal->iopriv;
	int (*_write8)(struct rtw_hal_com_t *hal, u32 base, u32 addr, u8 val);
	int ret;

	_write8 = io_priv->io_ops._sys_write8;
	ret = _write8(hal, base, addr, val);

	return ret;
}
int _hal_sys_write16(struct rtw_hal_com_t *hal, u32 base, u32 addr, u16 val)
{
	struct hal_io_priv *io_priv = &hal->iopriv;
	int (*_write16)(struct rtw_hal_com_t *hal, u32 base, u32 addr, u16 val);
	int ret;

	_write16 = io_priv->io_ops._sys_write16;
	ret = _write16(hal, base, addr, val);

	return ret;
}
int _hal_sys_write32(struct rtw_hal_com_t *hal, u32 base, u32 addr, u32 val)
{
	struct hal_io_priv *io_priv = &hal->iopriv;
	int (*_write32)(struct rtw_hal_com_t *hal, u32 base, u32 addr, u32 val);
	int ret;

	_write32 = io_priv->io_ops._sys_write32;
	ret = _write32(hal, base, addr, val);
...
#ifdef CONFIG_AXI_HCI
u8 dbg_hal_sys_read8(struct rtw_hal_com_t *hal, u32 base, u32 addr, const char *caller, const int line)
{
	u8 val = _hal_sys_read8(hal, addr);
...
u16 dbg_hal_sys_read16(struct rtw_hal_com_t *hal, u32 base, u32 addr, const char *caller, const int line)
{
	u16 val = _hal_sys_read16(hal, base, addr);
...
u32 dbg_hal_sys_read32(struct rtw_hal_com_t *hal, u32 base, u32 addr, const char *caller, const int line)
{
	u32 val = _hal_sys_read32(hal, base, addr);
...
int dbg_hal_sys_write8(struct rtw_hal_com_t *hal, u32 base, u32 addr, u8 val, const char *caller, const int line)
{
	if (match_write_sniff(hal, addr, 1, val)) {
		PHL_INFO("DBG_IO %s:%d hal_write8(0x%04x, 0x%02x)\n"
			 , caller, line, addr, val);
	...
	return _hal_sys_write8(hal, base, addr, val);
}
int dbg_hal_sys_write16(struct rtw_hal_com_t *hal, u32 base, u32 addr, u16 val, const char *caller, const int line)
{
	if (match_write_sniff(hal, addr, 2, val)) {
		PHL_INFO("DBG_IO %s:%d hal_write16(0x%04x, 0x%04x)\n"
			 , caller, line, addr, val);
	...
	return _hal_sys_write16(hal, base, addr, val);
}
int dbg_hal_sys_write32(struct rtw_hal_com_t *hal, u32 base, u32 addr, u32 val, const char *caller, const int line)
{
	if (match_write_sniff(hal, addr, 4, val)) {
		PHL_INFO("DBG_IO %s:%d hal_write32(0x%04x, 0x%08x)\n"
			 , caller, line, addr, val);
	...
	return _hal_sys_write32(hal, base, addr, val);
}
#endif
#endif
