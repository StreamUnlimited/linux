	       ...
	       struct mac_ax_rx_cnt *rxcnt)
{
	...
	u16 *buf = rxcnt->buf;
	u8 rpt_sel_5, rpt_sel_4, rpt_sel_0_3;
	...
	for (i = 0; i < MAC_AX_RX_PPDU_MAX; i++) {
		...
		rpt_sel_5 = (idx[i] >> 5) & 0x1;
		rpt_sel_4 = (idx[i] >> 4) & 0x1;
		rpt_sel_0_3 = idx[i] & 0xF;
		val32 = MAC_REG_R32(REG_RXERR_RPT);
		val32 &= 0x09FFFFFF;
		val32 = BIT_SET_RXERR_RPT_SEL_V1_3_0(val32, rpt_sel_0_3);
		val32 |= (rpt_sel_5 ? BIT_RXERR_RPT_SEL_V1_5 : 0) |
			 (rpt_sel_4 ? BIT_RXERR_RPT_SEL_V1_4 : 0);
		MAC_REG_W32(REG_RXERR_RPT, val32);
		switch (rxcnt->op) {
		case MAC_AX_RXCNT_R:
			buf[i] = MAC_REG_R16(REG_RXERR_RPT);
			break;
		case MAC_AX_RXCNT_RST_ALL:
			MAC_REG_W16(REG_RXERR_RPT, 0);
			break;
		default:
			return MACNOITEM;
		}
...
u32 mac_get_mem_size(struct mac_ax_adapter *adapter, enum mac_ax_mem_sel sel)
{
	...
	switch (sel) {
	case MAC_AX_MEM_TX_BUF:
		size = TX_BUFFER_SIZE;
		break;
	case MAC_AX_MEM_RX_BUF:
		size = RX_BUFFER_SIZE;
		break;
	case MAC_AX_MEM_TXRPT_BUF:
		size = TXRPT_BUFFER_SIZE;
		break;
	case MAC_AX_MEM_IQ_BUF:
		size = IQ_DUMP_BUFFER_SIZE;
		break;
	case MAC_AX_MEM_PHYSTS_BUF:
		size = PHYSTS_BUFFER_SIZE;
		break;
	case MAC_AX_MEM_CHINFO_BUF:
		size = CHINFO_BUFFER_SIZE;
		break;
	case MAC_AX_MEM_DFS_BUF:
		size = DFS_BUFFER_SIZE;
		break;
	case MAC_AX_MEM_SECURITY_CAM:
		size = SECURITY_CAM_MEM_SIZE;
		break;
	case MAC_AX_MEM_MBSSID_CAM:
		size = MBSSID_CAM_MEM_SIZE;
		break;
	case MAC_AX_MEM_BA_CAM:
		size = BA_CAM_MEM_SIZE;
		break;
	default:
		size = 0;
		break;
...
u32 cam_aces_cfg(struct mac_ax_adapter *adapter,
		 enum mac_ax_mem_sel sel, u8 en)
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	u32 val32;
	...
	case MAC_AX_MEM_BA_CAM:
		if (en) {
			val32 = MAC_REG_R32(REG_BACAMCMD);
			val32 |= BIT_RXBACAM_DA_EN;
			MAC_REG_W32(REG_BACAMCMD, val32);
		} else {
			val32 = MAC_REG_R32(REG_BACAMCMD);
			val32 &= ~BIT_RXBACAM_DA_EN;
			MAC_REG_W32(REG_BACAMCMD, val32);
	...
	case MAC_AX_MEM_MBSSID_CAM:
		if (en) {
			val32 = MAC_REG_R32(REG_MBIDCAM_CFG);
			val32 |= BIT_MBIDCAM_DA_EN;
			MAC_REG_W32(REG_MBIDCAM_CFG, val32);
		} else {
			val32 = MAC_REG_R32(REG_MBIDCAM_CFG);
			val32 &= ~BIT_MBIDCAM_DA_EN;
			MAC_REG_W32(REG_MBIDCAM_CFG, val32);
...
static u32 __dump_mac_mem(struct mac_ax_adapter *adapter,
			  enum mac_ax_mem_sel sel, u32 strt_addr, u8 *buf, u32 len,
			  u32 dbg_path)
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	u32 cnt, val32;
	...
	case MAC_AX_MEM_RX_BUF:
		...
		break;
	case MAC_AX_MEM_TXRPT_BUF:
		base = TXRPT_BUFFER_SIZE;
		cam_cfg_flag = 0;
		break;
	case MAC_AX_MEM_IQ_BUF:
		base = IQ_DUMP_BUFFER_SIZE;
		cam_cfg_flag = 0;
		break;
	case MAC_AX_MEM_PHYSTS_BUF:
		base = PHYSTS_BUFFER_SIZE;
		cam_cfg_flag = 0;
		break;
	case MAC_AX_MEM_CHINFO_BUF:
		base = CHINFO_BUFFER_SIZE;
		cam_cfg_flag = 0;
		break;
	case MAC_AX_MEM_DFS_BUF:
		base = DFS_BUFFER_SIZE;
		cam_cfg_flag = 0;
		break;
	case MAC_AX_MEM_SECURITY_CAM:
		base = SECURITY_CAM_MEM_SIZE;
		cam_cfg_flag = 1;
		break;
	case MAC_AX_MEM_MBSSID_CAM:
		base = MBSSID_CAM_MEM_SIZE;
		cam_cfg_flag = 1;
		break;
	case MAC_AX_MEM_BA_CAM:
		base = BA_CAM_MEM_SIZE;
		cam_cfg_flag = 1;
		break;
	default:
		return MACNOITEM;
	...
	if (cam_cfg_flag) {
		ret = cam_aces_cfg(adapter, sel, MAC_AX_FUNC_EN);
		if (ret != MACSUCCESS) {
			PLTFM_MSG_ERR("[ERR]cam access en fail %d\n", ret);
			ret2 = cam_aces_cfg(adapter, sel, MAC_AX_FUNC_DIS);
			if (ret2 != MACSUCCESS)
				PLTFM_MSG_ERR("[ERR]cam access dis fail %d\n",
					      ret2);
			return ret;
		}
	}
	...
	while (cnt < len) {
		val32 = SYS_REG_R32(base, strt_addr + cnt);
		if (dbg_path) {
			*(u32 *)(buf + cnt) = val32;
		} else {
	...
	if (cam_cfg_flag) {
		ret = cam_aces_cfg(adapter, sel, MAC_AX_FUNC_DIS);
		if (ret != MACSUCCESS) {
			PLTFM_MSG_ERR("[ERR]cam access dis fail %d\n", ret);
		}
	}
...
#define REG_PAGE_SIZE 0x100
	...
	for (j = page; j < page + REG_PAGE_SIZE; j += 16) {
		val32_1 = SYS_REG_R32(base, j);
		val32_2 = SYS_REG_R32(base, j + 4);
		val32_3 = SYS_REG_R32(base, j + 8);
		val32_4 = SYS_REG_R32(base, j + 12);
			  ...
			  enum mac_ax_reg_sel sel)
{
	...
	case MAC_AX_REG_MAC:
		...
		for (i = srt; i <= end; i++) {
			page = i << 8;
			__dump_reg_range(adapter, MAC_BASE, page);
		}

		/*extend page, page11/12/14/15/16/17*/
		srt = MAC_EXT_PAGE_SRT;
		end = MAC_EXT_PAGE_END;
		for (i = srt; i <= end; i++) {
			if (i == 0x13) {
				continue;
			}
	...
	case MAC_AX_REG_BB:
		...
		for (i = srt; i <= end; i++) {
			page = i << 8;
			__dump_reg_range(adapter, BB_BASE, page);
		}
		...
		break;
	case MAC_AX_REG_IQK:
		/*0x4001_1b00~0x4001_1BFF*/
		srt = IQK_PAGE_SRT;
		end = IQK_PAGE_END;
		PLTFM_MSG_ALWAYS("dump IQK REG\n");
		__dump_reg_range(adapter, BB_BASE, IQK_PAGE_SRT << 8);
		PLTFM_MSG_ALWAYS("dump IQK REG all\n");
		break;
	case MAC_AX_REG_RFC:
		PLTFM_MSG_ALWAYS("dump RFC REG\n");
		...
		for (i = srt; i <= end; i++) {
			page = i << 8;
			__dump_reg_range(adapter, RF_BASE, page);
		}

		PLTFM_MSG_ALWAYS("dump RFC REG all\n");
		break;
	case MAC_AX_REG_A_RFC:
		//modify later
		break;
	default:
...
u32 mac_reg_dump(struct mac_ax_adapter *adapter,
		 enum mac_ax_reg_sel sel)
{
	u32 ret;
