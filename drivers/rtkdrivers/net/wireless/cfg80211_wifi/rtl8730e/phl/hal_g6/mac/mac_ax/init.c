...
#include "hw.h"
#include "mac_priv.h"
#include "mac/mac_ax/mac_8730e/init_8730e.h"
		...
		struct mac_ax_pltfm_cb *pltfm_cb)
{
	...
	case MAC_AX_CHIP_ID_8730E:
		adapter = get_mac_8730e_adapter(intf, cv, drv_adapter,
						pltfm_cb);
		break;
#endif
...
u32 hci_func_en(struct mac_ax_adapter *adapter)
{
	...
	val32 |= BIT_HCI_TXDMA_EN | BIT_HCI_RXDMA_EN;
	MAC_REG_W32(REG_CR, val32);
...
u32 mac_pre_sys_init(struct mac_ax_adapter *adapter)
{
	struct mac_ax_priv_ops *p_ops = adapter_to_priv_ops(adapter);
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	u32 ret = MACSUCCESS;
	...
	ret = adapter->ops->set_hw_value(adapter, MAC_AX_HW_EN_BB_RF, &enable_bb);

	/*0x4200827c[17]  1:test mode  0:normal mode*/
	if (SYS_REG_R32(SYSTEM_CTRL_BASE_LP, REG_SYSTEM_CFG0) & BIT(17)) {
...
u32 mac_sys_init(struct mac_ax_adapter *adapter)
{
	...
	value16 = MAC_REG_R16(REG_CR);
	value16 |= (BIT_TXDMA_EN | BIT_RXDMA_EN | BIT_PROTOCOL_EN
		    | BIT_SCHEDULE_EN | BIT_MACTXEN | BIT_MACRXEN
		    | BIT_MAC_SEC_EN);
	MAC_REG_W16(REG_CR, value16);
		 ...
		 struct mac_ax_intf_info *intf_info)
{
	...
	ret = mac_ops->pre_sys_init(adapter);
	if (ret != MACSUCCESS) {
		if (ret == MACMODEFAIL) {
			PLTFM_MSG_ERR("[WARN]In test mode %d\n", ret);
		} else {
	...
	ret = mac_ops->enable_fw(adapter, 1);
	if (ret != MACSUCCESS) {
	...
	mac_ops->init_pwr_reg(adapter, false, 0, NULL);
...
#if MAC_AX_AXI_SUPPORT
	ret = ops->enable_fw(adapter, 0);
	if (ret != MACSUCCESS) {
		PLTFM_MSG_ERR("[ERR]disable_fw %d\n", ret);
		goto end;
	}
#endif
