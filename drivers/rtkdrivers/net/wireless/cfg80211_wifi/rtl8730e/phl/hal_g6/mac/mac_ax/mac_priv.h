...
struct mac_ax_priv_ops {
	...
	u32(*init_trx_cfg)(struct mac_ax_adapter *adapter, enum halmac_trx_mode mode);
	u32(*init_protocol_cfg)(struct mac_ax_adapter *adapter);
	u32(*init_edca_cfg)(struct mac_ax_adapter *adapter);
	u32(*init_wmac_cfg)(struct mac_ax_adapter *adapter);
	u32(*enable_port)(struct mac_ax_adapter *adapter, u8 port, bool en);
	u32(*cfg_mac_addr)(struct mac_ax_adapter *adapter, u8 port,
			   union halmac_wlan_addr *addr);
	u32(*cfg_bssid)(struct mac_ax_adapter *adapter, u8 port,
			union halmac_wlan_addr *addr);
	u32(*cfg_transmitter_addr)(struct mac_ax_adapter *adapter, u8 port,
				   union halmac_wlan_addr *addr);
	...
	u32(*set_rrsr_cfg)(struct mac_ax_adapter *adapter,
			   struct mac_ax_rrsr_cfg *cfg);
	u32(*get_csi_rrsr_cfg)(struct mac_ax_adapter *adapter,
			       struct mac_ax_csi_rrsr_cfg *cfg);
	u32(*set_csi_rrsr_cfg)(struct mac_ax_adapter *adapter,
			       struct mac_ax_csi_rrsr_cfg *cfg);
...
#endif
#if MAC_AX_AXI_SUPPORT
	u32(*get_txbd_reg_axi)(struct mac_ax_adapter *adapter, u8 dma_ch, u32 *reg,
			       enum axi_bd_ctrl_type type);
	u32(*set_txbd_reg_axi)(struct mac_ax_adapter *adapter, u8 dma_ch,
			       enum axi_bd_ctrl_type type, u32 val0, u32 val1, u32 val2);
	u32(*get_rxbd_reg_axi)(struct mac_ax_adapter *adapter, u8 dma_ch, u32 *reg,
			       enum axi_bd_ctrl_type type);
	u32(*set_rxbd_reg_axi)(struct mac_ax_adapter *adapter, u8 dma_ch,
			       enum axi_bd_ctrl_type type, u32 val0, u32 val1, u32 val2);
	u32(*poll_dma_all_idle_axi)(struct mac_ax_adapter *adapter);
	u32(*clr_idx_ch_axi)(struct mac_ax_adapter *adapter,
			     struct mac_ax_txdma_ch_map *txch_map,
			     struct mac_ax_rxdma_ch_map *rxch_map);
	u32(*get_err_flag_axi)(struct mac_ax_adapter *adapter,
			       struct mac_ax_pcie_err_info *out_info);
	u32(*sync_trx_bd_idx)(struct mac_ax_adapter *adapter);
#ifdef RTW_WKARD_GET_PROCESSOR_ID
	u32(*chk_proc_long_ldy_axi)(struct mac_ax_adapter *adapter, u8 *val);
#endif
#endif
