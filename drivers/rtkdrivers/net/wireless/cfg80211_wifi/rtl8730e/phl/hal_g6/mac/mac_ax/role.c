...
u32 mac_set_slot_time(struct mac_ax_adapter *adapter, enum mac_ax_slot_time st)
{
	...
	switch (st) {
	case MAC_AX_SLOT_TIME_9US:
		MAC_REG_W8(REG_TSFT_SYN_OFFSET_SLOT + 3, 9);
		break;
	case MAC_AX_SLOT_TIME_20US:
		MAC_REG_W8(REG_TSFT_SYN_OFFSET_SLOT + 3, 20);
		break;
	default:
		ret = MACFUNCINPUT;
		break;
		     ...
		     struct mac_ax_role_info *info)
{
	...
	case MAC_AX_PORT_0:
		...
		bss_clr = BIT_SET_R_BSS_COLOR0(bss_clr, info->bss_color);
		MAC_REG_W32(REG_TRAN_BSSID0_H, bss_clr);
		break;
	case MAC_AX_PORT_1:
		bss_clr = MAC_REG_R32(REG_TRAN_BSSID1_H);
		bss_clr = BIT_SET_R_BSS_COLOR1(bss_clr, info->bss_color);
		MAC_REG_W32(REG_TRAN_BSSID1_H, bss_clr);
		break;
	default:
		break;
	}
			...
			struct mac_role_tbl *role)
{
	...
	struct mac_role_tbl_head *list = adapter->role_tbl;
...
u32 mac_add_role(struct mac_ax_adapter *adapter, struct mac_ax_role_info *info)
{
	...
	mac_en = check_mac_en(adapter, MAC_AX_MAC_SEL);
	if (mac_en != MACSUCCESS) {
		PLTFM_MSG_WARN("%s MAC not enable\n", __func__);
	}
			     ...
			     struct mac_ax_role_info *info)
{
	...
	sta.opmode = (info->opmode == MAC_AX_ROLE_CONNECT) ? 1 : 0;
	sta.band = info->band;
	...
	fwcmd_tbl.cmd_id = FWCMD_H2C_JOININFO_CMDID;
	fwcmd_tbl.data0 = (sta.opmode ? FWCMD_H2C_JOININFO_OPMODE : 0) |
