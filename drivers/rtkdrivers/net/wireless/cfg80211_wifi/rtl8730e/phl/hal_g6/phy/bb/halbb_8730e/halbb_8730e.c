...
void halbb_start_pmac_tx_8730e(struct bb_info *bb,
			       struct halbb_pmac_info *tx_info,
			     ...
			     enum phl_phy_idx phy_idx)
{
	...
	if (!tx_info->en_pmac_tx) {
		halbb_stop_pmac_tx_8730e(bb, tx_info, phy_idx);
		/* PD hit enable */
	...
	halbb_start_pmac_tx_8730e(bb, tx_info, tx_info->mode, tx_info->tx_cnt,
...
void halbb_set_tmac_tx_8730e(struct bb_info *bb, enum phl_phy_idx phy_idx)
{
	...
	// TMAC Tx path
	halbb_set_reg_cmn(bb, 0x09a4, 0xc, 0, phy_idx);
	// TMAC Tx power
				    ...
				    enum phl_phy_idx phy_idx)
{
	...
	*/
	...
	if (bound == 0) {
		halbb_set_reg_cmn(bb, SEG0R_PD_SPATIAL_REUSE_EN_C, SEG0R_PD_SPATIAL_REUSE_EN_C_M, 0, phy_idx);
	...
	BB_DBG(bb, DBG_PHY_CONFIG, "[PD Bound] bound=%d!\n", bound);
	if (bound < 40) {
		BB_DBG(bb, DBG_PHY_CONFIG,
	...
	BB_DBG(bb, DBG_PHY_CONFIG, "[PD Bound] bound_idx=%d!\n", bound_idx);
	halbb_set_reg_cmn(bb, SEG0R_PD_LOWER_BOUND_C, SEG0R_PD_LOWER_BOUND_C_M, bound_idx, phy_idx);
	halbb_set_reg_cmn(bb, SEG0R_PD_SPATIAL_REUSE_EN_C, SEG0R_PD_SPATIAL_REUSE_EN_C_M, 1, phy_idx);
					...
					enum phl_phy_idx phy_idx)
{
	...
	if (bound == 0) {
		halbb_set_reg_cmn(bb, CCA_RSSI_LMT_EN_C, CCA_RSSI_LMT_EN_C_M, 0, phy_idx);
		BB_DBG(bb, DBG_PHY_CONFIG,
		       "[PD Bound] Set Boundary to default!\n");
		return true;
	}
	...
	halbb_set_reg_cmn(bb, RSSI_NOCCA_LOW_TH_C, RSSI_NOCCA_LOW_TH_C_M, bound_tmp, phy_idx);
	halbb_set_reg_cmn(bb, RSSI_NOCCA_HIGH_TH_C, RSSI_NOCCA_HIGH_TH_C_M, 0x7f, phy_idx);
	halbb_set_reg_cmn(bb, CCA_RSSI_LMT_EN_C, CCA_RSSI_LMT_EN_C_M, 1, phy_idx);
...
u8 halbb_querry_pd_lower_bound_8730e(struct bb_info *bb, bool get_en_info, enum phl_phy_idx phy_idx)
{
	u8 tmp;
	...
	if (get_en_info) {
		tmp = (u8)halbb_get_reg_cmn(bb, SEG0R_PD_SPATIAL_REUSE_EN_C, SEG0R_PD_SPATIAL_REUSE_EN_C_M, phy_idx);
	} else {
