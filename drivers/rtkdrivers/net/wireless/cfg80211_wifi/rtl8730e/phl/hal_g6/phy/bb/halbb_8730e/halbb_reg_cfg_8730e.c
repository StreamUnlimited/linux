...
void halbb_cfg_bb_gain_8730e(struct bb_info *bb, u32 addr, u32 data)
{
	...
	if (addr == 0xfe) {
		...
		if (type == 0) {
			...
			gain->rxbb_gain[band_idx][path][0]  = (data >> (16)) & 0xff;
		}
	} else if (cfg_type == 1) { /*Pin for gain index*/
		type = (u8)(addr & 0xff);
		if (type == 0) {
			for (i = 0; i < 4; i++) {
				gain->pin_for_gain_idx[band_idx][path][i] = (data >> (8 * i)) & 0xff;
			}
		} else if (type == 1) {
			for (i = 0; i < 4; i++) {
				gain->pin_for_gain_idx[band_idx][path][4 + i] = (data >> (8 * i)) & 0xff;
			}
		} else if (type == 2) {
			for (i = 0; i < 4; i++) {
				gain->pin_for_gain_idx[band_idx][path][8 + i] = (data >> (8 * i)) & 0xff;
			}
		} else if (type == 3) {
			for (i = 0; i < 4; i++) {
				gain->pin_for_gain_idx[band_idx][path][12 + i] = (data >> (8 * i)) & 0xff;
			}
		}
	} else {
