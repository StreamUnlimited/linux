...
void halbb_ch_inf_physts_get_buf(struct bb_info *bb, u8 *rpt_buf,
...
bool halbb_cfg_ch_info_cr(struct bb_info *bb, struct bb_ch_info_cr_cfg_info *cfg)
{
	...
	/* disable empty report avoid mac buf full */
	halbb_set_reg(bb, CH_INFO_EMPTY_REPORT_EN_C, CH_INFO_EMPTY_REPORT_EN_C_M, 0);
...
enum bb_ch_info_t halbb_ch_info_get_data(struct bb_info *bb, u8 **addr, u32 len,
		struct bb_ch_rpt_hdr_info *hdr,
		struct bb_phy_info_rpt *phy_info,
		struct bb_ch_info_drv_rpt *drv)
{
	...
	u8 *praw = (u8 *)ch_rpt->bb_ch_info_raw_i.octet;
	u8 seq;
	...
	*addr += ch_rpt->ch_rpt_hdr_len;
	...
	if (ch_rpt->bb_ch_info_raw_i.octet) {
		if (ch_rpt->csi_raw_data_total_len > ch_rpt->bb_ch_info_raw_i.ch_info_buf_len) {
			BB_DBG(bb, DBG_CH_INFO, "[Error] buf len %d, data len %d\n", ch_rpt->bb_ch_info_raw_i.ch_info_buf_len,
			       ch_rpt->csi_raw_data_total_len);
		} else {
			if (!hdr->is_pkt_end) {
				halbb_mem_cpy(bb, &praw[ch_rpt->csi_raw_data_total_len - ch_rpt->total_len_remnant - drv->raw_data_len],
					      *addr, drv->raw_data_len);
			} else {
				halbb_mem_cpy(bb, &praw[ch_rpt->csi_raw_data_total_len - ch_rpt->total_len_remnant],
					      *addr, ch_rpt->total_len_remnant);
			}
		}
					...
					struct bb_ch_info_drv_rpt *drv)
{
	...
	rpt = halbb_ch_info_get_data(bb, &addr, len, hdr, phy_info, drv);
...
void halbb_ch_info_bbcr_init(struct bb_info *bb)
{
	...
	cfg->ch_i_blk_start_idx = 0;
	cfg->ch_i_blk_end_idx = 0;
	cfg->ch_i_ele_bitmap = 0x1; /*Nr X Nc: 1 X 1*/
	cfg->ch_i_type = BB_CH_LEGACY_CH;
	cfg->ch_i_seg_len = 0;
