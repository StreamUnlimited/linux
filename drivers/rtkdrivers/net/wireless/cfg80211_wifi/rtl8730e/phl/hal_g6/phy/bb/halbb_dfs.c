				  ...
				  bool w56_band)
{
	...
	/* PW unit: 200ns ; PRI unit: 25us */
	/*Type {1,2,3,4,5,6,7,8}*/ /*Type3.4 T1 max=120us, Type5.6.7.8 T1 max=240us*/
	u8 pw_min_mic_w53_tab[DFS_RDR_TYP_NUM] = {2, 2, 2, 2, 2, 2, 2, 2};
	u16 pw_max_mic_w53_tab[DFS_RDR_TYP_NUM] = {25, 75, 25, 75, 8, 8, 8, 8};
	u8 pri_min_mic_w53_tab[DFS_RDR_TYP_NUM] = {40, 25, 35, 21, 26, 33, 35, 44};
	u8 pri_max_mic_w53_tab[DFS_RDR_TYP_NUM] = {200, 200, 196, 196, 27, 34, 36, 45};
				 ...
				 struct hal_dfs_rpt *rpt, u16 dfs_rpt_idx)
{
	...
	u8 cur_seq_num_sg1 = 0, pre_seq_num_sg1 = 0;// For TW DFS
	u16 pw = 0;
	...
	halbb_radar_seq_inspctn(bb, dfs_rpt_idx, cur_seq_num, pre_seq_num, is_sg1);
...
bool halbb_radar_detect(struct bb_info *bb, struct hal_dfs_rpt *dfs_rpt)
{
	...
	u16 pw_diff_chrp_sg1 = 0, pw_diff_lng_sg1 = 0, pw_max = 0;
	...
	if (bb_dfs->l_rdr_exst_flag) {
		...
		if (bb_dfs->is_tw_en) {
			...
			if (pw_max > bb_dfs->pw_max_th) {
				bb_dfs->lng_rdr_cnt_sg1 = 0;          // reset lng_rdr cnt
				if (bb_dfs->dbg_hwdet_prnt_en || bb_dfs->dbg_brk_prnt_en) {
					BB_DBG(bb, DBG_DFS, "pw_max BRK, reset lng_rdr cnt_sg1!\n");
				}
			}
	...
	if (bb_dfs->is_tw_en) {
		if (rdr_detected_sg1) {
			...
			if (pw_diff_chrp_sg1 > bb_dfs->pw_diff_th) {
				rdr_detected_sg1 = false;
				if (bb_dfs->dbg_hwdet_prnt_en || bb_dfs->dbg_brk_prnt_en) {
