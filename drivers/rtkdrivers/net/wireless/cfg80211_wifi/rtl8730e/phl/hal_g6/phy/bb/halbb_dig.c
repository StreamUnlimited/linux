...
u8 halbb_get_lna_idx(struct bb_info *bb, enum rf_path path)
{
	...
	lna_idx = (u8)halbb_get_reg(bb, cr->path0_lna_init_idx, cr->path0_lna_init_idx_m);
	return lna_idx;
}
...
u8 halbb_get_rxb_idx(struct bb_info *bb, enum rf_path path)
{
	...
	rxb_idx = (u8)halbb_get_reg(bb, cr->path0_rxb_init_idx, cr->path0_rxb_init_idx_m);
	return rxb_idx;
}
...
void halbb_sdagc_follow_pagc_config(struct bb_info *bb, bool set_en)
{
	...
	halbb_set_reg_cmn(bb, cr->path0_p20_follow_by_pagcugc_en_a,
			  cr->path0_p20_follow_by_pagcugc_en_a_m, val, bb->bb_phy_idx); /* 8730e support bw20 only */
...
void halbb_dig_set_lna_tia_rxb(struct bb_info *bb, u8 lna, u8 tia, u8 rxb, enum phl_phy_idx phy_idx)
{
	struct bb_dig_info *bb_dig = &bb->bb_dig_i;
	struct bb_dig_cr_info *cr = &bb_dig->bb_dig_cr_i;
	halbb_set_reg_cmn(bb, cr->path0_lna_init_idx, cr->path0_lna_init_idx_m, lna, phy_idx);
	halbb_set_reg_cmn(bb, cr->path0_tia_init_idx, cr->path0_tia_init_idx_m, tia, phy_idx);
	halbb_set_reg_cmn(bb, cr->path0_rxb_init_idx, cr->path0_rxb_init_idx_m, rxb, phy_idx);
	return;
}
void halbb_set_lna_tia_rxb_by_igi(struct bb_info *bb, u8 igi_new, enum phl_phy_idx phy_idx)
{
	u8 lna = 6;
	u8 tia = 1;
	u8 rxb = 19;
	int i = 0;
	s8 idx_tbl[7][4] = {
		{-65, 60, 6, 1}, /* Pin, LNA+TIA gain, LNA_idx, TIA_idx*/
		{-55, 48, 5, 0},
		{-45, 38, 4, 0},
		{-35, 30, 3, 0},
		{-25, 22, 2, 0},
	...
	s8 pin = (s8)igi_new - 110;
	for (i = 0; i < 7; i++) {
		if (pin < idx_tbl[i][0]) {
			lna = idx_tbl[i][2];
			tia = idx_tbl[i][3];
			rxb = SUBTRACT_TO_0(-7 - pin + 6, idx_tbl[i][1]);
			if (i == 0 && rxb > 19) {
				rxb = 19;
			} else if (rxb > 31) {
				rxb = 31;
	...
	halbb_dig_set_lna_tia_rxb(bb, lna, tia, rxb, phy_idx);
}
void halbb_dig_cfg_bbcr(struct bb_info *bb, u8 igi_new)
{
	...
	post_pd_mode = halbb_get_reg(bb, PATH0_R_PURE_POST_PD_MODE_C, PATH0_R_PURE_POST_PD_MODE_C_M);
	if (post_pd_mode == 0) {
		if (g_pre_post_pd_mode != post_pd_mode) {
			halbb_dig_set_lna_tia_rxb(bb, 6, 1, 31, HW_PHY_0);
		}
	} else {
		halbb_set_lna_tia_rxb_by_igi(bb, igi_new, HW_PHY_0);
	}
	g_pre_post_pd_mode = post_pd_mode;
}
...
void halbb_tdmadig_callback(void *context)
{
	...
	if ((bb->phl_com->hci_type == RTW_HCI_PCIE) || (bb->phl_com->hci_type == RTW_HCI_AXI)) {
...
#if defined(BB_8730E_SUPPORT) || defined(BB_8720E_SUPPORT)
		cr->path0_rxb_init_idx = PATH0_R_RXBB_IDX_INIT_C;
		cr->path0_rxb_init_idx_m = PATH0_R_RXBB_IDX_INIT_C_M;
		cr->cca_rssi_lmt_en_a = CCA_RSSI_LMT_EN_C;
		cr->cca_rssi_lmt_en_a_m = CCA_RSSI_LMT_EN_C_M;
		cr->rssi_nocca_low_th_a = RSSI_NOCCA_LOW_TH_C;
		cr->rssi_nocca_low_th_a_m = RSSI_NOCCA_LOW_TH_C_M;
		cr->path0_p20_follow_by_pagcugc_en_a = PATH0_R_FOLLOW_BY_PAGCUGC_EN_C;
		cr->path0_p20_follow_by_pagcugc_en_a_m = PATH0_R_FOLLOW_BY_PAGCUGC_EN_C_M;
#else
