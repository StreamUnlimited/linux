...
void halbb_la_set_mac_trig_time(struct bb_info *bb, u32 trig_time, u8 *unit, u8 *unit_num)
{
	...
	*unit_num = (u8)(trig_time >> *unit);
	...
	BB_TRACE("2. [Set Trig-Time] Time=%d * unit=2^%d us\n",
		 *unit_num, *unit);
}
...
bool halbb_la_mac_cfg_buf(struct bb_info *bb, enum la_buff_mode_t mode)
{
	...
	case BB_RTL8730E:
		if (mode == LA_BUFF_64K) {
			buf->buffer_size = 0x10000;        /*64K Byte*/
		} else {
			buf->buffer_size = 0;
		}
		break;
...
void halbb_la_bb_set_cmn_reset(struct bb_info *bb)
{
	...
	la->la_smp_rate = LA_SMP_CLK_40;
	la->la_mac_cfg_i.la_trigger_time = 204;
#else
...
void halbb_la_bb_set_general(struct bb_info *bb)
{
	...
	/*[DBCC]*/
	dma->dma_dbgport_phy_sel = (bb->bb_phy_idx == HW_PHY_0) ? 0 : 1;
...
#ifndef CONFIG_WIFI_VERIFY_TRUEPHY
				halbb_delay_ms(bb, 100);
#else
				halbb_delay_ms(bb, 20);
#endif
				i++;
				continue;
		       ...
		       char *output, u32 *_out_len)
{
...
#if 0//ndef CONFIG_WIFI_VERIFY_TRUEPHY
	halbb_la_reset(bb);
#else
	bb->cr_type = BB_CLIENT;
	bb->ic_type = BB_RTL8720E;
	halbb_cr_cfg_la_init(bb);
	halbb_la_drv_buf_release(bb);
	halbb_la_init(bb);
#endif
#if defined(BB_8720E_SUPPORT) || defined(BB_8730E_SUPPORT)
	trig_time_cca = 204; // us
#else
	/*Trig Time*/
	trig_time_cca = ((la->la_string_i.smp_number_max >> (bw + 1)) / 10)
			- (2 << (2 - bw)) - (2 - bw);
	...
	if (test_case == 0) {
		...
		dma->dma_hdr_sel_60 = LA_HDR_CRC_OK;
#if defined(BB_8720E_SUPPORT) || defined(BB_8730E_SUPPORT)
		dma->dma_data_type = DMA00_NRML_1s_14b; /* bb trigger issue */
#else
		dma->dma_data_type = DMA01_NRML_2s_12b;
#endif
		dma->dma_dbgport_base_n = 31;
		      ...
		      u32 *_out_len)
{
	...
	} else if (_os_strcmp(input[1], "datavld") == 0) {
		u32 idx;
		for (idx = 0; idx < 32 * 1024; idx += 2) {
			if (buf->octet[idx]) {
				break;
			}
		}
		for (; idx < 32 * 1024; idx += 2) {
			if (buf->octet[idx] == 0) {
				break;
			}
			BB_TRACE("[0x%x] %08x%08x\n", 0x20060000 + idx * 4, SWAP4BYTE(buf->octet[idx + 1]), SWAP4BYTE(buf->octet[idx]));
		}
