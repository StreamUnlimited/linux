...
/* parsing hdr,IE 0/1/2/13/14/15*/
#define HALBB_PHYSTS_PARSING_DEFAULT
#ifdef HALBB_PHYSTS_PARSING_DEFAULT
/* parsing hdr,IE 0/1/2/ */
#define HALBB_PHYSTS_PARSING_BASIC
#endif
			...
			struct physts_rxd *desc)
{
	...
	if (desc->data_rate <= BB_11M) {
		psts_1->ch_idx = ie_1->ch_idx_seg0; /*need CH_idx for CCK rate*/
		return true;
	}
...
#endif
#ifndef HALBB_PHYSTS_PARSING_BASIC
			  ...
			  struct physts_result *bb_rpt)
{
	...
	/*Fix the lite can not get this is_su info */
	desc->is_su = 1;
	if (physts->frc_mu == 0) {
		//desc->is_su = 1;
		if (ie_map_type == DL_MU_SPOOFING || ie_map_type == HE_MU || ie_map_type == VHT_MU) {
			desc->is_su = 0;
		}
	}
	// if is_su rewrite macid_su
	if (desc->is_su) {
		desc->macid_su = desc->user_i[0].macid;
	}
...
{
	struct bb_physts_info *physts = &bb->bb_physts_i;
#if !defined(HALBB_PHYSTS_PARSING_DEFAULT) || defined(HALBB_DFS_SUPPORT)
	u32 u32_tmp = 0;
	u32 mask_tmp = 0;
	u8 i = 0;
#endif
