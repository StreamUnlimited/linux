...
bool ldpc_extra_check(u32 n_avbits, u32 n_pld, u8 code_rate)
{
	...
	u32 table0[4][4] = {{456, 304, 228, 152}, {732, 488, 366, 244},
		{1458, 972, 729, 486}, {972, 1296, 1458, 1620}
	};//912 * (1-R), 1464 * (1-R), 2916 * (1-R), 1944 * R
...
bool halbb_he_mcs_table(struct bb_info *bb, const struct plcp_mcs_table_in_t *in, struct plcp_mcs_table_out_t *out)
{
	...
	out->valid = ((in->nss > 0 && in->nss <= 8) && (!in->dcm || in->mcs == 0 || in->mcs == 1 || in->mcs == 3 || in->mcs == 4) && (!in->dcm || in->nss == 1 ||
			in->nss == 2) && (in->fec == LDPC || (in->mcs <= 9 && in->ru_size <= RU242)));
	if (!out->valid) {
		//rtw_error("invalid mcs input");
		return false;
	}
...
void halbb_get_mcs_out(struct bb_info *bb, const struct plcp_tx_pre_fec_padding_setting_in_t *in, struct plcp_tx_pre_fec_padding_setting_par_t *par,
		       struct plcp_tx_pre_fec_padding_setting_out_t *out, bool *valid)
{
...
void halbb_get_nsym_init(struct bb_info *bb, const struct plcp_tx_pre_fec_padding_setting_in_t *in, struct plcp_tx_pre_fec_padding_setting_par_t *par)
{
	...
	for (u = 0; u < par->com.n_usr_refine; u++) {
		...
		par->usr[u].n_sym_init = halbb_ceil((par->usr[u].apep_refine << 3) + n_service + n_tail * par->usr_mcs_out[u].n_es,
						    par->usr_mcs_out[u].n_dbps << par->com.stbc) << par->com.stbc;
	...
	par->com.pre_fec_padding_factor_init = (u16)
					       par->usr[u_max].pre_fec_padding_factor_init;//?????????????????????????????????????????????????????????????????????????????????
...
void halbb_get_nsym(struct bb_info *bb, const struct plcp_tx_pre_fec_padding_setting_in_t *in, const struct plcp_tx_pre_fec_padding_setting_par_t *par,
	...
	if (!par->com.ndp_en) {
		...
		for (u = 0; u < par->com.n_usr_refine; u++) {
			...
			if (par->usr_mcs_out[u].fec == LDPC) {
				...
				out->ldpc_extra = ldpc_extra_check(n_avbits, n_pld,
								   (u8)par->usr_mcs_out[u].code_rate);//?????????????????????????????????????????????????????????????????????????????????
				if (out->ldpc_extra > 0) {
					break;
				}
			}
...
void halbb_get_txtime(struct bb_info *bb, const struct plcp_tx_pre_fec_padding_setting_in_t *in, struct plcp_tx_pre_fec_padding_setting_par_t *par,
		      struct plcp_tx_pre_fec_padding_setting_out_t *out)
{
	...
	out->tx_time_0p4us = par->com.preamble_0p4us + par->com.n_ma * par->com.n_ltf_sym * par->com.t_ltf_sym_0p4us + out->n_sym * par->com.t_sym_0p4us + out->t_pe *
			     10;
...
void halbb_refine_input(struct bb_info *bb, const struct plcp_tx_pre_fec_padding_setting_in_t *in, struct plcp_tx_pre_fec_padding_setting_par_t *par)
{
	...
	par->com.preamble_0p4us = must_preamble_0p4us_table[in->format_idx] + (u16)par->com.n_ltf_sym * (u16)par->com.t_ltf_sym_0p4us +
				  (u16)par->com.n_hesigb_sym * 10;//?????????????????????????
...
//			return;
		...
		tmp = (max_tx_time_0p4us > par->com.preamble_0p4us + (disam + 2) * par->com.t_sym_0p4us) &&
		      par->com.doppler_mode > 0  ? (in->max_tx_time_0p4us - par->com.preamble_0p4us - (disam + 2) * par->com.t_sym_0p4us) /
		      (par->com.n_ltf_sym * par->com.t_ltf_sym_0p4us + par->com.m_ma * par->com.t_sym_0p4us) : 0;
		par->com.n_ma = par->com.doppler_mode > 0 ? tmp : 0;
		par->com.n_sym_init = ((u16)max_tx_time_0p4us - par->com.preamble_0p4us - ((u16)par->com.n_ma * (u16)par->com.n_ltf_sym * (u16)par->com.t_ltf_sym_0p4us)) /
				      (u16)par->com.t_sym_0p4us - disam; // Look

		if (par->com.stbc) {
			par->com.n_sym_init = (u16)halbb_ceil(par->com.n_sym_init, 2) << 1;        //???????????????????????????????
		}

		remain_time_0p4us = (u8)(max_tx_time_0p4us - par->com.preamble_0p4us - par->com.n_sym_init * par->com.t_sym_0p4us - par->com.n_ma * par->com.n_ltf_sym *
					 par->com.t_ltf_sym_0p4us);//????????????????????????????????? false?
		...
		if (par->com.spec_idx == SPEC_HE && !par->com.tb_trig) {
			...
			//tb_tri_en
			par->com.tb_trig_t_pe = (max_tx_time_0p4us - par->com.preamble_0p4us - par->com.n_sym_init * par->com.t_sym_0p4us - par->com.n_ma * par->com.n_ltf_sym *
						 par->com.t_ltf_sym_0p4us) / 10;
		...
		for (u = 0; u < par->com.n_usr_refine; u++) {
			n_dbps_last = par->com.pre_fec_padding_factor_init == 4 ? par->usr_mcs_out[u].n_dbps : par->usr_mcs_out[u].he_n_dbps_short *
				      par->com.pre_fec_padding_factor_init;
		...
		for (u = 0; u < par->com.n_usr_refine; u++) {
			...
			if (par->com.spec_idx == SPEC_LEGACY || (par->com.spec_idx == SPEC_HT && par->usr[u].n_mpdu_refine == 1)) {
				...
				par->usr[u].apep_refine = (par->usr[u].n_mpdu_refine - 1) * (4 + 4 * halbb_ceil(par->usr[u].mpdu_length_byte_refine,
							  4)) + (4 + par->usr[u].mpdu_length_byte_refine);
			}
		}
	}
}
