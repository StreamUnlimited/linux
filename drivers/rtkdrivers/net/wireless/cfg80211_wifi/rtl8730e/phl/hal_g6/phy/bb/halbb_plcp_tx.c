		  ...
		  struct bb_plcp_cr_info *cr, enum phl_phy_idx phy_idx)
{
	...
	// Triangular shaping
	if (bb->ic_type == BB_RTL8720E || bb->ic_type == BB_RTL8730E) {
		      ...
		      enum phl_phy_idx phy_idx)
{
	...
	if (in->ppdu_type == HE_TB_FMT) {
		...
		case 2:
			...
			else if ((in->usr[0].ru_alloc >> 1) == 9 || (in->usr[0].ru_alloc >> 1) == 41 || (in->usr[0].ru_alloc >> 1) == 55 || (in->usr[0].ru_alloc >> 1) == 62 ||
				 (in->usr[0].ru_alloc >> 1) == 65) {
				ch20_with_data = (bb->ic_type == BB_RTL8852A || bb->ic_type == BB_RTL8852B) ? 0xc0 : 0x3;
			}
			...
			else if ((in->usr[0].ru_alloc >> 1) == 27 || (in->usr[0].ru_alloc >> 1) == 48 || (in->usr[0].ru_alloc >> 1) == 58 || (in->usr[0].ru_alloc >> 1) == 63 ||
				 (in->usr[0].ru_alloc >> 1) == 66) {
			     ...
			     struct usr_plcp_gen_in *user, enum phl_phy_idx phy_idx)
{
	...
	if (bb->ic_type == BB_RTL8192XB) {
		...
		if (bb->dyn_pmac_tri_en) {
			if (in->usr[0].mcs <= 7 || ht_mcs <= 7) {
				halbb_set_reg_cmn(bb, cr->tx_tri_idx, cr->tx_tri_idx_m, 1, phy_idx);
				halbb_set_reg_cmn(bb, cr->tx_tri_pw_ofst, cr->tx_tri_pw_ofst_m, 0x10, phy_idx);
...
void halbb_cr_cfg_plcp_init(struct bb_info *bb)
{
	...
	case BB_CLIENT:
		...
		cr->b_rate_idx = BMODE_RATE_IDX_C;
		cr->b_rate_idx_m = BMODE_RATE_IDX_C_M;
		cr->b_locked_clk_en = BMODE_LOCKED_CLK_EN_C;
		cr->b_locked_clk_en_m = BMODE_LOCKED_CLK_EN_C_M;
#else
...
#if defined(BB_8730E_SUPPORT) || defined(BB_8720E_SUPPORT)
		cr->usr0_precoding_mode_idx = TXUSRCT0_PRECODING_MODE_IDX_C;
		cr->usr0_precoding_mode_idx_m = TXUSRCT0_PRECODING_MODE_IDX_C_M;
		cr->usr1_precoding_mode_idx = TXUSRCT1_PRECODING_MODE_IDX_C;
		cr->usr1_precoding_mode_idx_m = TXUSRCT1_PRECODING_MODE_IDX_C_M;
		cr->usr2_precoding_mode_idx = TXUSRCT2_PRECODING_MODE_IDX_C;
		cr->usr2_precoding_mode_idx_m = TXUSRCT2_PRECODING_MODE_IDX_C_M;
		cr->usr3_precoding_mode_idx = TXUSRCT3_PRECODING_MODE_IDX_C;
		cr->usr3_precoding_mode_idx_m = TXUSRCT3_PRECODING_MODE_IDX_C_M;
#else
		...
		cr->max_mcs = TXINFO_MAX_MCS_C;
		cr->max_mcs_m = TXINFO_MAX_MCS_C_M;
		cr->tx_tri_idx = TXINFO_TX_BANDEDGE_C;
		cr->tx_tri_idx_m = TXINFO_TX_BANDEDGE_C_M;
		cr->tx_tri_pw_ofst = TXINFO_PW_OFST_SEG0_DB_C;
		cr->tx_tri_pw_ofst_m = TXINFO_PW_OFST_SEG0_DB_C_M;
		break;
#endif
