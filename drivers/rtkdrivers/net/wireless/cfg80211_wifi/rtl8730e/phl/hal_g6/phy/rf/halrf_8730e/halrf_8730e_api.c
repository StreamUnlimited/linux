...
u8 halrf_get_thermal_8730e(struct rf_info *rf, enum rf_path rf_path)
{
	...
	thermal = (u8)halrf_rrf(rf, RF_PATH_A, 0x42, 0x0001FE);

	if (rf->support_ability & HAL_RF_THER_TRIM) {
		symbol_bit = (rf->ther_ofst_lsb & BIT(7)) >> 7;
		if (symbol_bit == 1) {
			thermal = thermal + (rf->ther_ofst_lsb & 0x3);
		} else {
			thermal = thermal - (rf->ther_ofst_lsb & 0x3);
		}
...
s8 halrf_get_ther_protected_threshold_8730e(struct rf_info *rf)
{
	...
	tmp = halrf_get_thermal(rf, RF_PATH_A);
