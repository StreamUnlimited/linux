...
void halrf_addck_8730e(struct rf_info *rf)
{
	...
	u32	offset_val = 0;
	u32	bb_20F0 = 0;
	s32	val_avg_I = 0, val_avg_Q = 0, offset_val_I_i = 0, offset_val_Q_i = 0;
	bool	do_pass = true;

	RF_DBG(rf, DBG_RF_DACK, "[ADCK]===> %s\n", __func__);
	...
	for (i = 0; i < 16; i++) {
		...
		offset_val_Q_i = offset_val & 0x000003FF;
		offset_val_I_i = (offset_val & 0x000FFC00) >> 10;
		/*
			RF_DBG(rf, DBG_RF_RXDCK, "[AFE DCK][before] read 1730 = %x, [19:10] = %x, [9:0] = %x\n",
				offset_val, offset_val_I[i], offset_val_Q[i]);
		*/
		if (offset_val_I_i & BIT(9)) {
			offset_val_I_i = offset_val_I_i - 1024;
		}
		if (offset_val_Q_i & BIT(9)) {
			offset_val_Q_i = offset_val_Q_i - 1024;
		}
		/*
			RF_DBG(rf, DBG_RF_RXDCK, "[AFE DCK]offset_val_I[%d] = %d, offset_val_Q[%d] = %d\n",
				i, offset_val_I[i], i, offset_val_Q[i]);
		*/
		val_avg_I += offset_val_I_i;
		val_avg_Q += offset_val_Q_i;
	}
	/*
...
void halrf_dack_8730e_s0(struct rf_info *rf)
{
	...
	u32	reg_backup[7];
	u32	sys_2c, sys_30, sys_34, sys_38, sys_result, val32; //sys_3c,
	u32	reg_1E58, dck_dos_I, dck_dos_Q, dck_kosen_I, dck_kosen_Q;
	u8	i = 0;

	RF_DBG(rf, DBG_RF_DACK, "[DACK]WL DACK start!!!\n");
