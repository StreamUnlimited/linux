...
u8 _halrf_ch_to_idx_8730e(struct rf_info *rf, u8 channel)
{
	u8	channelIndex;
...
u8 _halrf_idx_to_ch_8730e(struct rf_info *rf, u8 idx)
{
	u8 channelIndex;
...
void _halrf_tssi_rf_setting_8730e(struct rf_info *rf,
		...
		enum phl_phy_idx phy, enum rf_path path)
{
	...
	if (channel >= 1 && channel <= 14) {
		halrf_wreg(rf, 0x58d8, 0xffffffff, 0x8008b733);        //r_tssi_bypass_by_C_max = r_page58_D8[8:0]
	} else {
		halrf_wreg(rf, 0x58d8, 0xffffffff, 0x8008b733);
	}
			       ...
			       enum phl_phy_idx phy, enum rf_path path)
{
	...
	halrf_wreg(rf, 0x5814, 0x003c0000, 0x9);
				      ...
				      enum phl_phy_idx phy, enum rf_path path)
{
	...
	s8 thermal_up_b[DELTA_SWINTSSI_SIZE] = {0}, thermal_down_b[DELTA_SWINTSSI_SIZE] = {0};
...
void _halrf_tssi_set_dac_gain_tbl_8730e(struct rf_info *rf,
					enum phl_phy_idx phy, enum rf_path path)
{
	RF_DBG(rf, DBG_RF_TX_PWR_TRACK, "======>%s   path=%d\n", __func__, path);
...
void _halrf_tssi_slope_cal_8730e(struct rf_info *rf,
				 enum phl_phy_idx phy, enum rf_path path)
{
	u8 channel = rf->hal_com->band[phy].cur_chandef.center_ch;
	...
	for (j = 0; j < 4; j++) {
		...
		_halrf_tssi_hw_tx_8730e(rf, phy, path, 100, 5000, power_high[j], rate, bw, true);
		...
		for (k = 0; halrf_rreg(rf, 0x1c18, BIT(16)) == 0; k++) {
			...
			if (k > 100) {
				...
				RF_DBG(rf, DBG_RF_TX_PWR_TRACK, "[TSSI PA K] TSSI finish bit k > 100us path=%d\n",
				       i);
				_halrf_tssi_hw_tx_8730e(rf, phy, path, 100, 5000, power_high[j], rate, bw, false);
				return;
			}
		}
		_halrf_tssi_hw_tx_8730e(rf, phy, path, 100, 5000, power_high[j], rate, bw, false);
		tssi_cw_rpt_high = halrf_rreg(rf, 0x1c18, 0x000001ff);

		/*low power*/
		halrf_wreg(rf, 0x5820, 0x80000000, 0x0);
		halrf_wreg(rf, 0x5820, 0x80000000, 0x1);

		_halrf_tssi_hw_tx_8730e(rf, phy, path, 100, 5000, power_low[j], rate, bw, true);
		...
		_halrf_tssi_hw_tx_8730e(rf, phy, path, 100, 5000, power_high[j], rate, bw, false);
		tssi_cw_rpt_low = halrf_rreg(rf, 0x1c18, 0x000001ff);
...
void _halrf_tssi_set_rf_gap_tbl_8730e(struct rf_info *rf,
				      enum phl_phy_idx phy, enum rf_path path)
{
	RF_DBG(rf, DBG_RF_TX_PWR_TRACK, "======>%s   path=%d\n", __func__, path);
		...
		enum phl_phy_idx phy, enum rf_path path)
{
	...
	//halrf_wreg(rf, 0x5600, 0x3fffffff, 0x000000);
		if (channel >= 1 && channel <= 14) {
		halrf_wreg(rf, 0x5600, 0x0000003f, 0x21);//r_tssi_curve_p0
		halrf_wreg(rf, 0x5600, 0x00003f00, 0x2a);//r_tssi_curve_p1
		halrf_wreg(rf, 0x5600, 0x003f0000, 0x2d);//r_tssi_curve_p2
		halrf_wreg(rf, 0x5600, 0x3f000000, 0x3f);//r_tssi_curve_p3

		halrf_wreg(rf, 0x5630, 0x000003ff, 0x3ed);//r_tssi_J_ofdm_G0
		halrf_wreg(rf, 0x5630, 0x000FFC00, 0x032);//r_tssi_J_ofdm_G1
		halrf_wreg(rf, 0x5630, 0x3FF00000, 0x034);//r_tssi_J_ofdm_G2
		halrf_wreg(rf, 0x5634, 0x000003ff, 0x082);
		...
		halrf_wreg(rf, 0x563c, 0x000003ff, 0x3ed);//r_tssi_J_cck_G0
		halrf_wreg(rf, 0x563c, 0x000FFC00, 0x032);//r_tssi_J_cck_G1
		halrf_wreg(rf, 0x563c, 0x3FF00000, 0x034);//r_tssi_J_cck_G2
		halrf_wreg(rf, 0x5640, 0x000003ff, 0x082);
		...
		halrf_wreg(rf, 0x56c0, 0x00000007, 0x4);
		halrf_wreg(rf, 0x56c0, 0x00000038, 0x5);
		halrf_wreg(rf, 0x56c0, 0x000001C0, 0x6);
		halrf_wreg(rf, 0x56c0, 0x00000E00, 0x7);
		} else if (channel >= 36 && channel <= 64) {
		halrf_wreg(rf, 0x5600, 0x0000003f, 0x21);//r_tssi_curve_p0
		halrf_wreg(rf, 0x5600, 0x00003f00, 0x27);//r_tssi_curve_p1
		halrf_wreg(rf, 0x5600, 0x003f0000, 0x2a);//r_tssi_curve_p2
		halrf_wreg(rf, 0x5600, 0x3f000000, 0x3f);//r_tssi_curve_p3

		halrf_wreg(rf, 0x5630, 0x000003ff, 0x3f0);//r_tssi_J_ofdm_G0
		halrf_wreg(rf, 0x5630, 0x000FFC00, 0x021);//r_tssi_J_ofdm_G1
		halrf_wreg(rf, 0x5630, 0x3FF00000, 0x03d);//r_tssi_J_ofdm_G2
		...
		halrf_wreg(rf, 0x5630, 0x000003ff, 0x3f6);//r_tssi_J_ofdm_G0
		halrf_wreg(rf, 0x5630, 0x000FFC00, 0x027);//r_tssi_J_ofdm_G1
		halrf_wreg(rf, 0x5630, 0x3FF00000, 0x043);//r_tssi_J_ofdm_G2
		halrf_wreg(rf, 0x5634, 0x000003ff, 0x088);
		halrf_wreg(rf, 0x5634, 0x000ffc00, 0x000);
		halrf_wreg(rf, 0x5634, 0x3ff00000, 0x000);
		halrf_wreg(rf, 0x5638, 0x000003ff, 0x000);
		halrf_wreg(rf, 0x5638, 0x000ffc00, 0x000);
		...
		halrf_wreg(rf, 0x5630, 0x000003ff, 0x3fd);//r_tssi_J_ofdm_G0
		halrf_wreg(rf, 0x5630, 0x000FFC00, 0x02e);//r_tssi_J_ofdm_G1
		halrf_wreg(rf, 0x5630, 0x3FF00000, 0x04e);//r_tssi_J_ofdm_G2
		halrf_wreg(rf, 0x5634, 0x000003ff, 0x08a);
		halrf_wreg(rf, 0x5634, 0x000ffc00, 0x000);
		halrf_wreg(rf, 0x5634, 0x3ff00000, 0x000);
		halrf_wreg(rf, 0x5638, 0x000003ff, 0x000);
		halrf_wreg(rf, 0x5638, 0x000ffc00, 0x000);
		...
		enum phl_phy_idx phy, enum rf_path path)
{
	...
	group_idx = _halrf_tssi_get_ofdm_efuse_group_8730e(rf, phy);
		...
		enum phl_phy_idx phy, enum rf_path path)
{
	...
	group_idx = _halrf_tssi_get_tssi_trim_efuse_group_8730e(rf, phy);
				...
				enum phl_phy_idx phy, enum rf_path path)
{
	...
	_tssi_backup_bb_registers_8730e(rf, phy, bb_reg, bb_reg_backup,
					backup_num);
	...
	for (j = 0; j < 2; j++) {
		...
		RF_DBG(rf, DBG_RF_TX_PWR_TRACK,
		       ...
		       0x5820, halrf_rreg(rf, 0x5820, 0xffffffff), path);

		_halrf_tssi_hw_tx_8730e(rf, phy, path, 100, 5000, power[j], rate, bw, true);
		...
		for (k = 0; halrf_rreg(rf, 0x1c18, BIT(16)) == 0; k++) {
			...
			if (k > retry) {
				RF_DBG(rf, DBG_RF_TX_PWR_TRACK, "[TSSI PA K] TSSI finish bit k > %d mp:100ms normal:30us path=%d\n",
				       k, path);
				_tssi_reload_bb_registers_8730e(rf, phy, bb_reg, bb_reg_backup,
								backup_num);
				_halrf_tssi_hw_tx_8730e(rf, phy, path, 100, 5000, power[j], rate, bw, false);
	...
	if (channel >= 1 && channel <= 14) {
		...
		tmp = ((tssi_alim_offset[1] & 0x3ff) << 10) |
		      (tssi_alim_offset[2] & 0x3ff);
		halrf_wreg(rf, 0x5630, 0x000fffff, tmp);//ofdm
		halrf_wreg(rf, 0x563c, 0x000fffff, tmp);//cck
...
void _halrf_tssi_alimentk_done_8730e(struct rf_info *rf,
				     enum phl_phy_idx phy, enum rf_path path)
{
	struct halrf_tssi_info *tssi_info = &rf->tssi;
	u8 channel = rf->hal_com->band[phy].cur_chandef.center_ch;
...
void halrf_tssi_backup_txagc_8730e(struct rf_info *rf, enum phl_phy_idx phy, bool enable)
{
	...
	s16 xdbm;
	u32 i, j, tx_couter = 0;
	u8 ch_idx = _halrf_ch_to_idx_8730e(rf, channel);
	u8 channel_tmp;
	...
	_halrf_tssi_hw_tx_8730e(rf, phy, RF_PATH_AB, 1000, 100, power_tmp, T_HT_MF, 0, true);

#if 0
	/*Path B*/
	tx_couter[RF_PATH_B] = halrf_rreg(rf, 0x1a40, 0xffff);

	_halrf_tssi_hw_tx_8730e(rf, phy, RF_PATH_B, 1000, 100, xdbm, T_HT_MF, 0, true);
	halrf_delay_ms(rf, 15);
	_halrf_tssi_hw_tx_8730e(rf, phy, RF_PATH_B, 1000, 100, xdbm, T_HT_MF, 0, false);
#endif
	...
	tssi_info->backup_txagc_oft_ther[RF_PATH_A][ch_idx] =
		halrf_get_thermal_8730e(rf, RF_PATH_A);

	tssi_info->backup_txagc_oft_ther[RF_PATH_B][ch_idx] =
		halrf_get_thermal_8730e(rf, RF_PATH_B);

	_halrf_tssi_hw_tx_8730e(rf, phy, RF_PATH_AB, 1000, 100, power_tmp, T_HT_MF, 0, false);
	...
	//_tssi_reload_bb_registers_8730e(rf, phy, bb_reg, bb_reg_backup, backup_num);
}
...
void halrf_tssi_tracking_8730e(struct rf_info *rf)
{
	...
	if (halrf_rreg(rf, 0x1c7c, BIT(18)) == 0) {
		tssi_info->base_ther[RF_PATH_A] = (u8)halrf_rreg(rf, 0x1cd0, 0x0000007F);
	}

	cur_ther = (u8)halrf_rreg(rf, 0x1c10, 0x7F000000); /*[30:24]*/
	//base_ther = (u8)halrf_rreg(rf, 0x1cd0, 0x0000007F);/*[6:0]*/
	//base_ther = thermal >> 1;
...
void halrf_tssi_pre_tx_mp_8730e(struct rf_info *rf, enum phl_phy_idx phy)
{
	struct halrf_tssi_info *tssi_info = &rf->tssi;
	...
	//halrf_delay_ms(rf, 15);
	tsside = halrf_rreg(rf, 0x5838, 0x003ff000);
	//halrf_wreg(rf, 0x5838, 0x003ff000,tsside - 6);

	/*Path A*/
	tx_couter = halrf_rreg(rf, 0x1a40, 0xffff);
	_halrf_tssi_hw_tx_8730e(rf, phy, RF_PATH_A, 40, 10, xdbm, T_HT_MF, 0, true);
	halrf_delay_ms(rf, 15);
	_halrf_tssi_hw_tx_8730e(rf, phy, RF_PATH_A, 40, 10, xdbm, T_HT_MF, 0, false);

	tssi_info->base_ther[RF_PATH_A] = (u8)halrf_rreg(rf, 0x1cd0, 0x0000007F);
	//halrf_btc_rfk_ntfy(rf, (BIT(phy) << 4), RF_BTC_TSSI, RFK_STOP);
	RF_DBG(rf, DBG_RF_TX_PWR_TRACK,
	       "txagc offset 0x1c60[7:0]=0x%x \n",
	       halrf_rreg(rf, 0x1c60, 0xff000000));
	halrf_wreg(rf, 0x5838, 0x003ff000, tsside);
			 ...
			 enum phl_phy_idx phy)
{
	...
	halrf_tssi_disable_8730e(rf, phy);
		_halrf_tssi_set_sys_8730e(rf, phy, i);
	_halrf_tssi_rf_setting_8730e(rf, phy, i);
		_halrf_tssi_ini_txpwr_ctrl_bb_8730e(rf, phy, i);
		_halrf_tssi_ini_txpwr_ctrl_bb_he_tb_8730e(rf, phy, i);
		_halrf_tssi_set_dck_8730e(rf, phy, i);
	_halrf_tssi_set_bbgain_split_8730e(rf, phy, i);
		_halrf_tssi_set_tmeter_tbl_8730e(rf, phy, i);
		_halrf_tssi_slope_cal_org_8730e(rf, phy, i);
	_halrf_tssi_set_alignment_default_8730e(rf, phy, i);
	_halrf_tssi_set_slope_8730e(rf, phy, i);
	_halrf_tssi_run_slope_8730e(rf, phy, i);
		halrf_tmac_tx_pause(rf, phy, true);
		_halrf_tssi_alimentk_8730e(rf, phy, i);
		halrf_tmac_tx_pause(rf, phy, false);

	if (rf->phl_com->drv_mode == RTW_DRV_MODE_MP) {
		if (tssi_info->tssi_type[HW_PHY_0] == TSSI_ON || tssi_info->tssi_type[HW_PHY_0] == TSSI_CAL) {
			halrf_tssi_enable_8730e(rf, phy);
			halrf_tssi_set_efuse_to_de_8730e(rf, phy);
			halrf_tssi_pre_tx_mp_8730e(rf, phy);
		} else {
			halrf_tssi_disable_8730e(rf, phy);
		}
		...
		if (rf->support_ability & HAL_RF_TX_PWR_TRACK) {
			halrf_tssi_enable_8730e(rf, phy);
			halrf_tssi_set_efuse_to_de_8730e(rf, phy);
			//halrf_tssi_backup_txagc_8730e(rf, phy, true);
		}
	}
			      ...
			      enum phl_phy_idx phy)
{
	...
	for (i = RF_PATH_A; i < TSSI_PATH_MAX_8730E; i++) {
		_halrf_tssi_rf_setting_8730e(rf, phy, i);
		_halrf_tssi_set_sys_8730e(rf, phy, i);
		//_halrf_tssi_ini_txpwr_ctrl_bb_8730e(rf, phy, i);
		//_halrf_tssi_ini_txpwr_ctrl_bb_he_tb_8730e(rf, phy, i);
		...
		//_halrf_tssi_set_bbgain_split_8730e(rf, phy, i);
		_halrf_tssi_set_tmeter_tbl_8730e(rf, phy, i);
		//_halrf_tssi_set_dac_gain_tbl_8730e(rf, phy, i);
		//_halrf_tssi_slope_cal_org_8730e(rf, phy, i);
		//_halrf_tssi_set_rf_gap_tbl_8730e(rf, phy, i);
		//_halrf_tssi_set_tssi_slope_8730e(rf, phy, i);

		if (tssi_info->alignment_done[i][band] == true) {
			_halrf_tssi_alimentk_done_8730e(rf, phy, i);
		}
	}
				       ...
				       enum phl_phy_idx phy)
{
	...
	halrf_tssi_get_efuse_8730e(rf, phy);
		...
		enum phl_phy_idx phy, u32 tssi_de, u8 path)
{
	...
	tssi_trim = _halrf_tssi_get_ofdm_tssi_trim_de_8730e(rf, phy, path);
			     ...
			     enum phl_phy_idx phy)
{
	...
	i = 0;
		/*_halrf_tssi_set_tssi_slope_8730e(rf, phy, i);*/
		_halrf_tssi_set_tssi_track_8730e(rf, phy, i);
		_halrf_tssi_set_txagc_offset_mv_avg_8730e(rf, phy, i);
			halrf_wreg(rf, 0x58e4, 0x00004000, 0x0);
			halrf_wreg(rf, 0x5820, 0x80000000, 0x0);
			halrf_wreg(rf, 0x5820, 0x80000000, 0x1);
				   ...
				   u8 path, s32 dbm, s32 puot)
{
	...
	tssi_trim = _halrf_tssi_get_ofdm_tssi_trim_de_8730e(rf, phy_idx, path);
				      ...
				      enum phl_phy_idx phy)
{
	...
	if (tssi_info->tssi_type[HW_PHY_0] == TSSI_CAL) {
		...
		for (i = 0; i < TSSI_PATH_MAX_8730E; i++) {
			tssi_trim = _halrf_tssi_get_ofdm_tssi_trim_de_8730e(rf, phy, i);
			tmp = (s32)tssi_trim;
	...
	for (i = 0; i < TSSI_PATH_MAX_8730E; i++) {
		/*CCK*/
		group_idx = _halrf_tssi_get_cck_efuse_group_8730e(rf, phy);
		tssi_trim = _halrf_tssi_get_ofdm_tssi_trim_de_8730e(rf, phy, i);
		...
		/*OFDM*/
		ofmd_de = _halrf_tssi_get_ofdm_efuse_tssi_de_8730e(rf, phy, i);
		tssi_trim = _halrf_tssi_get_ofdm_tssi_trim_de_8730e(rf, phy, i);
				    ...
				    enum phl_phy_idx phy, bool enable)
{
	...
	if (enable == true) {
		...
		_halrf_tssi_alimentk_done_8730e(rf, phy, RF_PATH_A);
		_halrf_tssi_alimentk_done_8730e(rf, phy, RF_PATH_B);
...
void halrf_tssi_scan_ch_8730e(struct rf_info *rf, enum rf_path path)
{
	...
	if (!(rf->phl_com->drv_mode == RTW_DRV_MODE_MP)) {
		...
		if (tssi_info->alignment_done[path][band] == true) {
			_halrf_tssi_alimentk_done_8730e(rf, 0, path);
		}
...
void halrf_tssi_hw_tx_8730e(struct rf_info *rf,
			    ...
			    bool enable)
{
	_halrf_tssi_hw_tx_8730e(rf, phy, path, cnt, 5000, dbm, T_HT_MF, 0, enable);
}
...
void halrf_tssi_ant_open_8730e(struct rf_info *rf)
{
	...
	if ((halrf_rreg(rf, 0x1c60, 0xff000000) == 0x7f) ||
		...
		halrf_tssi_disable_8730e(rf, HW_PHY_0);
		halrf_tssi_enable_8730e(rf, HW_PHY_0);
	}
}
