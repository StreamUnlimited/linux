...
void _halrf_txgapk_bkup_rf_8730e(
	struct rf_info *rf,
	u32 *rf_reg,
	u32 rf_bkup[][TXGAPK_RF_REG_NUM_8730E],
	u8 path)
{
	u8 i;

	for (i = 0; i < TXGAPK_RF_REG_NUM_8730E; i++) {
...
void _halrf_txgapk_bb_afe_by_mode_8730e(struct rf_info *rf,
...
void _halrf_txgapk_iqk_preset_by_mode_8730e(struct rf_info *rf,
		enum phl_phy_idx phy, enum rf_path path, bool is_dbcc)
{
	RF_DBG(rf, DBG_RF_TXGAPK, "[TXGAPK]======> %s\n", __func__);
...
void _halrf_txgapk_clk_setting_dac960mhz_by_mode_8730e
...
void _halrf_txgapk_before_one_shot_enable_8730e
(struct rf_info *rf)
{
	RF_DBG(rf, DBG_RF_TXGAPK, "[TXGAPK]======> %s\n", __func__);
...
void _halrf_txgapk_one_shot_nctl_done_check_8730e
(struct rf_info *rf, enum txgapk_id id, enum rf_path path)
{
	...
	RF_DBG(rf, DBG_RF_TXGAPK, "[TXGAPK]======> check 0x80fc[15:0] = 0x%x, IsReady = %d, ReadTimes = %d, 0x%x= 0x%x \n", r_80fc, is_ready, count, 0x8010,
	       halrf_rreg(rf, 0x8010, MASKDWORD));

}


void _halrf_txgapk_track_table_nctl_2g_8730e
(struct rf_info *rf, enum phl_phy_idx phy, enum rf_path path)
{
...
#else
	halrf_set_rx_dck_8730e(rf, phy, path, false);
#endif
	...
	_halrf_txgapk_before_one_shot_enable_8730e(rf);
	halrf_wreg(rf, 0x8000, MASKDWORD, cal_path[path]);

	_halrf_txgapk_one_shot_nctl_done_check_8730e(rf, TXGAPK_TRACK, path);
...
void _halrf_txgapk_track_table_nctl_5g_8730e
(struct rf_info *rf, enum phl_phy_idx phy, enum rf_path path)
{
	struct halrf_gapk_info *txgapk_info = &rf->gapk;
...
void _halrf_txgapk_track_table_nctl_8730e
(struct rf_info *rf, enum phl_phy_idx phy, enum rf_path path)
{
	u8 channel = rf->hal_com->band[phy].cur_chandef.center_ch;

	if (channel >= 1 && channel <= 14) {
		_halrf_txgapk_track_table_nctl_2g_8730e(rf, phy, path);
	} else {
		_halrf_txgapk_track_table_nctl_5g_8730e(rf, phy, path);
	}
}

void _halrf_txgapk_write_track_table_default_2g_8730e
(struct rf_info *rf, enum phl_phy_idx phy, enum rf_path path)
{
	struct halrf_gapk_info *txgapk_info = &rf->gapk;
	u32 bias = 0;
	...
	/* AP iFEM */
	/* AP eFEM */
	halrf_wrf(rf, path, TXGAPK_DEBUGMASK_8730E, 0x08000, 0x1); /* enter debug mode before write */

	halrf_wrf(rf, path, TXGAP_TB_ADDR_8730E, 0x0f, (0x0 | bias));
	halrf_wrf(rf, path, TXGAP_TB_VAL_8730E, 0x3f, 0x0);

	halrf_wrf(rf, path, TXGAP_TB_ADDR_8730E, 0x0f, (0x1 | bias));
	halrf_wrf(rf, path, TXGAP_TB_VAL_8730E, 0x3f, 0x0);

	halrf_wrf(rf, path, TXGAP_TB_ADDR_8730E, 0x0f, (0x2 | bias));
	halrf_wrf(rf, path, TXGAP_TB_VAL_8730E, 0x3f, 0x0);

	halrf_wrf(rf, path, TXGAP_TB_ADDR_8730E, 0x0f, (0x3 | bias));
	halrf_wrf(rf, path, TXGAP_TB_VAL_8730E, 0x3f, 0x0);

	halrf_wrf(rf, path, TXGAP_TB_ADDR_8730E, 0x0f, (0x4 | bias));
	halrf_wrf(rf, path, TXGAP_TB_VAL_8730E, 0x3f, 0x0);

	halrf_wrf(rf, path, TXGAP_TB_ADDR_8730E, 0x0f, (0x5 | bias));
	halrf_wrf(rf, path, TXGAP_TB_VAL_8730E, 0x3f, 0x0);

	halrf_wrf(rf, path, TXGAPK_DEBUGMASK_8730E, 0x08000, 0x0); /* exit debug mode after write */
}

void _halrf_txgapk_write_track_table_default_5gl_8730e
(struct rf_info *rf, enum phl_phy_idx phy, enum rf_path path)
{
	struct halrf_gapk_info *txgapk_info = &rf->gapk;
	u32 bias = 0;
...
void _halrf_txgapk_write_track_table_default_5gm_8730e
(struct rf_info *rf, enum phl_phy_idx phy, enum rf_path path)
{
	struct halrf_gapk_info *txgapk_info = &rf->gapk;
	u32 bias = 0;
...
void _halrf_txgapk_write_track_table_default_5gh_8730e
(struct rf_info *rf, enum phl_phy_idx phy, enum rf_path path)
{
	struct halrf_gapk_info *txgapk_info = &rf->gapk;
	u32 bias = 0;
...
void _halrf_txgapk_write_track_table_default_8730e
(struct rf_info *rf, enum phl_phy_idx phy, enum rf_path path)
{
	u8 channel = rf->hal_com->band[phy].cur_chandef.center_ch;

	if (channel >= 1 && channel <= 14) {
		_halrf_txgapk_write_track_table_default_2g_8730e(rf, phy, path);
	} else if (channel >= 36 && channel <= 64) {
		_halrf_txgapk_write_track_table_default_5gl_8730e(rf, phy, path);
	} else if (channel >= 100 && channel <= 144) {
		_halrf_txgapk_write_track_table_default_5gm_8730e(rf, phy, path);
	} else if (channel >= 149 && channel <= 177) {
		_halrf_txgapk_write_track_table_default_5gh_8730e(rf, phy, path);
	}
}

void _halrf_txgapk_write_track_table_2g_8730e
(struct rf_info *rf, enum phl_phy_idx phy, enum rf_path path)
{
	struct halrf_gapk_info *txgapk_info = &rf->gapk;
	...
	halrf_wrf(rf, path, TXGAP_TB_ADDR_8730E, 0x0f, 0x0 | bias);
	halrf_wrf(rf, path, TXGAP_TB_VAL_8730E, 0x0003f, (txgapk_info->track_ta[path][pa_change[0]] / 2) & 0x3f);

	halrf_wrf(rf, path, TXGAP_TB_ADDR_8730E, 0x0f, 0x1 | bias);
	halrf_wrf(rf, path, TXGAP_TB_VAL_8730E, 0x0003f, (txgapk_info->track_ta[path][pa_change[1]] / 2) & 0x3f);

	halrf_wrf(rf, path, TXGAP_TB_ADDR_8730E, 0x0f, 0x2 | bias);
	halrf_wrf(rf, path, TXGAP_TB_VAL_8730E, 0x0003f, (txgapk_info->track_ta[path][pa_change[2]] / 2) & 0x3f);

	halrf_wrf(rf, path, TXGAP_TB_ADDR_8730E, 0x0f, 0x3 | bias);
	halrf_wrf(rf, path, TXGAP_TB_VAL_8730E, 0x0003f, (txgapk_info->track_ta[path][pa_change[3]] / 2) & 0x3f);

	halrf_wrf(rf, path, TXGAP_TB_ADDR_8730E, 0x0f, 0x4 | bias);
	halrf_wrf(rf, path, TXGAP_TB_VAL_8730E, 0x0003f, (txgapk_info->track_ta[path][pa_change[4]] / 2) & 0x3f);

	halrf_wrf(rf, path, TXGAP_TB_ADDR_8730E, 0x0f, 0x5 | bias);
	halrf_wrf(rf, path, TXGAP_TB_VAL_8730E, 0x0003f, (txgapk_info->track_ta[path][pa_change[5]] / 2) & 0x3f);

	halrf_wrf(rf, path, TXGAPK_DEBUGMASK_8730E, 0x08000, 0x0); /* exit debug mode after write */
}

void _halrf_txgapk_write_track_table_5gl_8730e
(struct rf_info *rf, enum phl_phy_idx phy, enum rf_path path)
{
	struct halrf_gapk_info *txgapk_info = &rf->gapk;
	u8 pa_change[6] = {0};
...
void _halrf_txgapk_write_track_table_5gm_8730e
(struct rf_info *rf, enum phl_phy_idx phy, enum rf_path path)
{
	struct halrf_gapk_info *txgapk_info = &rf->gapk;
	u8 pa_change[6] = {0};
...
void _halrf_txgapk_write_track_table_5gh_8730e
(struct rf_info *rf, enum phl_phy_idx phy, enum rf_path path)
{
	struct halrf_gapk_info *txgapk_info = &rf->gapk;
	u8 pa_change[6] = {0};
...
void _halrf_txgapk_write_track_table_8730e
(struct rf_info *rf, enum phl_phy_idx phy, enum rf_path path)
{
	u8 channel = rf->hal_com->band[phy].cur_chandef.center_ch;

	if (channel >= 1 && channel <= 14) {
		_halrf_txgapk_write_track_table_2g_8730e(rf, phy, path);
	} else if (channel >= 36 && channel <= 64) {
		_halrf_txgapk_write_track_table_5gl_8730e(rf, phy, path);
	} else if (channel >= 100 && channel <= 144) {
		_halrf_txgapk_write_track_table_5gm_8730e(rf, phy, path);
	} else if (channel >= 149 && channel <= 177) {
		_halrf_txgapk_write_track_table_5gh_8730e(rf, phy, path);
	}
}

void _halrf_txgapk_power_table_nctl_2g_8730e
(struct rf_info *rf, enum phl_phy_idx phy, enum rf_path path)
{
	struct halrf_gapk_info *txgapk_info = &rf->gapk;
	/* u8 rfe_type = rf->phl_com->dev_cap.rfe_type; */
	...
	_halrf_txgapk_one_shot_nctl_done_check_8730e(rf, TXGAPK_PWR, path);
...
void _halrf_txgapk_power_table_nctl_5g_8730e
(struct rf_info *rf, enum phl_phy_idx phy, enum rf_path path)
{
	struct halrf_gapk_info *txgapk_info = &rf->gapk;
	/* u8 rfe_type = rf->phl_com->dev_cap.rfe_type; */
...
void _halrf_txgapk_power_table_nctl_8730e
(struct rf_info *rf, enum phl_phy_idx phy, enum rf_path path)
{
	u8 channel = rf->hal_com->band[phy].cur_chandef.center_ch;

	if (channel >= 1 && channel <= 14) {
		_halrf_txgapk_power_table_nctl_2g_8730e(rf, phy, path);
	} else {
		_halrf_txgapk_power_table_nctl_5g_8730e(rf, phy, path);
	}

}

void _halrf_txgapk_write_power_table_default_2g_8730e
(struct rf_info *rf, enum phl_phy_idx phy, enum rf_path path)
{
	struct halrf_gapk_info *txgapk_info = &rf->gapk;
	u32 bias = 0;
	...
	halrf_wrf(rf, path, TXGAPK_DEBUGMASK_8730E, 0x40000, 0x1); /* enter debug mode before write */

	halrf_wrf(rf, path, TXGAP_TB_ADDR_8730E, 0x07, 0x0 | bias);
	halrf_wrf(rf, path, TXGAP_TB_VAL_8730E, 0x0003f, 0x0);

	halrf_wrf(rf, path, TXGAP_TB_ADDR_8730E, 0x07, 0x1 | bias);
	halrf_wrf(rf, path, TXGAP_TB_VAL_8730E, 0x0003f, 0x0);

	halrf_wrf(rf, path, TXGAP_TB_ADDR_8730E, 0x07, 0x2 | bias);
	halrf_wrf(rf, path, TXGAP_TB_VAL_8730E, 0x0003f, 0x0);

	halrf_wrf(rf, path, TXGAPK_DEBUGMASK_8730E, 0x40000, 0x0); /* exit debug mode after write */
}

void _halrf_txgapk_write_power_table_default_5gl_8730e
(struct rf_info *rf, enum phl_phy_idx phy, enum rf_path path)
{
	struct halrf_gapk_info *txgapk_info = &rf->gapk;
	u32 bias = 0;
...
void _halrf_txgapk_write_power_table_default_5gm_8730e
(struct rf_info *rf, enum phl_phy_idx phy, enum rf_path path)
{
	struct halrf_gapk_info *txgapk_info = &rf->gapk;
	u32 bias = 0;
...
void _halrf_txgapk_write_power_table_default_5gh_8730e
(struct rf_info *rf, enum phl_phy_idx phy, enum rf_path path)
{
	struct halrf_gapk_info *txgapk_info = &rf->gapk;
	u32 bias = 0;
...
void _halrf_txgapk_write_power_table_default_8730e
(struct rf_info *rf, enum phl_phy_idx phy, enum rf_path path)
{
	u8 channel = rf->hal_com->band[phy].cur_chandef.center_ch;

	if (channel >= 1 && channel <= 14) {
		_halrf_txgapk_write_power_table_default_2g_8730e(rf, phy, path);
	} else if (channel >= 36 && channel <= 64) {
		_halrf_txgapk_write_power_table_default_5gl_8730e(rf, phy, path);
	} else if (channel >= 100 && channel <= 144) {
		_halrf_txgapk_write_power_table_default_5gm_8730e(rf, phy, path);
	} else if (channel >= 149 && channel <= 177) {
		_halrf_txgapk_write_power_table_default_5gh_8730e(rf, phy, path);
	}
}

void _halrf_txgapk_write_power_table_2g_8730e
(struct rf_info *rf, enum phl_phy_idx phy, enum rf_path path)
{
	struct halrf_gapk_info *txgapk_info = &rf->gapk;
	u8 rfe_type = rf->phl_com->dev_cap.rfe_type;
	...
	if (rfe_type <= 50) {
		...
		halrf_wrf(rf, path, TXGAP_TB_ADDR_8730E, 0x07, 0x0 | bias);
		halrf_wrf(rf, path, TXGAP_TB_VAL_8730E, 0x0003f, (txgapk_info->power_ta[path][pa_change[0]] / 2) & 0x3f);

		halrf_wrf(rf, path, TXGAP_TB_ADDR_8730E, 0x07, 0x1 | bias);
		halrf_wrf(rf, path, TXGAP_TB_VAL_8730E, 0x0003f, (txgapk_info->power_ta[path][pa_change[1]] / 2) & 0x3f);

		halrf_wrf(rf, path, TXGAP_TB_ADDR_8730E, 0x07, 0x2 | bias);
		halrf_wrf(rf, path, TXGAP_TB_VAL_8730E, 0x0003f, (txgapk_info->power_ta[path][pa_change[2]] / 2) & 0x3f);
	} else {
		/* AP eFEM */
		pa_change[0] = 14;
...
void _halrf_txgapk_write_power_table_5gl_8730e
(struct rf_info *rf, enum phl_phy_idx phy, enum rf_path path)
{
	struct halrf_gapk_info *txgapk_info = &rf->gapk;
	u8 rfe_type = rf->phl_com->dev_cap.rfe_type;
...
void _halrf_txgapk_write_power_table_5gm_8730e
(struct rf_info *rf, enum phl_phy_idx phy, enum rf_path path)
{
	struct halrf_gapk_info *txgapk_info = &rf->gapk;
	u8 rfe_type = rf->phl_com->dev_cap.rfe_type;
...
void _halrf_txgapk_write_power_table_5gh_8730e
(struct rf_info *rf, enum phl_phy_idx phy, enum rf_path path)
{
	struct halrf_gapk_info *txgapk_info = &rf->gapk;
	u8 rfe_type = rf->phl_com->dev_cap.rfe_type;
...
void _halrf_txgapk_write_power_table_8730e
(struct rf_info *rf, enum phl_phy_idx phy, enum rf_path path)
{
	u8 channel = rf->hal_com->band[phy].cur_chandef.center_ch;

	if (channel >= 1 && channel <= 14) {
		_halrf_txgapk_write_power_table_2g_8730e(rf, phy, path);
	} else if (channel >= 36 && channel <= 64) {
		_halrf_txgapk_write_power_table_5gl_8730e(rf, phy, path);
	} else if (channel >= 100 && channel <= 144) {
		_halrf_txgapk_write_power_table_5gm_8730e(rf, phy, path);
	} else if (channel >= 149 && channel <= 177) {
		_halrf_txgapk_write_power_table_5gh_8730e(rf, phy, path);
	}
}

void _halrf_txgapk_iqk_bk_reg_by_mode_8730e
(struct rf_info *rf, enum phl_phy_idx phy, enum rf_path path, bool is_dbcc)
{
	...
	if (!is_dbcc) {
		...
		_halrf_txgapk_one_shot_nctl_done_check_8730e(rf, TXGAPK_IQKBK, path);
...
void _halrf_txgapk_afe_bk_reg_by_mode_8730e
(struct rf_info *rf, enum phl_phy_idx phy, enum rf_path path, bool is_dbcc)
{
	RF_DBG(rf, DBG_RF_TXGAPK, "[TXGAPK]======> %s\n", __func__);
...
void _halrf_do_non_dbcc_txgapk_8730e(struct rf_info *rf,
				     enum phl_phy_idx phy)
{
	u8 i;
	...
	u32 rf_bkup[TXGAPK_RF_PATH_MAX_8730E][TXGAPK_RF_REG_NUM_8730E] = {{0}};
	u32 rf_reg[TXGAPK_RF_REG_NUM_8730E] = {0xdf};

	RF_DBG(rf, DBG_RF_TXGAPK, "[TXGAPK]======> %s   phy=%d\n", __func__, phy);


	_txgapk_backup_bb_registers_8730e(rf, phy, bb_reg, bb_reg_backup,
					  backup_num);

	for (i = 0; i < TXGAPK_RF_PATH_MAX_8730E; i++) {
		_halrf_txgapk_bkup_rf_8730e(rf, rf_reg, rf_bkup, i);
	}
	...
	for (i = 0; i < 2; i++) {
		/*_halrf_txgapk_iqk_dpk_init_reg_8730e(rf, phy, RF_PATH_A);*/
		/*_halrf_txgapk_nctl_8730e(rf, phy, RF_PATH_A);*/

		_halrf_txgapk_write_track_table_default_8730e(rf, phy, i);
		_halrf_txgapk_write_power_table_default_8730e(rf, phy, i);

		_halrf_txgapk_bb_afe_by_mode_8730e(rf, phy, i, false);
		_halrf_txgapk_iqk_preset_by_mode_8730e(rf, phy, i, false);
		_halrf_txgapk_clk_setting_dac960mhz_by_mode_8730e(rf, phy, i, false);

		/*halrf_btc_rfk_ntfy(rf, (BIT(phy) << 4), RF_BTC_TSSI, RFK_START);*/
		/*halrf_tmac_tx_pause(rf, phy, true);*/

		_halrf_txgapk_track_table_nctl_8730e(rf, phy, i);
		_halrf_txgapk_write_track_table_8730e(rf, phy, i);
		_halrf_txgapk_power_table_nctl_8730e(rf, phy, i);
		_halrf_txgapk_write_power_table_8730e(rf, phy, i);

		/*halrf_tmac_tx_pause(rf, phy, false);*/
		/*halrf_btc_rfk_ntfy(rf, (BIT(phy) << 4), RF_BTC_TSSI, RFK_STOP);*/

		_halrf_txgapk_iqk_bk_reg_by_mode_8730e(rf, phy, i, false);
		_halrf_txgapk_afe_bk_reg_by_mode_8730e(rf, phy, i, false);
	}
	...
	for (i = 0; i < TXGAPK_RF_PATH_MAX_8730E; i++) {
		_halrf_txgapk_reload_rf_8730e(rf, rf_reg, rf_bkup, i);
	}
	...
	_txgapk_reload_bb_registers_8730e(rf, phy, bb_reg, bb_reg_backup,
					  backup_num);
}
void _halrf_do_dbcc_txgapk_8730e(struct rf_info *rf,
				 enum phl_phy_idx phy)
{
	u32 bb_reg[1] = {0x2344};
	u32 bb_reg_backup[1] = {0};
	...
	_halrf_txgapk_write_track_table_default_8730e(rf, phy, path);
	_halrf_txgapk_write_power_table_default_8730e(rf, phy, path);

	_halrf_txgapk_bb_afe_by_mode_8730e(rf, phy, path, true);
	_halrf_txgapk_iqk_preset_by_mode_8730e(rf, phy, path, true);
	_halrf_txgapk_clk_setting_dac960mhz_by_mode_8730e(rf, phy, path, true);

	/*halrf_btc_rfk_ntfy(rf, (BIT(phy) << 4), RF_BTC_TSSI, RFK_START);*/
	/*halrf_tmac_tx_pause(rf, phy, true);*/

	_halrf_txgapk_track_table_nctl_8730e(rf, phy, path);
	_halrf_txgapk_write_track_table_8730e(rf, phy, path);


	_halrf_txgapk_power_table_nctl_8730e(rf, phy, path);
	_halrf_txgapk_write_power_table_8730e(rf, phy, path);

	/*halrf_tmac_tx_pause(rf, phy, true);*/
	/*halrf_btc_rfk_ntfy(rf, (BIT(phy) << 4), RF_BTC_TSSI, RFK_STOP);*/

	_halrf_txgapk_iqk_bk_reg_by_mode_8730e(rf, phy, path, true);
	_halrf_txgapk_afe_bk_reg_by_mode_8730e(rf, phy, path, true);
...
void _halrf_txgapk_get_ch_info_8730e(struct rf_info *rf, enum phl_phy_idx phy)
{
	struct halrf_gapk_info *txgapk_info = &rf->gapk;
			   ...
			   enum phl_phy_idx phy)
{
	...
	_halrf_txgapk_get_ch_info_8730e(rf, phy);
	...
	if (rf->hal_com->dbcc_en) {
		_halrf_do_dbcc_txgapk_8730e(rf, phy);
	} else {
		_halrf_do_non_dbcc_txgapk_8730e(rf, phy);
	}
