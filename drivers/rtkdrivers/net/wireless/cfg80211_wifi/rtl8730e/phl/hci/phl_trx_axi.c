...
void phl_tx_start_axi(struct phl_info_t *phl_info)
{
	void *drv = phl_to_drvpriv(phl_info);
	_os_atomic_set(drv, &phl_info->phl_sw_tx_sts, PHL_TX_STATUS_RUNNING);
}

void phl_tx_resume_axi(struct phl_info_t *phl_info)
{
	void *drv = phl_to_drvpriv(phl_info);
	_os_atomic_set(drv, &phl_info->phl_sw_tx_sts, PHL_TX_STATUS_RUNNING);
}

void phl_req_tx_stop_axi(struct phl_info_t *phl_info)
{
	void *drv = phl_to_drvpriv(phl_info);
...
void phl_tx_stop_axi(struct phl_info_t *phl_info)
{
	void *drv = phl_to_drvpriv(phl_info);
	_os_atomic_set(drv, &phl_info->phl_sw_tx_sts, PHL_TX_STATUS_SW_PAUSE);
}

bool phl_is_tx_sw_pause_axi(struct phl_info_t *phl_info)
{
...
void phl_rx_start_axi(struct phl_info_t *phl_info)
{
	void *drv = phl_to_drvpriv(phl_info);
	_os_atomic_set(drv, &phl_info->phl_sw_rx_sts, PHL_RX_STATUS_RUNNING);
}

void phl_rx_resume_axi(struct phl_info_t *phl_info)
{
	void *drv = phl_to_drvpriv(phl_info);
	_os_atomic_set(drv, &phl_info->phl_sw_rx_sts, PHL_RX_STATUS_RUNNING);
}

void phl_req_rx_stop_axi(struct phl_info_t *phl_info)
{
	void *drv = phl_to_drvpriv(phl_info);
...
void phl_rx_stop_axi(struct phl_info_t *phl_info)
{
	void *drv = phl_to_drvpriv(phl_info);
	_os_atomic_set(drv, &phl_info->phl_sw_rx_sts, PHL_RX_STATUS_SW_PAUSE);
}

bool phl_is_rx_sw_pause_axi(struct phl_info_t *phl_info)
{
	void *drvpriv = phl_to_drvpriv(phl_info);
...
void _phl_alloc_dynamic_rxbuf_axi(struct rtw_rx_buf *rx_buf,
				  struct phl_info_t *phl_info)
{
...
static void _phl_free_rxbuf_axi(struct phl_info_t *phl_info,
				struct rtw_rx_buf *rx_buf)
{
	struct rtw_hal_com_t *hal_com = rtw_hal_get_halcom(phl_info->hal);
...
static void _phl_free_rxbuf_pool_axi(struct phl_info_t *phl_info,
				     u8 *rxbuf_pool, u8 ch_num)
{
	...
	if (NULL != ring) {
		for (i = 0; i < ch_num; i++) {
			...
			_phl_free_rxbuf_axi(phl_info, ring[i].rx_buf);
			ring[i].rx_buf = NULL;
...
_phl_alloc_rxbuf_axi(struct phl_info_t *phl_info,
		     struct rtw_rx_buf_ring *rx_buf_ring)
{
	enum rtw_phl_status pstatus = RTW_PHL_STATUS_FAILURE;
	struct rtw_hal_com_t *hal_com = rtw_hal_get_halcom(phl_info->hal);
	...
	if (RTW_PHL_STATUS_SUCCESS != pstatus) {
		_phl_free_rxbuf_axi(phl_info, rx_buf);
		rx_buf = NULL;
	}
	return rx_buf;
}
...
static enum rtw_phl_status
_phl_alloc_rxbuf_pool_axi(struct phl_info_t *phl_info, u8 ch_num) {
	enum rtw_phl_status pstatus = RTW_PHL_STATUS_FAILURE;
	struct rtw_hal_com_t *hal_com = rtw_hal_get_halcom(phl_info->hal);
	struct rtw_rx_buf_ring *rx_buf_ring = NULL;
	struct rtw_rx_buf *rx_buf = NULL;
	...
	if (NULL != rx_buf_ring)
	{
		for (i = 0; i < ch_num; i++) {
			...
			rx_buf = _phl_alloc_rxbuf_axi(phl_info,
						      &rx_buf_ring[i]);
			if (NULL == rx_buf) {
				pstatus = RTW_PHL_STATUS_RESOURCE;
				break;
	...
	{
		_phl_free_rxbuf_pool_axi(phl_info, (u8 *)rx_buf_ring, ch_num);
	}
...
static void _phl_free_wd_page_axi(struct phl_info_t *phl_info,
				  struct rtw_wd_page *wd_page)
{
	u16 i = 0;
...
static void _phl_free_wd_ring_axi(struct phl_info_t *phl_info, u8 *wd_page_buf,
				  u8 ch_num)
{
	...
	if (NULL != wd_page_ring) {
		for (i = 0; i < ch_num; i++) {
			...
			_phl_free_wd_page_axi(phl_info,
...
static struct rtw_wd_page *_phl_alloc_wd_page_axi(
	struct phl_info_t *phl_info, _os_list *list)
{
	...
	if (RTW_PHL_STATUS_SUCCESS != pstatus) {
		_phl_free_wd_page_axi(phl_info, wd_page);
		wd_page = NULL;
	}
...
static enum rtw_phl_status
_phl_alloc_wd_ring_axi(struct phl_info_t *phl_info, u8 ch_num) {
	enum rtw_phl_status pstatus = RTW_PHL_STATUS_FAILURE;
	struct rtw_wd_page_ring *wd_page_ring = NULL;
	struct rtw_wd_page *wd_page = NULL;
	void *drv_priv = NULL;
	...
	if (NULL != wd_page_ring)
	{
		for (i = 0; i < ch_num; i++) {
			...
			wd_page = _phl_alloc_wd_page_axi(phl_info,
	...
	{
		_phl_free_wd_ring_axi(phl_info, (u8 *)wd_page_ring, ch_num);
	}
	FUNCOUT_WSTS(pstatus);
...
static void _phl_free_rxbd_axi(struct phl_info_t *phl_info,
...
static enum rtw_phl_status
_phl_alloc_rxbd_axi(struct phl_info_t *phl_info, u8 ch_num) {
	enum rtw_phl_status pstatus = RTW_PHL_STATUS_FAILURE;
	struct rtw_hal_com_t *hal_com = rtw_hal_get_halcom(phl_info->hal);
	...
	if (NULL != rxbd)
	{
		for (i = 0; i < ch_num; i++) {
			...
			_os_mem_set(phl_to_drvpriv(phl_info), rxbd[i].vir_addr, 0, buf_len);
	...
	{
		_phl_free_rxbd_axi(phl_info, (u8 *)rxbd, ch_num);
	}
	FUNCOUT_WSTS(pstatus);
...
static void _phl_free_txbd_axi(struct phl_info_t *phl_info, u8 *txbd_buf,
			       u8 ch_num)
{
...
static enum rtw_phl_status
_phl_alloc_txbd_axi(struct phl_info_t *phl_info, u8 ch_num) {
	enum rtw_phl_status pstatus = RTW_PHL_STATUS_FAILURE;
	struct rtw_hal_com_t *hal_com = rtw_hal_get_halcom(phl_info->hal);
	struct tx_base_desc *txbd = NULL;
	u32 buf_len = 0;
	...
	{
		_phl_free_txbd_axi(phl_info, (u8 *)txbd, ch_num);
	}
	FUNCOUT_WSTS(pstatus);
...
static void phl_rx_reset_axi(struct phl_info_t *phl_info)
{
...
void _phl_tx_flow_ctrl_axi(struct phl_info_t *phl_info, _os_list *sta_list)
{
	/* _phl_sort_ring_by_hw_res(phl_info); */
	phl_tx_flow_ctrl(phl_info, sta_list);
}

static enum rtw_phl_status _phl_handle_xmit_ring_axi
(struct phl_info_t *phl_info,
...
static void _phl_release_tx_done_res(struct phl_info_t *phl_info)
{
	...
	wd_ring = (struct rtw_wd_page_ring *)hci_info->wd_ring;
	for (ch = 0; ch < hci_info->total_txch_num; ch++) {
		if(trx_ops->check_tx_done(hal, ch)) {
			seq_idx = wd_ring[ch].wp_seq_start;

			/* polling pending wd to release */
			trx_ops->query_tx_res(hal_com, ch, &host_idx, &hw_idx);
			while (wd_ring[ch].hw_idx != hw_idx) {
			...
			/* condition seq_idx == wd_ring[ch].wp_seq_idx, the last one */
			wd_ring[ch].wp_seq_start = seq_idx;
		}
	}
...
static void _phl_tx_callback_axi(void *context)
{
	...
	_phl_release_tx_done_res(phl_info);
	...
	if (true == phl_check_xmit_ring_resource(phl_info, &sta_list)) {
		_phl_tx_flow_ctrl_axi(phl_info, &sta_list);

		phl_list_for_loop_safe(ring_sts, t, struct phl_ring_status,
				       &phl_info->t_fctrl_result, list) {
			list_del(&ring_sts->list);
			_phl_handle_xmit_ring_axi(phl_info, ring_sts);
...
static void _phl_rx_callback_axi(void *context)
{
	enum rtw_phl_status pstatus = RTW_PHL_STATUS_FAILURE;
	struct rtw_phl_handler *phl_handler
		= (struct rtw_phl_handler *)phl_container_of(context,
...
void phl_trx_resume_axi(struct phl_info_t *phl_info, u8 type)
{
	if (PHL_CTRL_TX & type) {
		phl_tx_resume_axi(phl_info);
	}

	if (PHL_CTRL_RX & type) {
		phl_rx_resume_axi(phl_info);
	}
}

void phl_trx_reset_axi(struct phl_info_t *phl_info, u8 type)
{
	struct rtw_phl_com_t *phl_com = phl_info->phl_com;
	...
	if (PHL_CTRL_TX & type) {
		phl_tx_reset_axi(phl_info);
		phl_reset_tx_stats(phl_stats);
	}

	if (PHL_CTRL_RX & type) {
		phl_rx_reset_axi(phl_info);
		phl_reset_rx_stats(phl_stats);
	}
}

void phl_trx_stop_axi(struct phl_info_t *phl_info)
{
	phl_trx_reset_axi(phl_info, PHL_CTRL_TX | PHL_CTRL_RX);
}

void phl_trx_deinit_axi(struct phl_info_t *phl_info)
{
	struct hci_info_t *hci_info = phl_info->hci;
	FUNCIN();
	_phl_free_rxbuf_pool_axi(phl_info, hci_info->rxbuf_pool,
				 hci_info->total_rxch_num);
	hci_info->rxbuf_pool = NULL;

	_phl_free_rxbd_axi(phl_info, hci_info->rxbd_buf,
			   hci_info->total_rxch_num);
	hci_info->rxbd_buf = NULL;

	_phl_free_wd_ring_axi(phl_info, hci_info->wd_ring,
...
enum rtw_phl_status phl_trx_init_axi(struct phl_info_t *phl_info)
{
	enum rtw_phl_status pstatus = RTW_PHL_STATUS_FAILURE;
	struct hci_info_t *hci_info = phl_info->hci;
	...
	do {
		...
		tx_handler->callback = _phl_tx_callback_axi;
		...
		rx_handler->callback = _phl_rx_callback_axi;
		...
		/* allocate wd page */
		pstatus = _phl_alloc_wd_ring_axi(phl_info, txch_num);
		if (RTW_PHL_STATUS_SUCCESS != pstatus) {
			break;
		}
		...
		/* allocate rx bd */
		pstatus = _phl_alloc_rxbd_axi(phl_info, rxch_num);
		if (RTW_PHL_STATUS_SUCCESS != pstatus) {
			break;
		}
		/* allocate wd page */
		pstatus = _phl_alloc_rxbuf_pool_axi(phl_info, rxch_num);
		if (RTW_PHL_STATUS_SUCCESS != pstatus) {
			break;
		}
...
enum rtw_phl_status phl_trx_config_axi(struct phl_info_t *phl_info)
{
	enum rtw_phl_status pstatus = RTW_PHL_STATUS_FAILURE;
	enum rtw_hal_status hstatus = RTW_HAL_STATUS_FAILURE;
	struct hci_info_t *hci_info = phl_info->hci;
...
enum rtw_phl_status
phl_prepare_tx_axi(struct phl_info_t *phl_info, struct rtw_xmit_req *tx_req) {
	enum rtw_phl_status pstatus = RTW_PHL_STATUS_FAILURE;
	enum rtw_hal_status hstatus = RTW_HAL_STATUS_FAILURE;
	struct rtw_hal_com_t *hal_com = rtw_hal_get_halcom(phl_info->hal);
	...
	do
	{
		...
		wd_ring[dma_ch].wp_seq_idx = wp_seq;
		...
		wd_page->total_len = phl_pkt_req.total_len;
		wd_page->seg_num = phl_pkt_req.seg_num;

		if (RTW_HAL_STATUS_SUCCESS == hstatus) {
			//hci_info->wp_seq[mid] = phl_pkt_req.wp_seq;
			hci_info->wp_seq[mid] = wp_seq;
				      ...
				      struct rtw_phl_rx_pkt **pphl_rx)
{
	...
	struct hal_spec_t *hal_spec = phl_get_ic_spec(phl_info->phl_com);
	struct hci_info_t *hci_info = (struct hci_info_t *)phl_info->hci;
	struct rx_base_desc *rxbd = (struct rx_base_desc *)hci_info->rxbd_buf;
	u8 *target_rxbd = NULL;
	u16 buf_size = 0;
...
static void phl_rx_process_axi(struct phl_info_t *phl_info,
			       struct rtw_phl_rx_pkt *phl_rx)
{
	switch (phl_rx->type) {
...
#else
		phl_rx_mdate_to_phy_sts(phl_info, phl_rx);
		phl_rx_handle_normal(phl_info, phl_rx);
#endif
#else
		phl_rx_handle_normal(phl_info, phl_rx);
	...
	case RTW_RX_TYPE_MAX:
		PHL_TRACE(COMP_PHL_RECV, _PHL_WARNING_, "phl_rx_process_axi(): Unsupported case:%d, please check it\n",
			  phl_rx->type);
		phl_recycle_rx_buf(phl_info, phl_rx);
		break;
	default :
...
static enum rtw_phl_status phl_rx_axi(struct phl_info_t *phl_info)
{
	enum rtw_phl_status pstatus = RTW_PHL_STATUS_FAILURE;
	struct hci_info_t *hci_info = (struct hci_info_t *)phl_info->hci;
	struct rtw_hal_com_t *hal_com = rtw_hal_get_halcom(phl_info->hal);
	...
	for (ch = 0; ch < hci_info->total_rxch_num; ch++) {
		...
		for (i = 0; i < rxcnt; i++) {
			...
			/* hana_todo */
			phl_rx->r.mdata.dma_ch = ch;
			phl_rx_process_axi(phl_info, phl_rx);
		}
...
enum rtw_phl_status phl_pltfm_tx_axi(struct phl_info_t *phl_info, void *pkt)
{
	enum rtw_phl_status pstatus = RTW_PHL_STATUS_SUCCESS;
	return pstatus;
}

void *phl_get_txbd_buf_axi(struct phl_info_t *phl_info)
{
	struct hci_info_t *hci_info = phl_info->hci;
...
void *phl_get_rxbd_buf_axi(struct phl_info_t *phl_info)
{
	struct hci_info_t *hci_info = phl_info->hci;
...
void phl_recycle_rx_pkt_axi(struct phl_info_t *phl_info,
			    struct rtw_phl_rx_pkt *phl_rx)
{
#ifdef CONFIG_DYNAMIC_RX_BUF
...
enum rtw_phl_status phl_register_trx_hdlr_axi(struct phl_info_t *phl_info)
{
	enum rtw_phl_status pstatus = RTW_PHL_STATUS_FAILURE;
	struct rtw_phl_handler *tx_handler = &phl_info->phl_tx_handler;
	struct rtw_phl_handler *rx_handler = &phl_info->phl_rx_handler;
...
void
phl_tx_watchdog_axi(struct phl_info_t *phl_info)
{
...
{
	ops.hci_trx_init = phl_trx_init_axi;
	ops.hci_trx_deinit = phl_trx_deinit_axi;
	ops.prepare_tx = phl_prepare_tx_axi;
	ops.recycle_rx_buf = phl_release_target_rx_buf;
	ops.tx = phl_tx_axi;
	ops.rx = phl_rx_axi;
	ops.trx_cfg = phl_trx_config_axi;
	ops.trx_stop = phl_trx_stop_axi;
	ops.recycle_busy_wd = phl_recycle_busy_wd;
	ops.read_hw_rx = phl_read_hw_rx;
	ops.pltfm_tx = phl_pltfm_tx_axi;
	ops.trx_reset = phl_trx_reset_axi;
	ops.trx_resume = phl_trx_resume_axi;
	ops.req_tx_stop = phl_req_tx_stop_axi;
	ops.req_rx_stop = phl_req_rx_stop_axi;
	ops.is_tx_pause = phl_is_tx_sw_pause_axi;
	ops.is_rx_pause = phl_is_rx_sw_pause_axi;
	ops.get_txbd_buf = phl_get_txbd_buf_axi;
	ops.get_rxbd_buf = phl_get_rxbd_buf_axi;
	ops.recycle_rx_pkt = phl_recycle_rx_pkt_axi;
	ops.register_trx_hdlr = phl_register_trx_hdlr_axi;
	ops.rx_handle_normal = phl_rx_handle_normal;
	ops.tx_watchdog = phl_tx_watchdog_axi;
}
