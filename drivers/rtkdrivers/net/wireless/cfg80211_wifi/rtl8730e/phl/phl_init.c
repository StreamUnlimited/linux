...
static enum rtw_phl_status phl_set_hci_ops(struct phl_info_t *phl_info)
{
	...
	if (phl_get_hci_type(phl_info->phl_com) == RTW_HCI_AXI) {
		phl_hook_trx_ops_axi(phl_info);
	}
#endif
...
u8 rtw_phl_sys_read8(void *phl, u32 base, u32 addr)
{
	struct phl_info_t *phl_info = (struct phl_info_t *)phl;

	return rtw_hal_sys_read8(phl_info->hal, base, addr);
}
u16 rtw_phl_sys_read16(void *phl, u32 base, u32 addr)
{
	struct phl_info_t *phl_info = (struct phl_info_t *)phl;

	return rtw_hal_sys_read16(phl_info->hal, base, addr);
}
u32 rtw_phl_sys_read32(void *phl, u32 base, u32 addr)
{
	struct phl_info_t *phl_info = (struct phl_info_t *)phl;

	return rtw_hal_sys_read32(phl_info->hal, base, addr);
}
void rtw_phl_sys_write8(void *phl, u32 addr, u32 base, u8 val)
{
	struct phl_info_t *phl_info = (struct phl_info_t *)phl;

	rtw_hal_sys_write8(phl_info->hal, addr, base, val);
}
void rtw_phl_sys_write16(void *phl, u32 addr, u32 base, u16 val)
{
	struct phl_info_t *phl_info = (struct phl_info_t *)phl;

	rtw_hal_sys_write16(phl_info->hal, addr, base, val);
}
void rtw_phl_sys_write32(void *phl, u32 addr, u32 base, u32 val)
{
	struct phl_info_t *phl_info = (struct phl_info_t *)phl;

	rtw_hal_sys_write32(phl_info->hal, addr, base, val);
}
...
#ifdef CONFIG_AXI_HCI
		phl_status = rtw_phl_tx_done(phl);
#endif
	}
