...
void _mcc_dump_pattern(struct rtw_phl_mcc_pattern *m_pattern)
{
	...
	PHL_TRACE(COMP_PHL_MCC, _PHL_INFO_,
		  ">>> _mcc_dump_pattern(): tob_r(%d), toa_r(%d), tob_a(%d), toa_a(%d), bcns_offset(%d), calc_fail(%d), d_r_d_a_spacing_max(%d), c_en(%d)\n",
...
bool _mcc_adjust_dur_for_2g_mcc_2role_bt(struct phl_mcc_info *minfo)
{
	...
	PHL_TRACE(COMP_PHL_MCC, _PHL_INFO_,
		  "_mcc_adjust_dur_for_2g_mcc_2role_bt(): mcc_intvl(%d), bt_dur(%d), d1(%d), d2(%d), d1_min(%d), d1_max(%d), d2_min(%d), d2_max(%d)\n",
		...
		struct rtw_phl_mcc_role *role_2g, struct rtw_phl_mcc_role *role_non_2g)
{
	...
	if (plcy_non2g->dur_info.dur_limit.enable &&
		...
		PHL_TRACE(COMP_PHL_MCC, _PHL_WARNING_,
			  "_mcc_adjust_dur_for_2_band_mcc_2role_bt(): dur_limit.max_dur(%d) < bt_dur(%d), We can't adjust 5G duration(%d) for BT\n",
	...
	if (*coex_mode == RTW_PHL_MCC_COEX_MODE_BT_MASTER) {
		...
		PHL_TRACE(COMP_PHL_MCC, _PHL_WARNING_,
			  "_mcc_adjust_dur_for_2_band_mcc_2role_bt(): coex_mode == RTW_PHL_MCC_COEX_MODE_BT_MASTER, we adjust 5G duration for BT\n");
	} else if (*coex_mode == RTW_PHL_MCC_COEX_MODE_WIFI_MASTER) {
		PHL_TRACE(COMP_PHL_MCC, _PHL_WARNING_,
			  "_mcc_adjust_dur_for_2_band_mcc_2role_bt(): coex_mode == RTW_PHL_MCC_COEX_MODE_WIFI_MASTER, we don't adjust 5G duration for BT\n");
		goto exit;
	} else {
					    ...
					    )
{
	...
	PHL_TRACE(COMP_PHL_MCC, _PHL_INFO_,
		  "_mcc_calculate_start_tsf(): start_tsf(0x%08x %08x), cur_tsf(0x%08x %08x), ref_role->bcn_intvl(%d), ref_role->duration(%d)\n",
		...
		struct rtw_phl_mcc_pattern *m_pattern)
{
	...
	if (ref_bcn_intvl != HANDLE_BCN_INTVL) {
		PHL_TRACE(COMP_PHL_MCC, _PHL_WARNING_,
			  "_mcc_calculate_2_clients_worsecase_pattern(): ref_bcn_intvl(%d) != HANDLE_BCN_INTVL(%d), now, we can't calculate the pattern\n",
	...
	struct rtw_phl_mcc_pattern *m_pattern)
{
	...
	if ((bcns_offset > (m_pattern->role_ref->bcn_intvl - min_bcns_offset))
		...
		PHL_TRACE(COMP_PHL_MCC, _PHL_WARNING_,
			  "_mcc_calculate_2_wrole_pattern_v2(): bcns_offset(%d) > max_offset(%d) || bcns_offset < min_offset(%d), goto calc_nego_pattern\n",
			  bcns_offset,
...
check_conflict2:
		...
		PHL_TRACE(COMP_PHL_MCC, _PHL_INFO_,
			  "_mcc_calculate_2_wrole_pattern_v2(): tob_r_cand(%d), toa_r_cand(%d), tob_a_cand(%d), toa_a_cand(%d), sum_last(%d), sum(%d)\n",
	...
	u16 ref_bcn_intvl, struct rtw_phl_mcc_pattern *m_pattern)
{
	...
	if (ref_bcn_intvl != HANDLE_BCN_INTVL) {
		PHL_TRACE(COMP_PHL_MCC, _PHL_WARNING_,
			  "_mcc_calc_2_wifi_slot_worsecase_pattern(): ref_bcn_intvl(%d) != HANDLE_BCN_INTVL(%d), now, we can't calculate the pattern\n",
			  ref_bcn_intvl, HANDLE_BCN_INTVL);
		goto exit;
	}
	_mcc_get_offset_range_for_2_clients_worsecase(d_r, d_a, ref_bcn_intvl,
		...
		struct rtw_phl_mcc_role *role_ref, struct rtw_phl_mcc_role *role_ano)
{
	...
	if (_mcc_discision_duration_for_2role_bt(minfo)) {
		...
		PHL_TRACE(COMP_PHL_MCC, _PHL_ERR_,
			  "_mcc_fill_2wrole_pattern_with_limitation(): Calc pattern fail with limitation of time slot, we ignore the limitation of time slot, it will degrade performance\n");
		goto exit;
	}
	...
	if (true == courtesy_i->c_en) {
		...
		PHL_TRACE(COMP_PHL_MCC, _PHL_INFO_,
			  "_mcc_fill_2wrole_pattern_with_limitation(): Enable courtesy function, time slot of macid:0x%x replace by time slot of macid:0x%x, courtesy_num(%d)\n",
...
decision:
	...
	PHL_TRACE(COMP_PHL_MCC, _PHL_INFO_,
		  "_mcc_2_clients_tracking(): Need to update new 2clients pattern, negative_sign(%d), diff(%d), tolerance(%d), mcc_intvl(%d)\n",
