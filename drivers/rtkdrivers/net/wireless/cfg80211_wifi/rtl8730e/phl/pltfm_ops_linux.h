...
{
	struct dvobj_priv *pobj = (struct dvobj_priv *)d;
	PAXI_DATA axi_data = dvobj_to_axi(pobj);
	struct platform_device *pdev = axi_data->pdev;

	axi_cache_inv(&pdev->dev, bus_addr_l, buf_sz, direction);
}
				   ...
				   _dma *bus_addr_h, u32 buf_sz, u8 direction)
{
	...
	axi_cache_wback(&pdev->dev, bus_addr_l, buf_sz, direction);
}
				    ...
				    u8 cache, u8 direction, void **os_rsvd)
{
	...
	if (cache) {
		return axi_alloc_cache_mem(&pdev->dev, bus_addr_l, buf_sz, direction);
	} else {
		return axi_alloc_noncache_mem(&pdev->dev, bus_addr_l, buf_sz);
	}
}
				  ...
				  u8 cache, u8 direction, void *os_rsvd)
{
	...
	if (cache) {
		return axi_free_cache_mem(&pdev->dev, vir_addr, bus_addr_l, buf_sz, direction);
	} else {
		return axi_free_noncache_mem(&pdev->dev, vir_addr, bus_addr_l, buf_sz);
	}
}
...
static __inline u8 _os_thread_deinit(void *drv_priv, _os_thread *thread)
{
	if (CHK_THREAD_STATUS(thread, THREAD_STATUS_STARTED)) {
		...
		return (rtw_thread_stop(thread->thread_handler) == true) ?
...
#include <axi_ops_linux.h>
static inline u8 _os_read8_axi(void *d, u32 addr)
{
	return os_axi_read8((struct dvobj_priv *)d, addr);
}
static inline u16 _os_read16_axi(void *d, u32 addr)
{
	return os_axi_read16((struct dvobj_priv *)d, addr);

}
static inline u32 _os_read32_axi(void *d, u32 addr)
{
...
static inline int _os_write16_axi(void *d, u32 addr, u16 val)
{
	return os_axi_write16((struct dvobj_priv *)d, addr, val);
}
...
{
	return os_axi_sys_read8((struct dvobj_priv *)d, base, addr);
}
static inline u16 _os_sys_read16_axi(void *d, u32 base, u32 addr)
{
	return os_axi_sys_read16((struct dvobj_priv *)d, base, addr);

}
static inline u32 _os_sys_read32_axi(void *d, u32 base, u32 addr)
{
	return os_axi_sys_read32((struct dvobj_priv *)d, base, addr);
}

static inline int _os_sys_write8_axi(void *d, u32 base, u32 addr, u8 val)
{
	return os_axi_sys_write8((struct dvobj_priv *)d, base, addr, val);
}
static inline int _os_sys_write16_axi(void *d, u32 base, u32 addr, u16 val)
{
	return os_axi_sys_write16((struct dvobj_priv *)d, base, addr, val);
}
static inline int _os_sys_write32_axi(void *d, u32 base, u32 addr, u32 val)
{
	return os_axi_sys_write32((struct dvobj_priv *)d, base, addr, val);
}
#endif/*#ifdef CONFIG_AXI_HCI*/
