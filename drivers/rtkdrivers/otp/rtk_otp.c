...
static int rtk_otp_readb(struct rtk_otp *rtk_otp, u32 addr, u8 *data, int mode)
{
	...
	if (addr >= rtk_otp->mem_len) {
		dev_err(rtk_otp->dev, "[%s] read addr: %x, mem_len: %x\n", __func__, addr, rtk_otp->mem_len);
		*data = 0xff;
		ret = -1;
		goto exit;
	}
	...
	if (idx < RTK_OTP_POLL_TIMES) {
		*data = (u8)(val & RTK_OTPC_MASK_EF_DATA_NS);
		dev_dbg(rtk_otp->dev, "[%s] addr %x: 0x%02x\n", __func__, addr, *data);
	} else {
		*data = 0xff;
		ret = -1;
		dev_err(rtk_otp->dev, "[%s] read addr: %x failed\n", __func__, addr);
	}
			...
			unsigned int reg, void *buf, size_t bytes)
{
	...
	if (addr + bytes > RTK_OTP_LMAP_LEN) {
		dev_err(rtk_otp->dev, "LogicalMap Read error %x+%x exceed limit\n", addr, bytes);
		return -1;
	}
	...
	while (otp_addr < RTK_LOGICAL_MAP_SECTION_LEN) {
		...
		if ((otp_addr & 0x03) != 0) {
			dev_err(rtk_otp->dev, "alignment error %x %x \n", otp_addr, otp_data);
		}
	}
...
static int rtk_otp_writeb(struct rtk_otp *rtk_otp, u32 addr, u8 data)
{
	...
	if (addr >= rtk_otp->mem_len) {
		dev_err(rtk_otp->dev, "[%s] write addr: %x, mem_len: %x\n", __func__, addr, rtk_otp->mem_len);
		ret = -1;
		goto exit;
	}
	...
	if (idx >= RTK_OTP_POLL_TIMES) {
		ret = -1;
		dev_err(rtk_otp->dev, "[%s] write addr: %x failed\n", __func__, addr);
	}
...
static int rtk_otp_write(struct rtk_otp *rtk_otp, u32 addr, u8 data)
{
	...
	if (rtk_otp_readb(rtk_otp, addr, &val, RTK_OTP_PGR_MODE) < 0) {
		dev_err(rtk_otp->dev, "%s addr: %x PMR read error\n", __func__, addr);
		ret = -1;
		goto exit;
	}
...
retry:
	...
	if (rtk_otp_writeb(rtk_otp, addr, data) < 0) {
		dev_err(rtk_otp->dev, "%s addr: %x otp write error\n", __func__, addr);
		ret = -1;
		goto exit;
	}
	...
	/*Read after program*/
	if (rtk_otp_readb(rtk_otp, addr, &val, RTK_OTP_PGR_MODE) < 0) {
		dev_err(rtk_otp->dev, "%s addr: %x PMR read after program error\n", __func__, addr);
		ret = -1;
		goto exit;
	}
	...
	if (val != target) {
		if (retry++ >= RETRY_COUNT) {
			dev_err(rtk_otp->dev, "%s addr: %x read check error\n", __func__, addr);
			ret = -1;
			goto exit;
		} else
			goto retry;
		...
		u16 offset, u8 bytes, u8 *data, int mode)
{
	...
	if (mode == RTK_OTP_PG_WORD && ((bytes > RTK_OTP_LPGPKT_SIZE) || ((bytes & 0x03) != 0))) {
		dev_err(rtk_otp->dev, "%s size error :%x len:%x \n", __func__, offset, bytes);
		ret = -1;
		goto exit;
	}

	if (offset > RTK_OTP_LMAP_LEN) {
		dev_err(rtk_otp->dev, "%s %x exceed limit\n", __func__, offset);
		ret = -1;
		goto exit;
	}
	...
	if (idx + bytes > RTK_LOGICAL_MAP_SECTION_LEN) {
		dev_err(rtk_otp->dev, "otp_pg_packet no enough space %x\n", idx);
		ret = -1;
		goto exit;
	}
			...
			unsigned int reg, void *val, size_t bytes)
{
	...
	while (remain > 0) {
		...
		if (ret < 0) {
			dev_err(rtk_otp->dev, "LogicalMap Read error when write %x\n", base);
			ret = -1;
			goto exit;
		}
...
static int rtk_otp_probe(struct platform_device *pdev)
{
	...
	if (!res) {
		dev_err(&pdev->dev, "failed to get base sys resource\n");
		ret = -ENOMEM;
		goto exit;
	}
	...
	if (!rtk_otp->sys_base) {
		dev_err(&pdev->dev, "failed to remap base sys resource\n");
		ret = -ENXIO;
		goto exit;
	}
