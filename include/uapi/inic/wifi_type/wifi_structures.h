...
#define INIC_MAX_PASSWORD_LENGTH (65)
#define INIC_MAX_CHANNEL_NUM (51)
...
typedef struct wifi_autoreconn_param {
	rtw_security_t security_type;
	char *ssid;
	int ssid_len;
	char *password;
...
typedef void (*p_wlan_autoreconnect_hdl_t)(wifi_autoreconn_param_t *recon_param);
...
typedef struct rtw_wifi_setting {
	...
	unsigned char 		ssid[INIC_MAX_SSID_LENGTH];   /**< the ssid of connected AP or softAP */
	...
	unsigned char 		password[INIC_MAX_PASSWORD_LENGTH];   /**< the password of connected AP or softAP */
...
typedef struct rtw_wifi_config {
	...
	unsigned char 		ssid[INIC_MAX_SSID_LENGTH];
	unsigned char		ssid_len;
	unsigned char		security_type;
	unsigned char		password[INIC_MAX_PASSWORD_LENGTH];
	unsigned char		password_len;
	unsigned char		channel;
} rtw_wifi_config_t;
