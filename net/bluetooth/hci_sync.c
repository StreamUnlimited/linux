...
static int hci_set_ext_adv_data_sync(struct hci_dev *hdev, u8 instance)
{
	...
	pdu.cp.length = len;
	pdu.cp.handle = adv ? adv->handle : instance;
...
static int hci_le_big_create_sync(struct hci_dev *hdev, void *data)
{
	...
	pdu.cp.sync_handle = cpu_to_le16(conn->sync_handle);
	...
	err = __hci_cmd_sync_status_sk(hdev, HCI_OP_LE_BIG_CREATE_SYNC,
				       sizeof(pdu.cp) + conn->num_bis, &pdu,
				       HCI_EVT_LE_BIG_SYNC_ESTABLISHED,
				       conn->conn_timeout, NULL);
	if (err == -ETIMEDOUT)
		hci_le_big_terminate_sync(hdev, pdu.cp.handle);
